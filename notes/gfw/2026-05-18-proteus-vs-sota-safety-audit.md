...
| Probe-anomaly detector (server-side) | ❌ | ❌ | ✅ sliding-window per-/24 (v4) / /48 (v6) cover-forward counter with fire-once-per-burst Prometheus alert; defaults: 300s window / 8 events / 16 K prefix cap. **Wired across BOTH α and β failure-close-sites** so QUIC-only probers cannot bypass the alarm. **Plus a bounded ring buffer of recent fires exposed as labelled Prometheus gauges** (`proteus_probe_anomaly_recent_secs{prefix="10.0.0.0/24"}`) — operators see WHICH /24 is probing them directly in Grafana / PromQL without log-grep. Commits landed 2026-05-18 (21 tests: 14 unit + 3 α e2e + 2 β e2e + 2 metrics-export e2e) |
