...
| Probe-anomaly detector (server-side) | ❌ | ❌ | ✅ sliding-window per-/24 (v4) / /48 (v6) cover-forward counter with fire-once-per-burst Prometheus alert; **wired across BOTH α and β failure-close-sites** (QUIC-only probers cannot bypass). **Plus a bounded ring buffer of recent fires** exposed as labelled Prometheus gauges (`proteus_probe_anomaly_recent_secs{prefix="10.0.0.0/24"}`) AND surfaced in `admin status` text + JSON. **Plus operator-opt-in auto-deny loop** (`probe_anomaly.autodeny_minutes > 0`): every fire injects the /24 into an in-binary TTL-bounded deny list; subsequent connections short-circuit at `admission_ok` before firewall/budget/rate-limit checks; entries auto-expire so false positives heal. `crates/proteus-transport-alpha/src/{probe_anomaly,auto_deny}.rs` + 6 β instrumentation points. Landed 2026-05-18 (32 tests: 14 + 9 unit + 3 α e2e + 2 β e2e + 2 metrics-export e2e + 2 auto-deny-loop e2e) |
