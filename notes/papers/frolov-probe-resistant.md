...
對 G6 的核心啟示：**「不回應」是最差的策略**。所有後 2020 年的高品質 probe-resistant 協議（REALITY、ShadowTLS、Hysteria2 obfs、TUIC v5）都改採**fallback-to-real-server**：handshake 失敗時把連線透明轉發到一個合法服務（如真實網站、SSH server），讓 censor 看到真實的應用回應。這是本論文間接催生的設計範式轉移。
