...
#### 2. Proteus server 設計具體 implications
...
QUIC RFC 9000 沒有 challenge ACK 概念（沒有 RST），所以 Cao 2016 直接不適用。**但 QUIC 自己的 rate limit / token / counter 設計仍可能犯同類錯誤**——例如 stateless retry token、connection ID rotation 邏輯。**Proteus 設計 review 必須對所有 shared-state mechanism 做 Cao 2016 風格 audit**。
...
**Proteus 任何新 feature 必須過這個 review**。
...
- **應用層 protocol 是否該對 transport side channel 提供額外保護**：HTTP/2/3 是否該有 application-layer connection liveness check？Proteus 該不該設計 redundant heartbeat with crypto auth？
