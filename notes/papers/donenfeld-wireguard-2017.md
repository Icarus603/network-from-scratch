...
**One-line**: WireGuard 設計與 Linux kernel 實作的 whitepaper——用 Noise IK 握手 + ChaCha20-Poly1305 record + 極簡 4000 行 kernel code，徹底重新定義 VPN 該長什麼樣；Proteus 的 anti-censorship variant 直接 base on WireGuard。
...
- **Proteus 教訓 #1**：簡潔即安全。WireGuard 4000 LoC vs IPsec/OpenVPN 100k+ LoC 對應 attack surface 縮減。
- **Proteus 教訓 #2**：「No negotiation」是 anti-Logjam 的 architectural choice。

## Open questions
- **WireGuard 在 GFW 下生存策略**：amneziawg 是 obfuscation fork；更系統的方案？Proteus 嘗試解此。
