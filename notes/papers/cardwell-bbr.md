...
BBR throughput 優勢在 Proteus 場景（typically lossy international link）**極顯著**——CUBIC 不適合。
...
Proteus server side：Linux fq qdisc enabled by default。Client side：應用層 pacing（QUIC 內建）。

#### 4. ProbeRTT 對 Proteus anti-fingerprinting 影響

每 10 秒 cwnd 縮到 4 MSS——**packet rate pattern 變化**——可能被 GFW 識別為 BBR 特徵。**Proteus 應該 jitter ProbeRTT 週期 + 偽裝 cwnd drop**。

#### 5. Brutal 是 BBR 的反向思路

BBR 假設 loss 可能 random 但仍**有限度地** respect path。Brutal 完全**不 respect**。**Proteus 提供 opt-in Brutal 但 default 走 BBR** 是 ethical & technical balance。
...
- **BBR fingerprint 對審查的影響**：BBR distinctive pacing pattern 是否成為 Proteus 識別 vector？open
