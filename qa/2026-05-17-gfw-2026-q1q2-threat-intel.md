...
2. ~~**Probe-anomaly detector**~~ ✅ **DONE 2026-05-18** —— `ProbeAnomalyDetector` 同樣用 per-/24（v4）/ /48（v6）prefix 鍵，sliding window（預設 300s）+ threshold（預設 8）。Fire-once-per-burst：同一 /24 在 window 內第一次過閾值即 emit 一次 WARN log + bump `proteus_probe_anomalies_fired_total` Prometheus counter，後續同 burst 沉默直到 window 清空。Memory cap 16 KiB prefixes（~1 MiB bookkeeping）。
   ...
   - **2026-05-18 P1 follow-up**：`AutoDenyList` TTL-bounded in-binary deny list — operator 設 `probe_anomaly.autodeny_minutes > 0` 啟用：每次 fire 自動把 /24 / /48 加入 deny list，後續連線在 `admission_ok` 最頂端 short-circuit（比 firewall snapshot + global handshake budget + rate-limiter 都更早），TTL 過後自動清除（false positive 自癒）。9 unit + 2 e2e tests。**Alerting 與 enforcement 分層**：detector 永遠 alert，auto-deny 只在 operator opt-in 時才 enforce — 預設不啟用以避免 false-positive 鎖死真實 client。
   - **2026-05-18 β pre-handshake hardening**：β 的 accept loop 在 `quinn::Incoming::remote_address()` 階段（QUIC handshake 完成之前）就 consult auto-deny list；命中即 `Incoming::ignore()`，quinn 完全不發任何 wire packet。從 prober 的視角伺服器像不存在一樣。**省下完整 TLS+QUIC handshake 的 ML-KEM CPU 成本** — 一個被 anomaly detector flag 過的 prober 再來打 β 不會消耗任何 handshake budget。+2 β wire-level e2e tests 證明 client connect 真的 time out 而非看到 CONNECTION_CLOSE。
   - **2026-05-18 auto-deny current-state surface（本次 iteration）**：之前只有「fire 事件」可見（透過 recent-fires ring，~16 entries），但 auto-deny entries 的 TTL 可遠長於 ring window — operator 失去「現在誰被擋住、還剩多久」的能見度。新增：(a) Prometheus 系列 `proteus_auto_deny_active_prefixes`（gauge）/ `proteus_auto_deny_inserted_total`（counter）/ `proteus_auto_deny_refused_inserts_total`（counter）/ 每個 entry 一條 `proteus_auto_deny_remaining_secs{prefix="…"}` 標籤式 gauge；(b) `admin status` 新增「Auto-deny list (operator-opt-in)」文字區塊（quiet by default，soonest-first，capped at 20）；(c) JSON 新增 `auto_deny_active` / `auto_deny_inserted_total` / `auto_deny_refused_inserts_total` / `auto_deny_entries[]`。**13 unit tests** (7 in `auto_deny.rs` + 6 in `admin.rs`)；back-compat shims `serve_with_auth_full` / `serve_on_listener_full` / `render_full` forward to `_v2` 變體傳 `auto_deny = None`，舊呼叫端零改動。
