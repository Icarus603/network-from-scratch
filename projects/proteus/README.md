...
| 4 | Application-layer active probing + timing analysis on cover URLs | escalating | ✅ cover-server splice + NO_ERROR closes; ✅ **cover-endpoint pool with per-src-IP /24 affinity** (2026-05-18, no rotation signal to single observer); ✅ **server-side probe-anomaly detector wired across both α and β** (2026-05-18, sliding-window per-/24 counter + Prometheus alert); ✅ **detector recent-fires ring exposed via `/metrics`** (2026-05-18, `proteus_probe_anomaly_recent_secs{prefix="…"}` labelled gauges → operator sees WHICH /24 fired directly in PromQL/Grafana, no log-grep required) |
