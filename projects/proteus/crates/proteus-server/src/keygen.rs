...
pub fn run_with_force(out_dir: &Path, force: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Iter-167: wrap the SK byte serialization in Zeroizing so the
    // Vec<u8> backing buffer is scrubbed when the function returns.
    // Pre-iter-167 the raw decapsulation-key bytes lingered on the
    // heap after the file write, joining the dozens of other
    // ml-kem-related stack/heap residues iter-163/164 closed for
    // the runtime crypto paths.
    let mlkem_sk_bytes = Zeroizing::new(mlkem_sk.as_bytes().to_vec());
    ...
    // Iter-167: same Zeroizing wrap for the X25519 SK serialization.
    // `StaticSecret::to_bytes()` returns a bare `[u8; 32]` that does
    // NOT zeroize on drop (StaticSecret itself is ZeroizeOnDrop, but
    // `to_bytes()` makes a copy).
    let x_sk_bytes = Zeroizing::new(x_sk.to_bytes());
    write_b64(&out_dir.join("server_lt.x25519.sk"), &*x_sk_bytes)?;
...
fn write_b64(path: &Path, bytes: &[u8]) -> std::io::Result<()> {
    ...
    // Iter-167: the base64 encoding of any of these keys is
    // directly substitutable for the raw bytes (via base64
    // decode), so the intermediate `b64` and `body` Strings need
    // their backing buffers scrubbed on drop. The pk-path also
    // goes through this helper where the encoding is non-secret;
    // conservatively scrubbing both costs ~one memset per call.
