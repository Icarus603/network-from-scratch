...
pub struct ServerConfig {
    ...
    /// base64-encoded, generated by `proteus-server knock-keygen`).
    /// **Path A foundation for REALITY-grade probe resistance**:
    /// when set, the binary loads the PSK at startup. The
    /// pre-auth-passthrough transport layer (separate iteration)
    /// consumes this PSK via [`proteus_handshake::knock::verify_knock`]
    /// to gate "is this a real Proteus client or a GFW probe?"
    /// — connections without a valid knock get transparently
    /// forwarded to the cover endpoint, indistinguishable from a
    /// real HTTPS reverse proxy.
    ///
    /// Operator distributes the SAME PSK bytes to every Proteus
    /// client out-of-band (same channel as the existing
    /// `server_lt.*.pk` bundle). Rotation is independent from
    /// the identity-key lifecycle — rotate when a client device
    /// is lost, not when ML-KEM rolls.
    ///
    /// Unset (default): probe-resistance gate is OFF, the binary
    /// runs in the existing auth-fail-then-cover-forward mode.
    /// Operators wanting probe resistance MUST mint a PSK and
    /// set this field; the absence of the PSK is a deliberate
    /// opt-in (the protocol still works end-to-end without it).
    ///
    /// File format: hand-editable, two comment lines + one
    /// base64 line. See `knock_keygen.rs` and
    /// `proteus_handshake::knock` for the wire-level details.
    #[serde(default)]
