...
3. **PT spec 是公開的——所有設計細節都可學**。我們的 G6 必須能在 PT 框架下作為 transport 註冊，否則無法 leverage Tor 生態。
...
**對 G6 的意義**：programmable shaping 是好概念，Maybenot 是後續落地版（Part 10.5）。
...
- Decoy routing concept 太重——G6 不打算走 ISP-cooperation 路線。但 Conjure 「dark IP」概念可借鑑：bridge IP 看起來像 unused 但響應 protocol。
...
- G6 必須用 uTLS（或同類 library），否則 client side TLS fingerprint 直接洩。

**Part 7 / Part 8 與 G6 設計**：uTLS 是 G6 client transport library 的 baseline。
...
| **G6 計畫** | hybrid | 多 profile | 強 | 強 | optional | – |
...
7. **「Look like nothing」（FTE 走的路）** 與 **「Look like everything benign」（meek/Snowflake 走的）** 是兩條哲學路線。G6 走後者。

## 與我們協議設計的關聯

1. **G6 必須能作為 Tor PT 註冊**：寫 pt-spec v2 compliant binary，Tor 用戶能 plug-in。
...
4. **PT performance vs covertness trade-off**：obfs4 fast but unsafe (2024); Snowflake covert but slow. 能否同時兼具？G6 中心命題。
