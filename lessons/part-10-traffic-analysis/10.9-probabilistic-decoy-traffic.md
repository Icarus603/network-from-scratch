...
本堂梳理 cover-traffic 設計空間，並評估 Proteus 是否可採用部分 cover-traffic（"selective cover"）。
...
**Proteus 屬於 low-latency**——但要研究能不能引入 partial cover traffic 提升保證。
...
- **Cover loops 概念可借鑑**——Proteus client 在 idle 時持續發 cover packets 到 Proteus server（或 self-loop）。
...
### 七、selective cover：Proteus 可採的妥協方案

**完整 cover (Loopix-style) 太貴**。Proteus 可採 **selective cover**：

```mermaid
flowchart TD
    A[Proteus client] --> B{user activity?}
...
- 在 user idle 期間，Proteus client 在 background 對 「decoy real CDN」（如 fetch JSON 從 ajax.googleapis.com）做 periodic fetch。
...
### 八、Constant-rate cover 與 Proteus capacity bound
...
**Proteus 模式 B "high-assurance mode"**：constant-rate 36 kbps，模擬 VoIP session。對 voice/messaging 用例適合，對 web 不適合。
...
如果 cover loop 都送到 Proteus server，對手見到 「持續流到 Proteus server」 itself 是 signature。**Cover destination 應多樣化**（多 CDN、多 endpoint）。
...
**對 Proteus**：cover-traffic 帶來的 capacity 下降是 quantifiable。Part 11 設計 Proteus 時，evaluate cover rate $\rho_z$ → 量 $I$ 下降，挑 Pareto-optimal $\rho_z$。

## 與我們協議設計的關聯

1. **Proteus default mode = light selective cover**：~5–10 KB/min，destination 多樣化。
2. **Proteus high-assurance mode = constant-rate 36 kbps**：voice/messaging 用，capacity $\approx 0$。
3. **Cover packet 必須 indist from real**：size、timing、destination 全 match。
4. **Avoid 「cover only when idle」陷阱**：always-on。
5. **形式化 capacity report**：在 Proteus evaluation 中給 $(\rho_z, \hat{C})$ 表。
...
3. Selective cover (Proteus default) 與 always-on cover 的安全差異？什麼 attack 是 selective 無法 cover 的？
4. Constant-rate channel 為什麼能達到 $I(X; Y) = 0$（理論上）？實務上達不到的原因？
5. Cover destination 若集中在 Proteus server，會洩什麼資訊？Proteus design 怎麼避免？
...
實務上 inequality 是 strict（dummies 在 implementation level 與 real 有 subtle 差別）—— **這是 Proteus testing 的關鍵 audit point**。
...
|---|---|---|---|
| Proteus light (default) | Selective ~5KB/min, multi-CDN decoy fetch | <2% bw | medium |
| Proteus medium | Always-on cover loops to multi destinations | ~30% bw | low |
| Proteus high | Constant-rate 36kbps | 100%+ bw | ~0 |
| Proteus messaging | Vuvuzela-style batched (future) | round-trip latency | DP bound |
...
1. **Proteus selective cover 的 capacity formula**：sparse cover 到底 reduce 多少 $I$？沒 closed-form。
...
4. **DP-style anonymity bound for low-latency systems**：Tor / Proteus 是否能給出 DP-form 保證？目前 high-latency systems 才有。
