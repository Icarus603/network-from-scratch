...
**Proteus 必須用 uTLS 或同類庫**。直接用 Go std lib 等於自殺。
...
**uTLS Chrome 120 ≠ Chrome 121**——版本必須跟著最新瀏覽器升級。**這是 Proteus 的 maintenance burden。**
...
- For authenticated client：bridge 用自己 key 替換 ServerHello，建立 Proteus tunnel。
...
- **TLS-in-TLS detection (Wu 24)** 是 emerging concern：REALITY tunnel 內部仍跑 TLS（如果 Proteus tunnel 上有 TLS）→ 內外 TLS 可被 nested fingerprint 偵測。**Proteus 設計時要避免 「真 TLS over REALITY TLS」 這種 stack**——本身就提供 inner TLS 線索。
...
```mermaid
flowchart LR
    C[Proteus Client] -->|H3 CONNECT-UDP| B[Proteus Bridge]
...
**Proteus 拿 MASQUE 作為 transport baseline 是強選擇**——標準化、有 multi-implementation（quiche、quinn、msquic、ngtcp2）、攻擊面隨 IETF 演化共享。
...
**對 Proteus**：
- 如果 Proteus 外層 = REALITY TLS，內層 Proteus 應用 = 純 binary stream（不再 TLS），可避開 TLS-in-TLS pattern。
- 如果 user end-to-end 是 HTTPS（例：browser → Proteus client → Proteus bridge → real-https-server），這個 stack 天然有 TLS-in-TLS。**這是必然存在的 deployment scenario**。
- Proteus must shape internal TLS handshake to **不像 internal TLS handshake**——或用 inner non-TLS protocol（如 H2 plaintext over TLS）。
...
App-level fingerprinting 表明：**「即使你 wrap 整個 traffic 進 Proteus tunnel，inner app 仍可能 fingerprint」**——例如 Telegram 與 WeChat 的 inner traffic shape 不同，從 Proteus tunnel 外仍可區分。

**Proteus 必須在 inner-payload 層做 normalization**（如 padding 到 fixed-rate），或 explicitly scope out（threat model 中標明 「不 protect inner app fingerprinting」）。
...
對 Proteus：multi-flow correlation 是 next-gen attack。Proteus 應在 client 端避免「同時多 flow 都明顯 Proteus 標識」。
...
**Proteus 設計：TLS 用 REALITY + uTLS；HTTP/2 用 proper conformance；TCP-level 用 Maybenot shaping；inner payload 用 Proteus own framing（不 nest TLS）。**

## 與我們協議設計的關聯

1. **Proteus transport = MASQUE-based H3** ；handshake 走 uTLS Chrome；inner 用 Proteus framing（不 nest TLS）。
2. **Bridge auth = REALITY-style**：unauthenticated 流量 fallback 到 real CDN proxy。
3. **TLS-in-TLS-aware**：Proteus 不在 Proteus tunnel 內 nest 第二層 TLS；user app TLS 由 user 自己負責。
...
# auth client
xray run -c reality-client.json  # 應該建立 Proteus tunnel
```

### 實驗 C：跑 TLS-in-TLS detection rule on Proteus

寫 detector 找 「TLS record size + handshake-like sub-record sequence」。在 Proteus 流量上跑——確認 inner 不 nest TLS 後 detector 失效。

## 自我檢查

1. JA3 與 JA4 差別在哪？為什麼 JA4 更難 spoof？
2. ESNI、ECH、REALITY 三條路為什麼是「平行而非串行」演化？哪些 GFW 都 block 了？
3. MASQUE 的 CONNECT-UDP 在 Proteus 中扮演什麼角色？比直接用 IPSec 好在哪？
4. TLS-in-TLS detection 為什麼是 emerging threat？Proteus 怎麼設計能避免？
5. AppScanner / FlowPrint 對 Proteus 提供哪些 inner-app protection 需求？Proteus 該負責這層嗎？
...
| 屬性 | obfs4 | meek | VLESS+REALITY | **Proteus 計畫** |
|---|---|---|---|---|
...
2. **MASQUE 大規模部署的 fingerprintability**：當 H3 流量還少，Proteus H3 可能是 「outlier」；該如何 anticipate？
...
5. **Application-layer fingerprint 對 Proteus 的 scope 界定**：使用者 inside Proteus 跑哪些 app 應該被 protect？或這是 user responsibility？
