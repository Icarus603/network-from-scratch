...
10.10 給出全景。本堂把所有 attack 與 defense **配對成 matrix**——對每個 known threat，列 G6 用什麼 mechanism 應對、什麼層級覆蓋、什麼 residual risk。

這份 matrix **直接成為 Part 11.1 的 threat model 與 11.2 的 spec rationale 的草稿**。**它必須被 audited 過——任何空格意味著 G6 在該 threat 上 vulnerability。**
...
| Threat | 攻擊細節 | G6 mechanism | Residual risk |
|---|---|---|---|
...
| TLS record size pattern | 特定大小序列 | H2 record 切分為固定大小 (max ~ MTU) | TLS-in-TLS detection vulnerable; G6 不 nest TLS |
...
| WF DL classifier (DF/Tik-Tok) | learned classifier on direction+timing | Surakav-style decoy + RegulaTor envelope + Maybenot | accuracy 隨 attacker compute 增加; G6 必須 publish own eval |
...
| Concept drift exploit (Wang 16) | model retrains | G6 protocol versioning encourages defender retraining; client rotates uTLS profile periodically | – |

### 六、Inter-flow / correlation threats

| Threat | 攻擊細節 | G6 mechanism | Residual risk |
|---|---|---|---|
| DeepCorr flow correlation (Nasr 18) | 跨 hop 流量對應 | timing jitter (±25ms) + per-burst random padding | G6 latency overhead +10–25ms |
| Watermarking attack | active inject pattern | random rate-jitter in client | – |
| Co-occurrence multi-flow (Holland 21) | 同時 multiple flows pattern | G6 multipath disperses | – |
| Cross-protocol fingerprint | G6 client signature in non-G6 流量 | G6 client only routes G6 traffic over tunnel | rest of host network traffic 是 user 責任 |

### 七、Long-term / oracle threats

| Threat | 攻擊細節 | G6 mechanism | Residual risk |
|---|---|---|---|
| Oracle DNS (Pulls 20) | 對手用 DNS log confirm 訪問 | DoH inside G6 tunnel | system DNS leak 仍可能（OS level 保護） |
| CT log oracle | 對手用 CT 看 client cert | G6 client 不 issue per-session cert | – |
...
| BGP route influence | 對手影響 routing | multi-bridge geographic fallback | global infra issue, beyond G6 spec |
| User cookie / login link | inner app login | G6 不負責 application-layer privacy | scope-out, document in threat model |

### 八、G6 layered defense architecture

```mermaid
flowchart TD
    A[user app] --> B[OS network stack]
    B --> C[G6 client lib<br/>uTLS Chrome + H3 + Maybenot shape]
    C --> D[QUIC connection]
    D --> E[G6 bridge<br/>REALITY fallback + Maybenot shape]
    E --> F[Internet]
...
### 十、Residual threats (G6 不主張保護)

明確 **scope out**：

1. **Inner application traffic analysis** (Bahramali 20 messaging WF)：G6 不 protect 「user 跑哪個 app」。是 application responsibility。
...
4. **OS-level DNS leak**: 若 OS bypass G6 DNS routing。
...
| Concept drift | Maybenot rotation | uTLS rotation | G6 version bump |

### 十二、Quantitative leakage report (Part 12 evaluation 必交)

G6 在 evaluation 階段 produces：

| Metric | Target | Measurement |
|---|---|---|
| WF DF accuracy on G6 | < 20% | 1000 sites closed, 100k unmonitored open, 30-day drift |
| Tik-Tok accuracy | < 25% | 同上 |
| Surakav-aware | < 30% | with retrained adversary |
| FEP detection FPR (against G6) | < 1% | replicate Wu 23 rule set |
...
G6 不是替代 VLESS+REALITY / Hysteria2，而是 **next-step**：
- G6 server 應能同 host 上 fallback to VLESS+REALITY (port 不同)。
- G6 client 自動 try G6, then VLESS, then Hysteria2 fallback。
- Common shared API for app integration。

**對部署 friction 是 G6 採用率關鍵**。
...
填同樣 matrix 對 VLESS+REALITY 看哪些 cell 是空的（vulnerable）—得到 G6 在哪些 threat 上能超 VLESS+REALITY。

### 實驗 B：寫 threat-coverage linter

Python script 讀 G6 spec (yaml)，自動 detect 哪些 threat 沒被 mapping 到 mechanism。**這是 ongoing 設計工具，Part 11 寫 spec 時不斷跑。**

### 實驗 C：reachability self-test

部署 G6 prototype，從 5 個不同地理位置 probe（用 VPN proxy 模擬不同 ASN），測 connection success rate / latency。

## 自我檢查

1. 對 FEP entropy detection，G6 用什麼機制？為什麼 internal payload 仍 high entropy 不違反？
2. 如何避免 TLS-in-TLS detection？G6 設計 implications？
3. REALITY 如果 target CDN down，fallback 怎麼處理？bridge 在 degraded mode 下還能保證 indistinguishability 嗎？
4. concept drift attack 對 G6 是什麼樣的 threat？G6 哪三個 layer 主動 mitigate？
...
> G6 spec $\Sigma$ covers threat $T$ if exists mechanism $m \in \Sigma$ such that $T$ is mitigated to acceptable residual risk per quantitative target.
...
2. **Quantitative leakage report 標準化**：怎麼讓 G6 的 leakage report comparable to 其他 protocol？需要 community standardization。
3. **Threat / mechanism 1-to-many mapping 自動驗證**：static analysis 能否從 spec 推 coverage？目前 manual。
4. **「Cost-to-censor」量化**：每個 G6 mechanism 對 censor 的 cost 多少？沒人量化。
5. **「Strategy proof」**：如果 censor 改變策略（如改用 ML 替代 rule-based）G6 自動 adapt 嗎？暫無。
