...
對 Proteus 設計者而言，這堂課的結論很硬：**「假裝 deep learning 不存在」的 defense 必死。我們必須對 SOTA DL adversary 給出 leakage bound。**
...
**對 Proteus 的意義**：data efficiency 改變了 attacker model——對手不需要長期蒐集即可上線新 site。
...
**意義**：Walkie-Talkie 用了 supersequence 假設「兩 site 看起來一樣」——但這是在 direction 上；timing 沒被覆蓋。Tik-Tok 用 timing 一招殺穿。**所有不處理 timing 的 defense 都 vulnerable。** Proteus 的 timing module 必須是 first-class。
...
**對 Proteus 評估的意義**：Part 12 的 evaluation 必須避開所有四個陷阱，否則 reviewer 直接 reject。
...
**對 Proteus 的意義**：對手 retraining 成本是真實 friction。Proteus 可以利用——例如刻意製造高頻 page variation（如 client-side random padding pattern 每 N 天輪換），人為加大 drift rate。
...
**結論**：在 Wang14 dataset 上 Bayes-optimal ~98–99%。Tik-Tok 99.5% 等於說「已經到天花板」。**進一步提升 DL 已沒空間——攻擊端的紅利結束。Defense 端的紅利還在。** Proteus 是站在 defense 紅利這側。
...
1. **Tik-Tok 是 baseline**：Proteus 的 evaluation 必須對 Tik-Tok。對 DF / AWF 已不足以 publish。
2. **Direction × time 是核心 channel**：Proteus 的 padding 不能只 reshape sizes/burst——必須同時打 timing。對應 Part 10.5 的 timing morphing。
3. **W-T 失敗給的教訓**：靠「supersequence 兩兩相同」只覆蓋 direction，留 timing 漏洞。Proteus 必須是 cross-channel 一致。
...
5. **Bayes-optimal ceiling 給 evaluation 落地**：在 Proteus evaluation 中，目標應是把 attacker accuracy 壓到接近 random guess (1/N)，而非「比 DF 差一點」。
...
把 trace 的 timing 隨機 jitter ±50ms，重訓 Tik-Tok：accuracy 應顯著下降。量化「timing precision → leakage bits」的曲線，這直接給 Proteus timing module 的設計參數。
...
| 維度 | Wang14/CUMUL/k-FP | DF/Tik-Tok | Proteus 規劃 |
|---|---|---|---|
