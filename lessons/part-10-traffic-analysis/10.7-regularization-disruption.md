...
- **G6 必須屬於某個 「benign protocol family」**，不要走 「無協議的 random pipe」 路線。
...
**結論**：除非你能 implement 完整協議，否則別走 mimicry。**Tunneling 是 G6 路線。**

### 四、Tunneling 的「真實 X 流量」概念

G6 設計目標：**透過真正的 HTTP/2 over TLS 1.3 with uTLS-Chrome fingerprint 跑流量**。對手看到的：
...
**G6 必須產生與此 structurally 相同的 wire trace**——packet count、size 分布、timing pattern 都接近。**這比簡單 "random pad to MTU" 難 10×，但這是 the way。**
...
提醒：**即使 G6 包成 HTTPS，也可能被次層分析**——「在加密 channel 內，使用者做什麼」。**G6 不必 protect 這個層級**（這是 application 責任），但要明確 scope 界定。
...
**G6 必須走 REALITY-style fallback**：對 unauthenticated probe 完全像 「真實 web server」。
...
- **DeepCorr (Nasr 18 CCS)** 證明：在 Tor 上 90%+ correlation accuracy。即使加密，flow timing 仍 correlate。**G6 timing module 必須打 correlation channel。**
...
這些 temporal pattern 個別化，可作為使用者 identity。**G6 不打算 protect 這個層級**（屬於 user behavior，需 application 配合）；但要 document 在 threat model 中。
...
G6 的 「regularization mandate」 落地為四條 protocol rule：
...
1. **G6 走 tunneling not mimicry**：真實 HTTP/2 over TLS 1.3，不是 fake HTTP-shaped random bytes。
...
### 實驗 B：DeepCorr 對 G6 timing module
...
4. Watermarking attack 與 fingerprinting attack 的差別在哪？G6 的 timing module 同時需要對抗兩者嗎？
5. 「regularization」 mandate 對 G6 設計引入哪些 cost？哪些技術是 enabler？
...
對 G6 而言，$\mathcal{B}$ = HTTP/2 over TLS 1.3 with Chrome fingerprint。
