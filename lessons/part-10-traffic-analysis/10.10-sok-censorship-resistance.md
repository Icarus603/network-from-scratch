...
1. 用 Khattak 16 的 6-axis taxonomy 把 Proteus 設計座標清楚定位。
2. 用 Tschantz 16 的 **empiricism mandate** 規劃 Proteus evaluation methodology。
3. 用 Pulls 20 / Cherubin 22 的「現實 attack」結果調整 Proteus threat model。
...
Proteus 規劃：distributed trust（多 bridge fallback）。
...
Proteus 預設：state-level adversary, multi-vantage, ML, long-term, can pressure CDN（部分）。
...
Proteus 規劃：single-bridge default + optional MPQUIC multipath。
...
Proteus 規劃：tunneling 經由 H3/MASQUE。
...
Proteus 規劃：REALITY-style fallback。
...
Proteus 規劃：self-hosted VPS default + optional CDN-dependent fallback。
...
| **Proteus 計畫** | distributed-optional | state-level | direct + opt. MP | tunneling H3 | REALITY-style | self-host + opt. CDN |
...
對 Proteus：**Part 12 的 evaluation 必須符合這 4 條**。否則 reviewer (especially Tschantz / Houmansadr camp) 直接 reject。
...
- **Proteus 應 anticipate oracle-attacker**——避免讓 client 直接 DNS-resolve target site。
- Use **DoH over Proteus tunnel** for inner-app DNS。
- 避免使用 public DNS。
...
- Proteus 的 defense **必須從 first packet 就 active**——不能依賴 「整個 trace 結束才 shape」。
...
對 Proteus：reachability target 應 ≥ 85% (VLESS+REALITY baseline) 或更高。
...
### 十、Tschantz mandate → Proteus evaluation plan

| Mandate | Proteus plan |
|---|---|
...
1. **Khattak 6 軸 = Proteus spec sheet 欄位**。Part 11 寫 spec 時直接套這 6 軸。
2. **Tschantz mandate = Proteus paper evaluation methodology**。Part 12.20 完整實施。
3. **Pulls 20 oracle attack = Proteus 必須處理 DNS**：DoH-over-tunnel + cache 機制。
4. **Cherubin 22 online attack = Proteus defense from-first-packet**：對應 RegulaTor / FRONT 設計。
5. **Wails 24 baseline = Proteus reachability target**：>= 85%。
...
1. Khattak 6 軸中，哪兩個 對 Proteus 是「最 contentious」（即不同選擇 trade-off 巨大）？
2. Tschantz 4 mandate 中，哪一個是 Proteus 最難達到？為什麼？
3. Pulls 20 的 oracle attack 對 Proteus 設計 implications 哪三條？
...
Proteus 的 design 目標：optimize all three.
...
| Khattak axis | Proteus value | rationale |
|---|---|---|
