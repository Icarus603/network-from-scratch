# 課堂 5.10 — Probabilistic / Statistical Formal Methods：對抗 ML classifier 的 Proteus 證明
...
- 必讀論文：
  ...
  - **Wu, Ensafi, Crandall et al.**. *How the Great Firewall of China Detects and Blocks Fully Encrypted Traffic*. USENIX Security 2023 — **必讀，Proteus 對手定義** precis: [`notes/papers/wu-fep-detection.md`](../../notes/papers/wu-fep-detection.md)
...
**Proteus** ("indistinguishability from cover traffic against Wu-FEP-style classifier with $\varepsilon \leq 0.1$") 是我們協議的**差異化賣點**。如果這條無法形式化保證，我們的「SOTA」claim 立刻退化成「希望 work」級別。
...
### 1. 為何 trace / symbolic FM 在 Proteus 上失效

對 Proteus 寫一個 typical attack scenario:
...
對 Proteus verification: SMC 是現實選項。Exact PRISM 對 toy 5-packet handshake OK，real-world traffic 必 SMC。
...
5. **協議 design 對 Proteus 的承諾應該 phrasing 為何**? 寫一段 RFC-style "Security Considerations" 句子。
