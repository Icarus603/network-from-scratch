...
- **CAR**：MASQUE 客戶端對外看起來就是「正在用 HTTP/3 訪問 CDN」。如果 cover 是 Cloudflare，attacker 想 distinguish G6 與正常 H3 訪問需要長期 aggregation。
...
- 用真實 TLS 1.3 stack。Client sends ClientHello with embedded G6 key share；server with TLS cert 認證 + 內嵌 G6 keying material in extension。
...
- Server 配第三方 popular SNI（如 `www.microsoft.com`），但**只在 G6 handshake 通過時切回自家**。否則 forward 到真實 cover server。
- **CAR-2**：強。Active probe 命中時看到真實 cover server 的回應。
- **CAR-1**：handshake 與真實 TLS 1.3 borrowing 同（因為就是 TLS 1.3 with stolen SNI）。
- **限制**：cover server 必須是 G6 server **可達且 popular** 的目標。GFW 內部黑名單 SNI 不可借。
...
- 自己跑一個 HTTPS server，G6 流量混進 HTTP/2 stream。
...
- **判決**：**留用**——固定 size 為 G6 default cell，小流量接受 wastage。
...
- 真實採樣 cover protocol 的 packet size + IAT distribution，G6 流量也照樣分佈。
...
- 用 GAN-style training 對 G6 packet schedule 做 perturbation，optimize against a surrogate classifier。
...
|---|---|---|---|---|---|
| **G6-α** | TLS-over-TCP-443 | TLS 1.3 borrow + hybrid PQ | REALITY-style | hybrid shaping | 接近 VLESS+REALITY+PQ |
| **G6-β** | QUIC over UDP-443 | TLS 1.3 (QUIC 內建) + hybrid PQ | REALITY-style on QUIC | hybrid shaping | 接近 Hysteria2 + REALITY + PQ |
| **G6-γ** | MASQUE/H3 over UDP-443 | TLS 1.3 + hybrid PQ + H3 wrap | CONNECT-UDP to cover CDN | hybrid shaping | 最接近 RFC IETF SOTA |
| G6-δ | TLS-over-TCP + QUIC fallback | TLS 1.3 + PQ | REALITY | hybrid | dual-transport，複雜 |
...
本堂收斂出三條 candidate。每條都對應一個 reference SOTA 協議（VLESS+REALITY / Hysteria2 / MASQUE），便於對標 benchmark。11.4 將給 design rationale doc，最終 pick **G6-γ MASQUE-based**（理由見下堂）。
...
3. MASQUE 比 raw QUIC CAR 強在哪？答：cover server 不只擋 active probe，passive aggregator 看到 G6 與 H3 流量的 fingerprint 也同。
4. 為什麼 Tamaraw 已知最強卻不直接拿來用？答：bandwidth overhead 太高，違反 G6 PERF-1。
...
G6 必須對 **open-world + multi-flow + DL** 設計，不是 closed-world。
...
Let D_G6   = distribution of (size, IAT) pairs from G6 traffic after shaping.

Target: TVD(D_G6, D_cover) ≤ ε_TVD
...
```
Mimicry: ∀ packet p ∈ G6, ∃ packet p' ∈ cover such that p ≈ p' on observable features.
Polymorphism: ∀ classifier f, ∀ packet p ∈ G6, the family of p (over key randomness) covers the support of f.
```
...
G6 採 **polymorphism**（隨機化 cell size、IAT、再 cover-condition 一層分佈），不是純 mimicry。
...
- Transport 收窄到 {T2, T4, T5}（即 G6-α/β/γ）。
...
1. **G6-γ MASQUE cover server 的協商機制**：cover 必須是 H3-capable，但 G6 client 與 cover server 的「knowledge of G6 capability」如何在不洩漏 G6 identity 前提下協商？目前無公開 best practice。
...
> **本堂結語**：解空間從 ~400 點收到 4 個 candidate。11.4 將在 G6-α/β/γ 中選一個，並寫 design rationale doc。
