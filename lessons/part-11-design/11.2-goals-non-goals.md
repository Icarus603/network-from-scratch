...
口號不能 review、不能 verify、不能驅動設計。要把它變成 Proteus 的合約，必須**精確量化**：
...
仿 Clark 1988，把 Proteus 的目標分三層：

```mermaid
flowchart TD
  T1["Top goal:<br/>同時頂尖抗審查 + 頂尖速度<br/>在 Proteus 威脅模型下"]
...
Clark 對 internet design 列了 7 個 sub-goals，第一個 survivability 排在 cost、accountability、monitoring 之前。對 Proteus，**順序如下**（順序在 spec 寫進序言）：
...
4. **Performance** — 是 Proteus 的差異點，但**晚於**安全與抗審查；不為效能犧牲前三項。
...
```
Adv_CAR(f) = |Pr[f=1 | Proteus] - Pr[f=1 | cover]|
```
...
直觀讀法：**若 attacker 能達到 Adv_CAR ≈ 0.99，Proteus 死定了**。

反過來，Proteus 的可生存區是 Adv_CAR 落在 attacker「FPR 預算內找不到 high-TPR operating point」的區域。Wang & Hopper PoPETs 2019 的數據（websites fingerprinting）顯示：當 padding budget ≥ 50%（每 packet 平均 padding 為原 size 一半）時，SOTA DL classifier 對 closed-world identification 的 Adv 約掉到 0.2 以下。

**Proteus 的 CAR-1 budget**（記錄為 spec 約束）：
...
ε_CAR 的具體值依 attacker 而異。Proteus spec 必須列出**所有 reference attackers**：
...
| **A_TLS-in-TLS**：Xue et al. USENIX 2024 detector | NA | published model | ε ≤ 0.10（理想），Proteus 結構應禁此攻擊 |
...
```
Adv_CAR-2(A) = |Pr[A → Proteus server detects "Proteus"] - Pr[A → cover server detects "Proteus"]|
```
...
1. **Probe 觸發 fallback 前的時間差**：Proteus server 在認證失敗時 forward 給 cover server。若 forward 引入 > 5ms 額外 RTT，attacker 可用 timing distinguish。

   Proteus 目標：fallback 引入 RTT inflation **< 1ms (p99)**。

2. **TCP TIME_WAIT 後的 server-side behaviour**：cover server 處理 5 個 probe 後可能 rate-limit。Proteus server 必須 mimic 相同 rate-limit。

   Proteus 目標：rate-limit profile 與 cover server **bit-identical**（透過 forward 自動達成）。
...
| **DEP-3** | 訂閱格式為 Proteus URI scheme（`proteus://`）+ JSON config block，沿 Sing-Box 約定 |
...
| 非目標 | 原因 |
|---|---|
| **多用戶 / SaaS 級 panel** | Proteus 是 transport，不是 BMS。Panel 由上層工具（如 ccb）提供。 |
| **匿名性（unlinkability）** | low-latency proxy 無法對抗 GPA。需匿名走 Tor over Proteus。 |
| **NAT traversal hole-punching** | Proteus 假設 server 有 stable public IP。p2p 屬另一個項目。 |
...
| **跨協議遷移**（如同時支援 SS、Trojan refly） | Proteus 是新協議，不做 multi-protocol switchboard。 |
| **GUI / UX** | 由上層客戶端負責。 |
...
2. 列出 WireGuard goal hierarchy（從 Donenfeld NDSS 2017 §3）。對照 Clark 1988 風格——WireGuard 是不是把 simplicity 列在 performance 之上？是。為何 Proteus 不能這樣？
3. 對你自家的 VPS 跑 ja3 fingerprint：`curl https://ja3er.com/json -k`，把回傳 JSON 對比 https://tlsfingerprint.io/top 上 popular browser fingerprint。記下 distance。這個距離就是 Proteus 必須在 spec 裡為 cover handshake parameters 鎖死的東西。
...
Open problem：在 Proteus 設計空間裡 closed-form best response 不存在（cover 是 distribution-level constraint）。Game-theoretic anti-censorship 是 PETS-/NDSS-level open problem。
...
- **Goodput 目標用 BBR-style metric**——意味著 Proteus 的擁塞控制應 BBR-family（11.4 細說）。
...
> **本堂結語**：設計目標表是 Proteus 的「合約 v0.0」。每個編號都會在 spec、verification、benchmark 三處被引用。下一堂 11.3 開始探索：給定這份合約，設計空間裡有哪些 candidate？
