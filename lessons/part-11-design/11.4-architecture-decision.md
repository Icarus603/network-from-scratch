# 課堂 11.4 — 主架構決策：Proteus design rationale

## 學前知道
- 前置課：11.1（capability matrix）、11.2（goals）、11.3（candidate Proteus-α/β/γ）。
...
- **Proteus-α**：TLS-over-TCP + REALITY + PQ
- **Proteus-β**：raw QUIC + REALITY-on-QUIC + PQ
- **Proteus-γ**：MASQUE-CONNECT-UDP over H3 + PQ
...
| 目標 | Proteus-α (TLS+REALITY) | Proteus-β (QUIC+REALITY) | Proteus-γ (MASQUE/H3) |
|---|---|---|---|
...
- **威脅**：當 Proteus 內部又 tunnel 一個 TLS handshake（例 client 透過 Proteus 連 https://target.com），inner TLS handshake 的 packet-size + segmentation pattern 在 outer TLS-over-TCP 中可被識別。
...
**對 Proteus-α 的影響**：致命。Proteus-α 是 TLS-over-TCP，inner 仍是 TCP stream，任何 user 跑 inner HTTPS 都會被 TLS-in-TLS detector 命中。

**對 Proteus-β 的影響**：較弱。QUIC 用 datagram + stream，inner HTTPS（若是 H3）會落在 QUIC stream 內，但 QUIC packet 已 padded 到 MTU 且 connection ID 加密，inner record boundary 不直接外洩。但 inner H1/H2 仍走 stream，仍有風險。

**對 Proteus-γ 的影響**：最弱。MASQUE CONNECT-UDP 的 inner 是 raw UDP datagram (encapsulated in HTTP Datagrams per RFC 9297)，沒有 TLS record layer 的中間段，inner TLS handshake 完全消失在 datagram 序列中。

**判決**：Proteus-α **out**。

### 3. 剩下 Proteus-β vs Proteus-γ

| 維度 | Proteus-β | Proteus-γ |
|---|---|---|
...
| Active probe response | Proteus server 親自處理 | cover CDN 親自處理（真 H3） |
...
### 4. 決策：Proteus-γ MASQUE-based，但 with Proteus-β fallback transport
...
flowchart TD
  ...
  ProteusS[Proteus Server<br/>VPS + H3 + MASQUE handler]
  ORIG[Origin: target.com]

  CLI -->|HTTPS H3 to CDN<br/>CONNECT-UDP :443| CDN
  CDN -. unmodified H3 .-> CLI
  CLI -->|inside Capsule<br/>UDP to ProteusS| ProteusS
  ProteusS -->|TCP/UDP| ORIG
  ORIG -. data .-> ProteusS
  ProteusS -. data via UDP .-> CLI
  CLI -.-> CDN

  classDef ours fill:#fde,stroke:#c39
  class ProteusS,CLI ours
```

> ⚠️ 上圖的核心：client 看上去就是「在用 H3 訪問 CDN」，CONNECT-UDP request 包在 H3 frame 內，從 censor 角度看與真實 H3 fetch 無區分。inside Capsule 的 inner traffic 是 Proteus ↔ ProteusS 的 QUIC connection。

但這要求 cover CDN 願做 MASQUE proxy。實際上 Cloudflare/Fastly 等並未對 individual users 開放 MASQUE proxy 服務（截至 2025 H1）。所以**現實 deploy 需要 ProteusS 自己跑一個 H3 server 並做 MASQUE handler**，且該 server 對外的 SNI 借真實 popular SNI（REALITY-style）：

```mermaid
flowchart TD
  CLI[Proteus Client]
  ProteusS["Proteus Server<br/>(borrows SNI of popular site)<br/>port :443 UDP H3"]
  COVER[Real Cover Server<br/>e.g. www.microsoft.com]
  ORIG[Origin: target.com]

  CLI -- "H3 fetch with SNI=www.microsoft.com<br/>+ embedded Proteus auth in CONNECT-UDP" --> ProteusS
  ProteusS -- "if auth fails: forward H3" --> COVER
  COVER -- response --> ProteusS --> CLI
  ProteusS -- "if auth OK: act as MASQUE proxy" --> ORIG

  classDef ours fill:#fde,stroke:#c39
  class ProteusS,CLI ours
```

這是 Proteus-γ 的最終架構：**MASQUE+REALITY combination**。

### 5. Fallback transport：Proteus-β

不是所有環境都允許 UDP 443。某些境內 ISP 對 UDP 流量做 QoS 降速，或 NAT 對 UDP 連線 binding 短。Proteus spec MUST 提供 **fallback transport**：

```
Primary: Proteus-γ MASQUE/H3 over UDP-443
Fallback: Proteus-β QUIC over UDP-443 (no H3 wrap, REALITY-on-QUIC)
Last resort: Proteus-α TLS-over-TCP-443 (with TLS-in-TLS mitigation: forced inner padding)
```
...
Proteus-β/γ 在 QUIC 之上。RFC 9002 default 是 NewReno；BBRv2 / BBRv3 是 SOTA（Cardwell 2017+IETF drafts）。Hysteria2 用一個自家「Brutal CC」（rate-paced, no AIMD），在 lossy link 上效能 > BBR 但 fairness 差。

Proteus 採 **BBRv3** 為 default，理由：
...
| Hash | BLAKE3 (for KDF inside Proteus); SHA-256 (for TLS 1.3 compatibility) | BLAKE3 速度快 5-10x；TLS 1.3 ciphersuite 用 SHA-256/384 |
...
2. **Handshake auth**：client 在 ClientHello extension（type = 0xfe0d, "reality" reserved private use）內嵌一個 X25519 public key + AEAD-encrypted authenticator。Server 用其長期 private key 試解 → 成功則認 Proteus，失敗則 forward。
...
- **TLS extension order**：handshake 由 cover server 處理；Proteus server 不 modify。
...
本堂之後所有設計動作（11.5–11.8 spec、11.9–11.11 formal verification）都基於 Proteus-γ + β/α fallback 架構。Spec 撰寫從 11.5 開始 byte-by-byte 定義 wire format。
...
3. 用 Cloudflare quiche （Rust）或 quic-go (Go) 寫一個 minimum H3 server，加一個 MASQUE CONNECT-UDP handler（RFC 9298 §4）。這就是 Proteus server 的 H3 layer minimum spike。

---

## 自我檢查

1. 為什麼 Proteus-α 是死的？提供至少兩個獨立論據（TLS-in-TLS + 還有什麼？）。
2. 為什麼選 MASQUE/H3 而不是 raw QUIC？答：cover server 是真 H3，passive aggregator 看不出 H3 與 Proteus 差異。
...
- **C7' Cover-CDN-side metadata leak**：如果 Proteus server 自己跑 H3，cover CDN 看不到。如果用 third-party MASQUE proxy（unrealistic for individual user），CDN log 可能能看到 Proteus client IP → cover URL pair。
...
這是 Proteus hybrid KE 採用此構造的 formal foundation。

**Active-probe indistinguishability formal target**：

```
Proteus server's network behaviour 對任何 probe sequence q,
  Output(Proteus_server, q) ≈_c Output(cover_server, q)
...
- **Architecture**: Proteus-γ MASQUE/H3 over UDP-443 + β fallback + α last-resort
...
4. **BBRv3 vs Hysteria Brutal fairness 量化**：在 Proteus deployment 規模下（thousands of nodes）兩者對 cross-traffic 公平性差距多大？需大規模 testbed。
5. **Fallback race 帶來的 fingerprint**：HappyEyeballs-style 三 transport race 是否引入新 fingerprint（時序、source port pattern）？

---

> **本堂結語**：Proteus 架構 lock。下一堂 11.5 開始 byte-by-byte 定義 wire format。
