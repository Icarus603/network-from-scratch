...
| Cap | Threat | Proteus mechanism | Spec § | Formal coverage | Residual / action |
|---|---|---|---|---|---|
...
|---|---|---|---|
| **SEC-1** | IND-CCA2 confidentiality | ✅ | ProVerif Q1 (ProteusHandshake.pv) |
...
- 部署建議：Proteus server 與 cover 應在同 datacenter（或 close peering）.
...
評：Yes — 普通 Chrome ClientHello 約 500-700B, Proteus + 1088B PQ ext makes CH ~2KB.

**Mitigation**：

- Chrome 從 Chrome 124（2024 Q2）開始預設啟 hybrid PQ (X25519MLKEM768), Cloudflare 報告 2025 部署率 > 25%.
- Proteus spec 強制 client follow chrome-N profile, 自動繼承 hybrid PQ ClientHello shape.
...
#### Attack R-3: 「H3 frame fingerprint：Proteus's CONNECT-UDP capsule 與 typical browser H3 fetch shape 不同」

評：Yes — browser H3 GET 通常 followed by 大量 RESPONSE chunks; Proteus CONNECT-UDP 是 bidirectional steady stream.

**Mitigation**：

- Proteus inner padding (1280B cell) makes outer H3 DATA frames uniformly large.
...
評：QUIC connection ID rotation under migration is per-spec. Proteus inherits.
...
- 11.13 spec 加 note: client SHOULD bind Proteus socket to specific interface + use static route to prevent routing table override.
...
#### Attack R-10: 「BBRv3 與 cross-traffic fairness 在 Proteus deployment 規模下未量測」

評：Yes — 大規模 Proteus 部署若使用 aggressive CC variant 可能影響其他 traffic.
...
| Property | Proteus | VLESS+REALITY | Hysteria2 | TUIC v5 | WireGuard |
|---|---|---|---|---|---|
...
> Proteus 在「formal verification 完整性 + hybrid PQ + 反 TLS-in-TLS architectural + active-probe-resistance」四維 SOTA。剩下 PERF/CAR 量化等 Part 12 evaluation 出爐.
...
5. 為什麼 Proteus 在 SOTA 比較表上 PCS=weak 而其他 = none？答：Proteus 是現有 anti-censorship protocols 中唯一 explicit Tamarin-verified PCS, 雖只 weak.
...
對 Proteus v0.0：所有 in-scope c 都 mapped 到 d 與 E（formal or empirical），故 conform. Residual = explicit OUT-OF-SCOPE entries.
