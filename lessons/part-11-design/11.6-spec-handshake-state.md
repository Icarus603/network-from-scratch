...
> Spec v0.0 全部寫進 `assets/spec/proteus-v0.0-handshake.md`。
...
sequenceDiagram
  ...
  Note over C: ClientHello with Proteus ext 0xfe0d (1186B payload)<br/>Hybrid X25519 + ML-KEM-768 ciphertext<br/>HMAC-validated auth tag

  C->>S: QUIC Initial: TLS 1.3 CH (1190B Proteus ext + TLS std exts)
  ...
  alt auth_tag valid AND timestamp fresh
    ...
    S->>C: QUIC Initial+Handshake: TLS 1.3 SH + EncryptedExtensions(Proteus server confirm)
...
Proteus ext 0xfe0d 在 CH 內，故 transcript 自動覆蓋 PQ ciphertext.
...
1. Sender 計算 `new_secret = HKDF-Expand-Label(old_secret, "proteus ratchet", "", 32)`.
...
Proteus 透過 `client_id = AEAD(server_PK_kdf, ad=nonce, pt="user_id")` 達成：

- 對 attacker，每次 client_id ciphertext 不同（nonce 變）→ 看似隨機。
- 對 server，可解 client_id → 知用戶。

> 與 Noise IK 對比：Noise IK 把 initiator static public key 在第一條 message 內以 chaining key 加密發送，attacker 不能識別 initiator。Proteus 借同一思路。

**Responder identity hiding** = 對 on-path attacker，看不出 server 是 Proteus 還是 cover？由 REALITY-style + cover forward 達成（11.7 詳述）。
...
def g6_client_handshake(server_static_pk, server_cover_url, user_id):
    ...
    client_id_ct = aead_encrypt(
        key=hkdf(server_static_pk, "proteus cid key"),
        nonce=urandom(12),
        ad="g6_cid",
        pt=user_id
    ...
    if sh.is_real_server_hello():
        ...
        # fallback: Proteus auth failed, but TLS is real H3 from cover
        # client should retry with different transport or give up
        raise ProteusAuthFallback()
```
...
- 與 cover forwarding 配合：replay 失敗 → forward，不洩漏 Proteus 是否存在。
...
3. 把 §3 key schedule 與 WireGuard noise IK 的 chaining key schedule 對比。注意 WireGuard 是純對稱 chaining，Proteus 沿 TLS 1.3 是 multi-secret derived。
...
Proteus 由 Finished message + transcript hash 達到該屬性。
...
對 Proteus：handshake 的 server 對 client 的認證來自 server's TLS cert + handshake signature；attacker 持 client sk 不能偽造 server cert.
...
| RFC 8446 §7.1 | TLS 1.3 key schedule（Proteus 借完） |
...
3. **Transcript binding 對 cover-borrowed CH 的形式驗證**：當 server fallbacks to cover, 此時 Proteus authentication is aborted, the transcript binding 是否仍 sound？需 ProVerif model 涵蓋 fallback path（11.10 to handle）。
4. **Pre-shared key vs 全新 hybrid KEM 對 PERF-2 影響量化**：對 N=10⁶ session/day deployment, 預算約多少 CPU 用於 ML-KEM vs HKDF chains？
5. **PQ signature 引入時程**：CA 與 cert chain 何時切 ML-DSA？Proteus 是否需要支援 chain-mixed cert（intermediate ECDSA + leaf ML-DSA）？
