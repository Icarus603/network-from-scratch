...
這給出 G6 設計的「實際對手」座標：on-path active + long-term + sub-national GPA。
...
### 4. 對手能力 Matrix（G6 對手畫像）

下表是 G6 設計時假設對手具備的全部能力。每一條都要對應一個防禦點（11.12 會逐項驗證）。
...
Goal SEC-1 (Confidentiality):
  ∀ PPT adversary A controlling C1+C2,
    Adv^{IND-CCA2}_{G6.AEAD}(A) ≤ ε_AEAD(λ) + ε_KEM(λ) + negl(λ)
```
...
KCI 是 anti-censorship 特別重要的屬性：服務端 long-term key 洩後，attacker 可冒充 client 連 server，但**不該能反過來冒充 server 對 client 通信**。Noise IK / WireGuard 已驗證滿足此性質（Donenfeld-Milner 2018；Lipp-Blanchet-Bhargavan 2019）。G6 採同類設計。
...
Goal CAR-1 (Censorship resistance / indistinguishability):
  ∀ classifier f trained on (G6 traffic, cover-protocol traffic):
    | Pr[f(G6) = "G6"] - Pr[f(cover) = "G6"] | ≤ ε_CAR
...
Goal CAR-2 (Active probing resistance):
  ∀ probe q from adversary not holding handshake secret,
    G6 server's response to q is indistinguishable from
...
|---|---|---|
| Global passive adversary 下的 unlinkability | 跨大陸的 packet-level correlation 已被多次 demo（Wang-Goldberg 2013 等）；任何低延遲 proxy 都做不到 | 用 Tor over G6 if 需 unlinkability |
...
| Spam / abuse on cover server | G6 是 transport，不是反濫用層 | 上游服務自行 rate-limit |
...
對 G6 的主要攻擊路徑（goal → 攻擊手段 → 對應前置條件 → 防禦點）：

```mermaid
flowchart TD
  ROOT[Bypass G6 protection]
  ROOT --> A1[Identify traffic as G6]
...
4. 如果我們把 C7 (sub-national GPA) 從 in-scope 移到 out-of-scope，G6 的設計能簡化多少？簡化的代價是什麼？
...
> **本堂結語**：威脅模型不是文書工作，是 anti-censorship 工程裡密度最高的設計動作。寫完一份能挑得起來的威脅模型，G6 的所有後續設計就是「對著這張清單一條一條兌現」。下一堂 11.2 我們把 CAR-1 從 ε_CAR 變成可量化的 budget。
