...
DNS 看似「**域名翻 IP**」一件事，**對 Proteus 是 5 個 first-order 問題**：

1. **Proteus 客戶端怎麼知道 Proteus server 的 IP？** 必須走 DNS——但 GFW DNS 污染嚴重。Bootstrap problem 沒解，Proteus client 連不上 server
2. **GFW DNS 對 Proteus 的攻擊面**：Hoang 2021 USENIX Sec 揭露**至少 311K 域名被 GFW DNS 污染**（含 ESNI / DoH / GitHub 等翻牆工具相關）；GFW 注 forged IP 模式具體可量化
3. **加密 DNS（DoT/DoH/DoQ）+ ECH (Encrypted Client Hello) 是 anti-censorship 工具，也是 GFW 攻擊目標**：GFW 對 DoH 流量 selective drop、對 ECH 部分動作——這直接影響 Proteus 部署策略
4. **ECS（EDNS Client Subnet）對 CDN-based Proteus deployment**：Proteus 若部署在 CloudFront / Cloudflare，ECS 決定 client 拿到哪個 anycast IP——**對 latency + 對 GFW 識別均有影響**
5. **SVCB / HTTPS RR + DDR (Discovery of Designated Resolvers) 是 IETF 2023+ 的新方向**：允許 service binding（同一 hostname 多 transport / port hint）+ encrypted DNS discovery——對 Proteus v2 設計直接相關
...
**HTTPS RR (type 65) 是 2023 後新標準**——含 ALPN hint、port、IP hints、**ECHConfig**——讓 client 在 DNS 階段就知道 transport details + 提前準備 ECH。對 Proteus 直接相關。
...
- **Proteus server IP TTL 短**（如 60 sec）→ 可快速切換 IP；但增加 DNS query 量、洩漏更多 query metadata
- **TTL 長**（如 86400）→ Proteus client 不容易切換到新 server
...
**對 Proteus**：**忽略 DNSSEC**——不依賴；改 trust path 走加密 DNS + cert pinning。
...
#### 8.5 對 Proteus 的具體 implications

- **Proteus server domain 必須避開 GFW filter regex**——不能用「**vpn.example.com**」這種顯眼名字
- **Proteus client bootstrap 不能依賴 plain DNS**——必須用 DoH（首選）、DoQ、或預配 IP
- **forged IP detection**：Proteus client 偵測 DNS response 是否 forged（如 response 含已知 forged IP set → 拒）
...
#### 9.2 ECS 對 Proteus 影響

- **若 Proteus 走 CDN-fronting**（Cloudflare Worker / CloudFront）：ECS 決定 edge IP——client geo 變 → IP 變 → GFW 黑名單 efficacy 變
...
- 若 Proteus 用 CDN-fronting，ECS 是 latency 友善但 privacy 災難
...
**Proteus 應該 publish HTTPS RR**：
- 含 Proteus ALPN（自定義 ALPN string）
- ECH config（加密 SNI）
- IP hints（client 直接知道，省 A/AAAA round trip）

**但 GFW 對 ECH 已開始 selective drop**——2024+ measurement 顯示中國對含 ECH 的 ClientHello packet drop 部分。**Proteus 設計必須有 ECH-less fallback**。
...
**對 Proteus**：client 啟動時做 DDR——若 local network 提供 encrypted DNS → 用；否則 fallback 到 Proteus 自帶 DoH endpoint。
...
| **11.4 主架構** | Proteus client bootstrap：DoH > DoQ > IP 預配；不用 plain DNS |
| **11.5 packet format** | 若 Proteus 走 CDN fronting，HTTPS RR 內標 ALPN |
...
| **12.7 server** | Proteus server domain 設計避開 GFW regex；HTTPS RR publish |
| **12.18 真實環境測試** | 必測 GFW DNS poisoning 各種 path；ECH 部分國家 drop |

### Proteus DNS strategy 提案
...
3. DNSSEC 為何 deployment ~5-15% 而不普及？對 Proteus 是否應整合 DNSSEC？
4. DoT / DoH / DoQ 三者主要差別？對抗 GFW 各自優劣？
5. ECS 對 CDN-fronted Proteus deployment 是 advantage 還是 trap？Proteus 該用 ECS 還是 ODoH？
6. ECH + HTTPS RR (RFC 9460) 怎麼整合？Proteus publish 什麼 RR 給 client？
7. Hoang 2021 GFWatch 顯示 GFW DNS injection 三個 injector + 11 forged IP groups + 41K overblocking——這些對 Proteus server domain 命名與部署有什麼具體建議？
...
⇒ Proteus client 必須**有方法區分** forged response vs real response——這正是 GFWatch 做的事情之一（觀察 response time、TTL、DF bit、IP set）。
...
| **Proteus server domain** | 避開 GFW regex；避免「vpn」「proxy」字眼 |
...
- **DNS-based covert channel for circumvention**：把 Proteus 流量塞進 DNS query (DNSCrypt-like)——歷史上 dnscat、iodine——2026 仍可行？open
- **HTTPS RR 對流量分析的新 dimension**：HTTPS RR 含 IP hints 與 ALPN——是否成為 DNS 階段就的 fingerprint surface？open
- **GFWeb（Hoang 2024）揭露的 web censorship 與 DNS censorship 互動**：how to design Proteus to avoid both layers？

---

下一堂：**1.15 BGP**——AS / Tier / IXP；BGP 路由洩漏與劫持（YouTube/Pakistan 2008、AS7007 1997、MyEtherWallet 2018）；「中轉節點」「BGP 加速」這些機場行話的真實意義；對應 Proteus 部署的 routing 策略。
