...
4. **Happy Eyeballs v2** 是 dual-stack client 必須懂——client-side UX 與 fallback 邏輯——**Proteus client 設計必須 inherit**
5. **IPv6 在 hyperscaler 與 mobile carrier 普及極快**：2024 Google statistics ~45% 全球 user 走 IPv6；T-Mobile US / Reliance Jio 印度等是 IPv6-only carrier；**Proteus dual-stack mandatory**
6. **IPv6 multicast / anycast / scope 設計**比 IPv4 完整——但**翻牆場景無直接利用價值**——只是部署現實必須懂

教科書講 IPv6 的問題：花太多篇幅講 address 表示法、不展開 extension header 的 dropping 災難、不講 Happy Eyeballs / RFC 6724 default selection 的 deployment 現實、不評 GFW 對 IPv6 態度。本堂從 deployment + 對 Proteus 影響切入。
...
**對 Proteus**：可考慮 set flow label，但 **不要太穩定**（cross-session linkability fingerprint）也**不要每 packet 變**（破壞 ECMP）——典型 per-QUIC-connection unique random。
...
⇒ **Proteus IPv6 packet 不應該使用 extension header**——基本不可用。**這個現實對「IPv6 比 IPv4 簡單」這個 narrative 是反例**。
...
#### 4.4 對 Proteus client 的具體 implication

Proteus client 連 Proteus server：
...
**Proteus 應該 design 為「Happy Eyeballs across Proteus servers」**：給 client multiple Proteus server endpoint，race connections——首個成功的取得連線。對 GFW 的單一封鎖點 robust。
...
**對 Proteus**：IPv6 是 marginal advantage——不是 silver bullet。**dual-stack + Happy Eyeballs 是正解**——而非 IPv6-only。
...
**對 Proteus**：client 在 IPv6-only network 上：
- DNS 拿 AAAA → 用 IPv6 直連 Proteus server（**理想**）
- DNS 拿 A 但無 AAAA → NAT64 翻譯 → 仍可用，但多一層 NAT 延遲
- ⇒ **Proteus server dual-stack 是 mandatory**
...
4. Happy Eyeballs v2 比 v1 進步在哪？對 Proteus client 設計 5 個具體影響各是什麼？
5. RFC 8981 + RFC 7217 並用如何同時滿足 stable inbound reachability 與 outbound privacy？
6. GFW 對 IPv6 與 IPv4 的對抗能力差異是什麼？Proteus 為何不走 IPv6-only？
7. 2024 IPv6 部署現實（India 75%、global 45%）對 Proteus baseline 的 deployment 計畫有什麼具體影響？
...
| **client Happy Eyeballs** | mandatory；可選 across-Proteus-server happy eyeball |
...
下一堂：**1.14 DNS 完整解剖**——報文 byte by byte、recursive/iterative、Kaminsky cache poisoning、DNSSEC 為何失敗、DoH/DoT/DoQ 取而代之、ECS 對 CDN 與翻牆雙刃；對應 Proteus 名稱解析設計與抗 DNS poisoning。
