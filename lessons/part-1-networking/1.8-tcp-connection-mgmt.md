...
Proteus baseline 走 UDP/QUIC，**理論上**不用 TCP。但你**必須**精通 TCP 連線管理，因為：

1. **GFW 的主要工具是 TCP RST injection**：絕大多數翻牆協議在中國被斷的根因是 RST。**理解 TCP state machine 才能設計抗 RST 機制**（QUIC 對此天然免疫，但 Proteus fallback transport 走 TCP 時必須處理）
...
5. **SYN flood / SYN cookies / challenge ACK 三個攻防回合** 揭示「**任何 stateful 中間設備在公網都是潛在 DoS target**」——對 Proteus server-side 設計極關鍵
6. **Cao 2016 CVE-2016-5696 是 side channel 經典案例**：增加安全性的 mechanism（RFC 5961 challenge ACK rate limit）反而**成為更強的 side channel**。**這個教訓對 Proteus 任何 rate-limit 設計都適用**
...
**Race condition / data loss 風險**：server crash 那瞬間 unACKed data 永遠丟。**對 Proteus 設計**：data plane 必須能 detect server restart（透過 control plane heartbeat），不能依賴純 transport-level 連通性。
...
TFO 失敗的 4 個教訓對 QUIC 0-RTT / Proteus 0-RTT 全適用：
...
- **改走 UDP/QUIC**（RFC 9000 內無 RST 概念；QUIC stateless reset 走密碼學）—— **Proteus baseline 走此路**
...
**Proteus baseline 走 QUIC = 天然免疫 RST injection**——這是設計選擇的核心理由之一。
...
**「shared state for security」 = 可能的 side channel**。Proteus 設計：
...
對 Proteus：**IPID 對 QUIC 不直接適用**（QUIC over UDP，UDP header 也有 fragmentation ID 但 entropy 略不同），但**思想直接適用**：避免 cross-flow state sharing。
...
**對 Proteus**：若走 TLS+TCP，SNI 必須加密（**ESNI / ECH, Encrypted Client Hello, draft-ietf-tls-esni-current**）——或走完全不暴露 SNI 的 transport（**REALITY 借用真實 server 的 ClientHello**）。
...
3. TFO 為什麼失敗？學到的 4 個教訓對 QUIC 0-RTT / Proteus 0-RTT 各對應什麼設計決策？
4. GFW RST injection 在 RFC 9293 TCP state machine 中為什麼可行？QUIC 為什麼免疫？這個免疫性是 Proteus baseline 走 QUIC 的核心理由之一——還有哪些理由？
...
6. TIME_WAIT 對高並發 server 為什麼是 port exhaustion 災難？`tcp_tw_recycle` 為何在 Linux 4.12 被 deprecate？對 Proteus server 部署有什麼具體配置建議？
...
下一堂：**1.9 TCP 完整解剖（二）：可靠傳輸**——sequence number、ACK、SACK/DSACK、RTO 計算（Karn's algorithm）、快速重傳、F-RTO、ER、Linux `tcp_input.c` 精讀；對應 Proteus reliable delivery 設計。
