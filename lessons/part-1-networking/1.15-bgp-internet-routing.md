...
1. **G6 server 部署的 AS / Transit 選擇直接影響 Throughput 與審查抗性**：CN2 GIA / CN2 GT / NTT / Cogent / HE.net 不同 transit 對中國 path 表現差 10 倍以上；機場圈「**BGP 加速**」、「**CN2 GIA 中轉**」這些行話的真實意義必須能精確解讀
2. **GFW 有 BGP 級封鎖能力但成本高**：理論上 GFW 可以對任意 IP prefix 注 BGP null route 全國封；實務上**幾乎不用**因為 selectivity 低（封一個 prefix = 封多用戶）；理解此邊界才能評估 G6 「**IP 被燒**」的恢復策略
3. **真實 BGP 事故對 G6 有威脅**：2010 China Telecom 18-min hijack、2008 Pakistan YouTube、2018 MyEtherWallet 都展示 BGP 攻擊面是 production reality
4. **RPKI / BGPsec / ROA** 是 BGP 安全的部分解——理解其部署率與 limits 影響 G6 deployment 信任假設
5. **Griffin-Wilfong 1999 證明 BGP 在 expressive policy 下可能 non-convergence**——這對 G6 control plane 設計是反面教材（**不要模仿 BGP**）
6. **Anycast 與 BGP 的耦合**：G6 anycast deployment 直接受 BGP best-path selection 影響——一旦 IP 被 BGP 級封，全球 anycast 都受影響

教科書講 BGP 的問題：技術細節（message types, FSM）講太多——不展開 routing economic / political layer、不講「中轉節點」「BGP 加速」實際含義、不評真實 hijack 事件對 internet 治理的衝擊。本堂從 G6 部署視角切入。
...
對 G6：**部署在 IXP-rich 位置的 VPS（如 Frankfurt、Tokyo）—— peering 多 → 出口到 transit 跳數少 → latency 低**。中國方向：**Tokyo / Singapore 距離近 + IXP 大 → 優選**。
...
**對 G6**：選 transit 時看其 supported communities——好 transit 給 G6 server admin 細粒度控制。
...
**對 G6**：RPKI ROA 對 G6 server prefix 是 nice-to-have（防別人 hijack 你 prefix），但 BGPsec 不指望。
...
**對 G6 implication**：BGP 級操控是 state-level capability——G6 設計**必須假設**對手有此能力，但無 protocol-layer 應對策略——靠基礎設施（RPKI、多 AS 部署、out-of-band signaling）。

#### 7.4 MyEtherWallet hijack (2018-04-24)

俄羅斯 AS 短暫 hijack Amazon Route53 用 prefix → 用戶被引去釣魚站 → ~$152K ETH 被偷。
**Lesson**：BGP hijack 對「**靠 DNS 找 server**」的服務致命——對 G6 client bootstrap 同樣警示。
...
1. AS / Tier 1/2/3 / IXP 三層結構各自意義？G6 部署選哪些 IXP-rich 城市最有 latency 優勢？
...
5. AS 7007 / Pakistan YouTube / China Telecom 2010 三個事件各自 lesson 是什麼？G6 部署可學到什麼？
6. 「BGP 加速 / CN2 GIA 中轉」真實技術意義是什麼？對 G6 server 選 transit 的具體 advice？
7. anycast 與 BGP 的耦合對 G6 anycast 部署的 advantage 與 risk 各是什麼？
8. Griffin-Wilfong 1999 BGP non-convergence 證明對 G6 control plane 設計有什麼反面教訓？
...
**對 G6 control plane 反面教訓**：**不要設計可表達**那種「**讓 application 自由設定 policy**」的 protocol——必須限制 expressive power 確保 termination。Raft / Paxos 等 proven algorithm 更佳。
...
- ✅ **Sermpezis et al. 2018 CCR BGP hijack survey**
...
| **不模仿 BGP 設計** | G6 control plane 用 Raft/Paxos；不開放 expressive policy |
...
下一堂：**1.16 CDN 與 Anycast**——Cloudflare 的 IP 段、CF Workers 為什麼能當免費中轉、Akamai 的選路邏輯、AWS Global Accelerator；對應 G6 「**CDN-fronting**」與 anycast deployment 的具體 trade-off。
