...
CDN + Anycast 對 Proteus 是 **two-edged sword**：

1. **CDN-fronting 是 Proteus 抗審查的潛在強武器**：把 Proteus 流量 hide 在 CDN 大流量中。GFW 不能封整個 Cloudflare（會殺很多合法網站）—— 這就是 Fifield 2015 domain fronting 的核心思路
2. **Anycast 對 Proteus 是 deployment trade-off**：好處：同一 IP 多地宣告 → 用戶自動 routing 到最近 instance；壞處：一旦 IP 被封 → **全球 instance 全封**——無 IP rotation 餘地
3. **Cloudflare Workers 是「免費全球邊緣計算」**：Proteus 可用 Worker 當 control plane / endpoint discovery / 甚至 data plane 部分 hop——但 ToS 風險真實存在
4. **Apple iCloud Private Relay + Cloudflare WARP 是 production-scale 的 MASQUE-based VPN**：對 GFW 是新類型挑戰；Proteus 應 study 此 architecture
...
教科書講 CDN 的問題：用 user perspective 講 caching benefits——不展開 anycast 機制細節、不講 CDN-fronting 對抗審查實踐、不評各家 CDN deployment trade-off。本堂從 Proteus 部署視角切入。
...
#### 2.3 對 Proteus anycast 部署的 implication
...
**結論**：**baseline Proteus 走 unicast multi-IP**；anycast 留給「**Cloudflare Worker-fronted 部分流量**」這種特殊架構。
...
- **Use case for Proteus**：
  - Endpoint discovery (Worker as API gateway)
  - Lightweight relay (TCP/WebSocket through Workers)
  - DNS-over-Worker (custom DoH)

**ToS 風險**：Cloudflare 對 VPN / proxy use 在 free tier 模糊——high traffic 可能被 throttle 或 ban。**production Proteus 不應主要依賴**。
...
**對 Proteus**：可用 Cloudflare Tunnel 把 Proteus server 「**藏**」在 Cloudflare 後——origin 無公網 IP，外部只看到 Cloudflare IP。**強力對抗 IP-based block**——但對 GFW selective drop 仍可能 vulnerable（Cloudflare 流量 GFW 可 selectivity throttle）。
...
**對 Proteus 是直接 competitor 也是 architectural reference**——WARP 顯示「**big CDN-anycast-based VPN at scale**」是可行 model。
...
- 對 Proteus 不主流——cost 過高、無 free tier、anti-circumvention 嚴格
...
- **對 Proteus**：可用，但 AWS 對 circumvention use cases**嚴格**——曾發生 (e.g. Signal CloudFront fronting 被 AWS 自己關)
...
- 對 Proteus：可選擇——比 Cloudflare 更技術中立但少 free tier

#### 4.4 Bunny CDN / KeyCDN / DigitalOcean Spaces CDN

小型，價格便宜，**對 circumvention use case 普遍不 enforce**。**Proteus 中小規模 deployment 可行 alternative**。
...
**現實**：domain fronting 沒死，但 **viable 的前端越來越少 + 每家 CDN 動態變化**。**Proteus 不能把 fronting 當 baseline**——只能作 fallback。
...
### 6. CDN-fronting 設計模式（Proteus 角度）

#### 6.1 Mode A: CDN as data plane

```
Proteus client → CDN edge (TLS+SNI 看似 allowed.com) → CDN → Proteus origin server
```

- **Pros**：全程匿名於 CDN；anti-block 強
- **Cons**：CDN 流量 cost；ToS 風險；CDN 看到 plain Proteus 流量（信任 CDN）
- **適合**：小規模、key user

#### 6.2 Mode B: CDN as endpoint discovery

```
Proteus client → CDN Worker (取得 endpoint list) → direct Proteus origin
```
...
```
Proteus client → CDN anycast POP via QUIC + MASQUE CONNECT-UDP → Proteus origin
```

- **Pros**：流量加密 + tunneling；CDN 是 Proteus 信任邊界
- **Cons**：需 CDN 支援 MASQUE relay（Cloudflare 已支援 WARP，他人 access 限制）
- **適合**：long-term direction，但需 production maturity

### 7. 對 Proteus anycast deployment 評估矩陣

| 屬性 | Proteus 不 anycast (multi-IP unicast) | Proteus anycast |
|---|---|---|
...
**Proteus baseline 推薦**：multi-IP unicast；anycast 留給 control plane / endpoint discovery / DNS-like 小流量用例。

### 8. iCloud Private Relay：production-scale 案例研究

Apple 2021 推出，**對 Proteus 設計極具參考**：
...
- **Two-hop with trust split** 是 strong privacy primitive——Proteus v2 可考慮
- **MASQUE over QUIC** 是 protocol baseline
- **Big CDN partnership** 給 anonymity set（千萬 user 流量混合）
- **GFW 對 iCloud Private Relay**：**完全封 by GFW**——「太強」對 censor 是 red flag——Proteus baseline 不能 mimic 此 architecture 對 GFW
...
**對 Proteus**：
- 若 Proteus 用 CDN-fronting（依賴 CDN 邊緣選擇）+ user 用 1.1.1.1（不傳 ECS）→ user 拿到 random / sub-optimal edge → latency 差
- 若 user 用 8.8.8.8（傳 ECS）→ 較好 edge but 隱私洩漏 client geo

⇒ Proteus client 設計：**自己做 DoH to specific resolver**，控 ECS policy。
...
4. iCloud Private Relay two-hop trust split 設計對 Proteus v2 是 architecture reference 還是反面教材？
5. Proteus baseline 為何不走 anycast？什麼條件下 control plane / endpoint discovery 用 anycast 合理？
6. Cloudflare Worker / Fastly Compute@Edge 對 Proteus 是 opportunity 還是 ToS risk？production 部署判斷 framework？
7. ECS 與 CDN edge selection 的互動——Proteus client 該如何處理 ECS 隱私 vs latency 取捨？
...
- ✅ **Fifield 2015 PoPETs Domain Fronting** ⭐ — 札記 [notes/papers/fifield-domain-fronting-2015.md](../../notes/papers/fifield-domain-fronting-2015.md)
...
| **anycast for main Proteus data plane** | no——封掉風險太集中 |
| **ECH integration** | 取代 fronting in long run |
| **trust split design** | study iCloud Private Relay—v2 evaluate two-hop |
| **anti-fingerprinting at CDN edge** | CDN sees plaintext Proteus protocol——consider end-to-end encryption past CDN |
...
- **匿名 CDN deployment**：Proteus self-host CDN-like layer 跨多 small VPS 是否可行？open
...
下一堂：**1.17 把所有東西串起來：「點開 google.com 的 50ms」**——real pcap frame-by-frame：從 ARP/DHCP 到 DNS（DoH）到 TCP+TLS+ECH 到 HTTP/3 到 render；對應 Proteus 設計時必須考慮的「**真實世界連線時序**」。
