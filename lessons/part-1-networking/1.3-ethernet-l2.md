...
1. **MTU 預算被 DC overlay 吃掉**：你的 G6 server 跑在 VPS 上。VPS 跑在 hyperscaler / Hetzner / Vultr / DigitalOcean 的 DC fabric 內。**這個 fabric 幾乎一定有 VXLAN 或 Geneve overlay**，每個 packet 出去前先被加 50~64 byte 的 outer encap。**你以為 1500 MTU，實際 application payload 預算 ≤ 1450**。對 QUIC initial packet 設計（必須 ≥ 1200 byte but 不能 fragment）這是**硬限制**
2. **GFW 不在 L2，但量化 GFW 部署位置需要 L2 知識**：理解 ISP backbone 是 L3 routing 而非 L2 switching，才能定位 GFW 在哪些 hop 介入
3. **理解 TUN vs TAP 的設計選擇**：WireGuard / OpenVPN / Tinc 在 L2 vs L3 的取捨；G6 為何走 L4 proxy 而非 L2 VPN
4. **DC fabric 已是「control plane（BGP EVPN）vs data plane（VXLAN）分離」的成熟工程**，這個 architecture 我們在 Part 11 設計 G6 的 control plane 時會直接參考
...
**4094 對我們協議的關聯**：你的 G6 server 部署在 hyperscaler 上時，**hyperscaler 內部不可能用 802.1Q VLAN 隔離客戶**（4094 連一個 region 客戶數都不夠）。**所以幾乎可以斷定他們用 VXLAN/Geneve**。
...
| 影響面 | 細節 |
|---|---|
| **MTU 預算** | underlay MTU 1500 → overlay VM MTU = 1500 - 50 = 1450（或 1446 含 inner VLAN）。**G6 QUIC packet 在 DC 內部最大 application payload ≤ 1450** |
...
| **流量分析 visibility** | 雲端 provider 看 underlay 流量（不是你 application）；service mesh sidecar 看 application 流量；G6 在這兩層之間 |
...
**Cilium / Tetragon eBPF service mesh 預設用 Geneve**。Kubernetes CNI 戰場：Calico 仍多用 VXLAN，Cilium 推 Geneve。對 G6 部署：**你 K8s sidecar inject 進 cluster 後，packet 會被加 Geneve header**（除非你跑 native routing 模式繞過 overlay）。
...
**對我們協議**：
- G6 是 L4/L5 proxy（接 TCP/UDP，不接 IP packet）——**根本不需要 TUN/TAP**
- 即便 G6 想做 transparent proxy mode，TUN 就夠用——**TAP 把 broadcast 流量過 tunnel 是純粹的 anti-feature**
...
| **DC 部署模型（12.7）** | 假設 G6 server 跑在 K8s pod 內，看到的「網路」是 Cilium/Calico overlay，不是 raw Linux netns |
...
| **不走 TAP** | G6 不引入 L2 broadcast——任何 broadcast/multicast 都是流量指紋 anti-feature |
...
**這就是為什麼設計 G6 必須把「underlay 不可見的 overlay」當成 first-class threat**——不是邊緣 case。
...
`-D` flag 設定 DF（Don't Fragment）。哪個 size 開始 work？這個 boundary 揭示了你 client → server path 上的 effective MTU。**G6 必須對齊到這個 boundary 之下**。
...
4. 一個 packet 從 K8s pod 出去，經過 Cilium Geneve overlay、再過 hyperscaler underlay VXLAN、再上公網。**從外到內**有幾層 encap？G6 packet 設計時 MTU 預算該怎麼算？
5. 為什麼 G6 不應走 TAP？TUN 和 TAP 各有什麼**研究級的**取捨——不只是工程方便性？
...
| **G6 control plane** | 學 EVPN 的「control plane signaling 取代 data plane flood」——比如 G6 client-server 之間的 endpoint discovery 應該走專屬 channel 而非 broadcast/DHT |

**關鍵原則**：G6 設計**不應假設 L2 broadcast / multicast 可用**——overlay 環境下這些常被禁用或加 overhead。任何依賴 broadcast 的 mechanism（如 service discovery via mDNS）都是 anti-feature。
...
- **2026 hyperscaler 的 L2 abstraction 是否還需要**？K8s + Cilium native routing 模式直接走 L3 + BGP，**完全跳過 L2 illusion**——這個趨勢若繼續，VXLAN/Geneve 在 5-10 年內可能被淘汰。對 G6：**到底要不要假設未來 deployment 環境仍有 overlay**？
- **In-band Network Telemetry (INT) 對 anti-fingerprinting 的影響**：Geneve TLV 可以攜帶 per-packet telemetry（switch ID、queue depth、timestamps）。如果未來 ISP 部署 INT，**G6 traffic 過 transit AS 時可能被注入 telemetry tag**——這變成新的 side channel
- **P4 可程式化 switch 對 DPI 的意義**：Barefoot Tofino + P4 讓 switch 在 line rate（6.5 Tbps）做 packet parsing。**理論上**可以做 ML-based traffic classification。GFW 若採購這個層級硬體，G6 對抗壓力上升。**目前公開資料未證實 GFW 用 Tofino，但無法排除**
- **CXL (Compute Express Link) + smartNIC**：CXL 2.0+ 讓 smartNIC 直接共享 host LLC——對 G6 zero-copy 設計影響大，但這是 server-side，影響 G6 server 而非客戶端
...
- **形式化證明 EVPN 收斂**：EVPN 在 multi-homing 場景下的 DF election 是否總會收斂？是否有 starvation case？**目前以工程實證為主，缺乏 formal model**——這個方向 G6 control plane 設計值得參考

---

下一堂：**1.4 IP 層：路由是個圖論問題**——路由表為何要 trie 結構（Linux LC-trie）、FIB 設計、ECMP、policy routing、source routing；對應 Phase III G6 server 出口路由策略。
