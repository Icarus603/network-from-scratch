...
- 觸發哪個 RFC mechanism
- 用了 Part 1 哪一堂的知識
- 可能 fail 的點 + 對 G6 設計的啟示

**對 G6**：G6 client 啟動時的 boot sequence **本質上就是 modified version of 這個 50ms**——把握這 50ms 的時序就把握 G6 client design 的骨架。
...
**對 G6 啟示**：
- G6 client 也需要 endpoint normalization（typo correction、protocol scheme defaulting）
- G6 endpoint preload list 可借鏡 HSTS preload 機制——「**內建可信 G6 server list**」防 DNS poisoning
...
**對 G6 啟示**：G6 client 同樣應發 dual-stack lookup + HTTPS RR——獲取 G6-specific ALPN + ECH 後一次解析。
...
### 對 G6 client 啟動的 1:1 映射

G6 client 啟動序列 **本質上是這個 50ms 的 modified version**：
...
這堂課**本身就是 G6 client design 的 architectural reference**：

| google.com 50ms 環節 | G6 對應設計 |
|---|---|
| HSTS preload | G6 server preload list (內建可信 endpoint) |
| DoH first | G6 client 同 DoH baseline |
| HTTPS RR + ALPN | G6 publish HTTPS RR 含 G6 custom ALPN |
| ECH | G6 baseline ECH usage |
| Happy Eyeballs v2 | G6 across-endpoint Happy Eyeballs |
| QUIC Initial >= 1200B | G6 同 QUIC RFC 9000 baseline |
| Connection coalescing | G6 多 service 同 connection |
| Anycast/edge POP | G6 multi-IP unicast (反 anycast) |
| Connection migration | G6 inherit QUIC migration |
...
### 任務 4（10 min）：對自己 G6 connection 做同樣分析

```bash
# 用 G6 client (e.g. sing-box / clash-meta) 啟動 verbose log
# 同時 tcpdump
sudo tcpdump -i en0 -nn -w /tmp/g6.pcap host <your G6 server>

# 連到 G6 後訪問任意 url
# stop tcpdump

# Wireshark 看：
# 1. DNS lookup G6 server (if applicable)
# 2. G6 handshake (QUIC or TLS)
# 3. Application traffic begin

# 對比 google.com 時序——你 G6 連線比 google.com 慢多少？哪部分最大？
```
...
6. Connection coalescing 在 HTTP/3 比 HTTP/2 更激進——為什麼？對 G6 server 設計有何啟示？
7. 若 google.com handshake 在 t=6ms 失敗（網路問題），browser 怎麼 recover？對 G6 client failure handling 啟示？
8. 同 page 100+ subresource 的時序怎麼安排？preload scanner 機制對 G6 v2 多 service deployment 啟示？
...
g6_boot_sequence:
  ...
  dns:
    - DoH query for G6 control endpoint
    - HTTPS RR for ALPN + ECH + IP hints
    - Happy Eyeballs v2 across endpoints
  control_plane:
    - HTTPS request to Cloudflare Worker (or similar)
    - retrieve signed endpoint list (G6 server pool)
    - rotate based on health + geo
  data_plane:
    - QUIC handshake to selected G6 server
    - ECH encrypted SNI (innerSNI = G6-specific)
    - ALPN custom = "g6/1"
...
下一堂：**1.18 Linux 網路 stack 巡禮**——skbuff 結構、netfilter hooks、qdisc、TC；一個封包從 NIC 到 socket 的完整路徑；對 G6 server 在 Linux 上的部署位置抉擇。Part 1 finale。
