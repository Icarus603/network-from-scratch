...
1. **Proteus server 跑在 Linux 上**——理解 packet 從 NIC 到 Proteus process 走過哪些 code，才知道哪裡可以優化
2. **Proteus 可選的 packet interception 點不只一個**：
...
4. **netfilter / nftables / TC / qdisc 是 Proteus server 部署必須懂的工具**——killswitch、流量整形、DDoS 防護全要用
...
#### 5.2 對 Proteus server killswitch 設計
...
g6_killswitch_implementation:
  ...
  rule_set:
    - allow loopback
    - allow established (conntrack) connections initiated by Proteus client
    - allow DNS to specific resolver (or DoH endpoints)
    - allow Proteus control + data channels
    - drop all else (default)
  install_order: BEFORE Proteus process start (avoid window)
```
...
#### 6.1 主要 qdisc 對 Proteus 的影響

| Qdisc | 適用 | Proteus 推薦 |
|---|---|---|
| **fq_codel** | default; 對 bufferbloat 友善；fair queue + CoDel AQM | mobile / 一般 server |
| **fq** | 純 fair queue + TCP pacing；對 BBR 必要 | BBR-using Proteus server |
...
- **baseline Proteus 走 user space socket**——portable + debug 友善
- **performance-critical Proteus server 可選 eBPF/XDP**：DDoS drop + 流量 fast path
- **Proteus client 不建議 eBPF**：需 root + portability 問題
...
|---|---|
| **12.4 data path** | Proteus server choose: user space (baseline) vs eBPF (high-perf) vs DPDK (specialty) |
...
| **Proteus killswitch** | nftables baseline；eBPF cgroup-bpf 進階 |
...
6. Netfilter 5 hooks 各自時機？NAT 在哪個 hook 發生？Proteus killswitch 該放哪個 hook 與哪個 chain？
7. XDP / TC eBPF / cgroup-bpf 三個 attachment point 各自適合什麼場景？Proteus server production 偏好哪個？
...
⇒ **Proteus user space 走 standard socket：~300K pps 上限/core**；要超越必須 eBPF / AF_XDP / DPDK。
...
- **Marinos et al. 2014 SIGCOMM *Network Stack Specialization for Performance*** — Sandstorm / Namestorm — 札記 [notes/papers/marinos-network-stack-specialization-2014.md](../../notes/papers/marinos-network-stack-specialization-2014.md)
...
| **NUMA-aware deployment** | bind Proteus process to NIC-attached socket |
...
- **Proteus server-side eBPF programs 的 production hardening**：verifier 嚴格但 maintenance cost 高——是否 production-ready
...
- 2.11-2.14 整合 + 對 Proteus 設計的具體 implications
