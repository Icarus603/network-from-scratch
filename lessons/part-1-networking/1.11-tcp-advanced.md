...
本堂三個主題看似不相關，但**對 Proteus 都是 first-order 決策**：

1. **TCP/UDP offload**（TSO/GSO/USO/GRO/LRO/RSS）—— 直接決定 Proteus server 能否跑滿 1Gbps+ throughput。理解 offload 才知道「why wireshark 抓到 65535 byte packet 但 wire 上是 1500 byte」這個常見困惑
2. **MPTCP** —— 多路徑傳輸的 reference design。**Proteus v2 若考慮 multipath QUIC（IETF 在制定）**，MPTCP 是必須先精通的前作；MPTCP 也是 Apple Siri / Korea Telecom 已部署的 production technology
3. **TCP-AO** —— TCP-level authentication，BGP / LDP / RPKI sessions 用。**Proteus 若有 control plane over TCP 必須懂 TCP-AO**，且 TCP-MD5 是 BGP 老 server 仍跑的 legacy，了解其 deprecation 是必要

教科書講這三個的問題：
- offload 只在「**NIC feature**」名目下一筆帶過——不講對流量分析的影響
- MPTCP 多數教科書略過——卻是 modern mobile 與 multi-link DC 的 production 技術
- TCP-AO 幾乎只 BGP 圈知道——但對任何 long-lived TCP connection（包括 Proteus control channel）都直接適用

本堂從 Proteus 設計視角切入，深度展開三個主題的工程現實 + 攻防面。
...
**Proteus baseline = QUIC = 必須 enable USO** for production server。
...
**對 Proteus server**：default RSS 用 5-tuple → 同 flow 同 core——cache friendly。
**對 Proteus multipath（v2）**：不同 subflow 不同 5-tuple → 不同 core → 可選擇是否 align with application-level scheduling。

#### 1.6 對 Proteus 設計的具體 implications

| Offload | Proteus 設計影響 |
|---|---|
| **USO** | QUIC server 必須測試 USO enabled vs not，量化 throughput delta |
| **GRO** | Proteus server receive path 應 evaluate enabling GRO for QUIC |
| **TSO/GSO** | Proteus-over-TCP fallback 需要 |
...
**Proteus baseline 走單路徑 QUIC——不採用 MPTCP**。理由：
...
**Proteus v2 evaluate multipath QUIC**：當 IETF spec stable 後考慮。
...
#### 3.2 對 Proteus v2 設計的具體 considerations
...
multipath_quic_eval_criteria:
  ...
  congestion_control:
    ...
    - 防止 multi-path Proteus 對 fair single-path flow 不公
  handover_seamless:
...
**TCP-AO 設計給「**只需保護 single TCP connection**」場景**——典型 BGP / LDP / Proteus 控制連線。
...
**Proteus baseline 走 QUIC——天然加密無需 TCP-AO**。但：
- Proteus fallback TCP transport 必須走 TLS（不必 TCP-AO，因為 TLS 已給類似保護）
- Proteus control plane 若有 BGP-like signaling（如多 server pool 之間的 health gossip），可考慮 TCP-AO 作為 transport security 基礎
- **TCP-AO 對 Proteus 主要是 reference architecture**——學「**per-connection key derivation + key rotation in-band**」這個設計模式
...
**Proteus server** 直接 inherit 此模式。

#### 5.4 zerocopy send（MSG_ZEROCOPY, Linux 4.14+）

`sendmsg(... , MSG_ZEROCOPY)` 讓 kernel 不複製 user buffer——直接 DMA。**省 CPU 30-40%**，對高 throughput Proteus server 友善。
...
5. TCP-AO 對 TCP-MD5 的 5 個主要改進是什麼？per-connection traffic key derivation 對 Proteus control plane 設計有什麼借鏡？
6. Proteus baseline 為何不走 MPTCP 風格？什麼條件下 v2 應該 evaluate multipath QUIC？
7. NIC offload（TSO/GSO/USO）對 Proteus anti-fingerprinting 設計的影響是什麼？wire 上看到的 packet size 是否還是 Proteus server 控制的？
...
- **可程式化 NIC（SmartNIC, P4-based）** 對 Proteus server 角色：offload 全部 Proteus logic 到 NIC——是 future direction

---

下一堂：**1.12 UDP 完整解剖**——header 8 byte 拆解、UDP-Lite、UDP fragmentation、UDP `connect()` 為何有意義；對應 Proteus baseline UDP transport 的所有 corner case。
