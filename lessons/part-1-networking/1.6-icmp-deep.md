...
ICMP 在大多數人腦中等於 `ping`。但對 Proteus 設計，ICMP 是**至少 5 件大事**的交集：
...
2. **ICMP 是 GFW 的工具**：Ensafi et al. 2015 證實 GFW 不只 passive DPI——還用 **active probing**（含 ICMP / TCP SYN / TLS ClientHello replay）主動連接可疑 server 判斷是否 circumvention service。**Proteus server 必須能識別並抵抗這類 probe**
...
5. **QUIC RFC 9000 與 ICMP 的微妙互動**：QUIC 設計上 **DPLPMTUD-first、ICMP-optional**——但仍**接受並利用** ICMP PTB（packet too big）作 fast hint。**Proteus QUIC fork 設計必須明確此處取捨**
...
Proteus baseline 走 QUIC → 必須實作 DPLPMTUD。具體：
...
⇒ **PMTUD「成功」是例外，blackhole 才是常態**。**Proteus 設計必須假設 blackhole**——DPLPMTUD 是 mandatory，不是 nice-to-have。
...
#### 5.3 對 Proteus server 的影響

**Proteus server 必須處理 active probing**：
...
**Proteus baseline 建議**：rate-limit ICMP echo（防 ping flood、避免成為 amplification）+ 不主動回 ICMP error（**就算 packet 真的 too big，也不回 PTB——讓 client DPLPMTUD 自己探**）。代價：debug 困難——必須有 out-of-band debug channel。
...
- 對 Proteus **不適合用作 baseline**：ICMP throughput 極低（10-100 KB/s）、被 IDS 識別容易
...
**「ICMP 是 hint 不是 truth」**——這個原則對 Proteus 所有控制訊號設計都該適用：
...
對 Proteus：**了解 traceroute 機制 = 了解網路 visibility 的邊界**。從外部 traceroute 你 server 能看到什麼？**到 server 前一跳通常就停**（很多 hyperscaler 把 final hop 設為 not reply ICMP）。**這對 Proteus server 隱蔽性是好事**。
...
| 設計面 | ICMP 知識的影響 |
|---|---|
| **11.1 威脅模型** | active probing（GFW）必須列入；ICMP RST injection 必須考慮（雖然 Proteus 走 UDP 不直接受 RST，但類似 off-path 攻擊存在） |
...
### Proteus server 對 ICMP 的具體 policy 提案
...
icmp_policy:
  ...
  time_exceeded:
    send_on_ttl_zero: false       # traceroute 看不到 Proteus server
    accept_as_hint: true
  redirect:
...
**思考**：你的 host 預設是否 accept redirect？rate limit 是多少？對 Proteus server 應該怎麼調？
...
5. Ensafi et al. 2015 IMC 顯示 GFW active probing 的 trigger 機制是什麼？Proteus server 應該怎麼配置 ICMP 來最小化指紋暴露但仍維持運維可調試性？
6. QUIC RFC 9000 §10.2.4 為何要求「ICMP 只能作 hint 不能作 authoritative signal」？這個原則對 Proteus control plane 設計有什麼推廣價值？
...
**Houmansadr 2013 "Parrot is Dead" 的核心 message**：obfuscation protocols（如 SkypeMorph 模仿 Skype）對抗 active probing **全部失敗**——任何 imperfection 都會被 D 抓到。**Proteus 設計必須假設「完全模仿不可能」，走另一條路：cryptographic indistinguishability 而非 protocol mimicry**。
...
下一堂：**1.7 NAT 完整分類學**——Full Cone / Restricted / Symmetric / CGNAT 與 hole-punching、STUN/TURN/ICE；對應 Proteus P2P 假設與 CGNAT 環境下的部署現實。
