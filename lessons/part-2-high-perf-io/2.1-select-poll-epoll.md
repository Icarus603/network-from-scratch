...
2. **epoll 的紅黑樹 + ready list 雙結構為何是「正確」的資料結構選擇**：這對我們之後設計 G6 control plane 的 connection table 是直接 reusable 的思路
...
4. **驚群問題（thundering herd）與 `SO_REUSEPORT` 的關係**：直接決定 G6 多 worker 架構的設計
...
**G6 設計選擇預告**：我們在 Part 12 大概率用 ET，因為 1M qps 級別的 syscall amortization 重要；但每個 worker 的 connection handler 必須有「每次 epoll 事件後強制 drain」的 invariant，並且配 fuzz test 驗證。
...
**G6 設計取捨**：我們在 server 端會用 SO_REUSEPORT + per-worker epoll，這是 2026 高效能 server 的標準骨架（nginx、HAProxy、envoy 都這樣）。
...
2026 trend：搭 `EPOLLEXCLUSIVE` + `SO_BUSY_POLL` + `SO_PREFER_BUSY_POLL`（5.x 引入）能做到 microsecond 級事件延遲。但通常我們 G6 不需要這麼極限——這 trick 留給特定 deployment scenario。
...
1. **Server 骨架**：G6 server 是 `SO_REUSEPORT × N worker × per-worker epoll(ET) × accept-loop`。這幾乎是 2026 高吞吐 server 的唯一答案
...
4. **busy poll 留作可選**：對「最低延遲」mode，可以給 G6 加 `--busy-poll-us=50` 選項，但預設關閉
...
7. 寫出一個 G6 server 主迴圈的 pseudocode（accept worker × N + epoll ET + drain 到 EAGAIN + EPOLLEXCLUSIVE 或 SO_REUSEPORT）
...
⭐ **G6 設計的歷史座標**：我們的 server 至少要打到 C1M，理想是 C10M。本堂的 epoll 配 `SO_REUSEPORT` 是 C1M 的標配；要進 C10M 必須引入 2.2 io_uring 或 2.7 XDP。
...
⇒ **G6 必須在 user space 加 per-connection priority + token bucket** 才能避免「一條惡意 client 把 worker 餓死其他 connection」。
...
| Connection table | 啟發：紅黑樹 + ready list 雙結構在 user-space connection-tracking 也適用 | G6 是否需要 per-CPU connection table、是否用 lock-free hash（5.x 議題） |
| Fairness | 暴露：epoll 不 fair；user space 必須補 | 用哪個 fairness algorithm（DRR / WFQ / token bucket） |
| Latency 模式 | 暴露：busy_poll / SO_PREFER_BUSY_POLL 是 µs 級 latency 唯一路徑 | G6 是否暴露成 user-tunable |
...
> ⭐ G6 論文如果要 push 系統頂會，**第 1 條和第 2 條** 是 reasonable 的副產出。
