...
> macOS 是 Proteus client 的主要 dev / target 平台，必須清楚它的能力邊界

[Part 0 工具篇](../part-0-orientation/0.5-tooling.md) 已確認：開發在 macOS（Apple Silicon），部署在 Linux VPS。所以**Proteus 必然有兩條路徑**：
...
3. **Network Extension framework**：Apple 後 2015 起逼所有 VPN 用 NE，舊式 `kext` 已死。**這直接決定 Proteus client 怎麼接 macOS 系統**
...
macOS kqueue 對「**client 級流量 (1Gbps、~1K connection)**」綽綽有餘。**Proteus client 用 kqueue 不會撞瓶頸**。
...
⭐ **Proteus 適合 NEPacketTunnelProvider**（packet 級）或 NETransparentProxyProvider（連線級，可選只代理特定 app）。
...
- 無 zero-copy（Swift Data 是 copy）——這是 Proteus client 性能限制之一
...
⭐ **Proteus 推薦走 NETransparentProxyProvider**（macOS 11+）：
...
**Proteus 商用 deployment**：必須過這一關。沒有 Apple Developer ID 寫不出可發行的 NE provider。
...
codesign --sign "Developer ID Application: ..." --options runtime \
         --entitlements Proteus.entitlements --deep YourApp.app
...
未公證的 app 在 Sequoia+ 必須 user 手動「允許」（且越來越難）。**Proteus 必須 notarize**。
...
```bash
sudo dtrace -n 'syscall::recvfrom:entry /pid == $target/ { @[probefunc] = count(); }' -p $(pgrep ProteusClient)
```
...
`pf` 跟 iptables 是兩套不同語法。對 Proteus client 不直接 relevant（client 不做 firewall），但**理解 pf 才能 debug 流量**。
...
⭐ Proteus macOS client 應該基於 `Network` framework + NE Provider，**不要直接用 BSD socket**。

### 7. Proteus macOS client 架構

```mermaid
flowchart TB
    subgraph App["Proteus.app (UI / config)"]
        UI["SwiftUI"]
        MGR["NETunnelProviderManager"]
    end
    subgraph Ext["Proteus NE Provider (extension)"]
        Tun["NEPacketTunnelProvider<br/>or NETransparentProxyProvider"]
        Crypto["Encrypt/decrypt"]
        Net["Network framework or BSD socket<br/>(to remote Proteus server)"]
    end
    Sys["macOS system"]
    Remote["Proteus server (Linux VPS)"]
...
- `Proteus.app`：Swift app，UI + config，從 keychain 拿 credentials
- `Proteus.NetworkExtension`：NE Provider extension（同 app bundle 但獨立 process）
...
對 Proteus：iOS app 要走 App Store 審查。歷史上 GFW 對抗類 VPN app 在 China App Store 全被下架，但在其他 region 仍可。
...
6. **iOS 區分 deploy**：若 Proteus 推 iOS，必須準備 App Store 審查 strategy（可能僅限 region）
...
2. macOS utun 跟 Linux tun 在 packet header 上的差異是什麼？對 Proteus client 跨平台 code 影響？
3. 為什麼 macOS 強制 NE Provider 而非允許 user-space daemon 自行接管網路？（提示：sandbox + 安全 model）
4. NEPacketTunnelProvider 跟 NETransparentProxyProvider 各自適合什麼場景？Proteus 應該用哪個？
5. macOS 為什麼沒有 eBPF？Apple 的替代方案能達到 eBPF 多少能力？
6. Entitlement 跟 Notarization 各自是什麼角色？Proteus 商用 deployment 必過哪幾關？
7. iOS 對 NE 的限制比 macOS 嚴在哪？Proteus iOS 版本可能面臨什麼 App Store 審查問題？
8. 寫一個 Proteus macOS client 的最小架構圖，標清 app process / NE extension process / 跟 system 互動 / 跟 remote server 通訊
...
對 Proteus macOS client：

| 威脅 | macOS 防護 |
|---|---|
| User 安裝惡意 NE 取代 Proteus | NE 簽名驗證 + user prompt |
...
⭐ macOS 安全模型很強，但 **Apple App Store 是政治點**：歷史上 Apple 應中國要求下架某些 VPN app。Proteus 部署策略要考慮：可能無法在 China region 上 App Store（**但 macOS 可繞 App Store 直接 sideload**，iOS 不行）。
...
這個 **strict process isolation** 是 macOS 安全模型的根本。Proteus 設計時不能假設兩 process 共享 state。
...
3. **macOS Network framework 對 Proteus QUIC 整合**：Apple Network framework 已 ship QUIC，能否拿來省下自己 QUIC implementation？trade-off 是 framework 隱藏細節
...
到 2.10，我們已分別講了 Linux + macOS 的高效能 I/O 能力。但 Proteus 客戶端要把流量「**從 system 偷出來送進我們協議**」，這需要 **TUN/TAP** 機制——既深又跨平台。下一堂 [2.11 TUN/TAP 完整深度](./2.11-tun-tap.md) 把 Linux `tun.c` 跟 macOS utun 一次講透，含 `ioctl` 介面、multi-queue、`IFF_NO_PI` 細節，以及為什麼 wireguard-go 用 macOS utun 時對性能造成具體影響。
