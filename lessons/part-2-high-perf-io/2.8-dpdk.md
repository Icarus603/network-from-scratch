...
**Proteus 暫不走這路**：用 user-space stack 等於放棄 Linux kernel 25 年的 TCP 演算法精煉（CUBIC、BBR、PRR、PRR 等）。
...
**Proteus 落點**：商用 VPN 邊界級，**用 kernel + io_uring + XDP，1-10 Gbps 完全夠用**。
...
對 Proteus 的判斷：**全部都是 deal breaker**。我們服務的不是 telecom 而是個人 / 小組用戶。

---

## 與我們協議設計的關聯

1. **Proteus v1 不用 DPDK**：太重、太貴。io_uring + XDP 已涵蓋我們的性能需求
2. **保留 DPDK port 可能性**：若 Proteus 進入 telecom-grade deployment（不太可能但保留可能），可寫 DPDK 版的 server
...
5. share-nothing thread-per-core 是 DPDK 的核心 thread model。對 Proteus server (io_uring + 加密) 是否值得借鏡？理由
6. NUMA-aware 在 single-socket 機器上意義有多大？Proteus 部署的 VPS 通常是 single-socket 還是 multi-socket？
7. 為什麼 Proteus 不走 DPDK？列 5 個具體理由
...
GFW / 大型 ISP IDS 商業設備（Sandvine、Allot）大量用 DPDK 做 line-rate 流量分類。對 Proteus implication：**對手 inspection 能力可以 line rate**。
...
DPDK 的 **latency variance** 顯著小於 kernel stack——這對 HFT、5G URLLC 是核心優勢。對 Proteus（網路 latency 主導），意義有限。
...
| 設計問題 | 本堂收窄了什麼 | 仍 open |
|---|---|---|
| 是否走 DPDK | **否**，Proteus 用 kernel + io_uring + XDP | telecom-grade deploy 路線 |
...
5. **對 Proteus 是否值得做 DPDK port**：純工程問題但研究意義有限。除非 Proteus 進 NFV 領域

---

## 對下一堂的鋪墊

DPDK 給你 raw packet 但**沒有 TCP/IP stack**。下一堂 [2.9 user-space TCP stack](./2.9-userspace-tcp.md) 講 mTCP / F-Stack / Seastar / lwIP / smoltcp——這些 user-space 實作怎麼補足那塊空白。重點：**Proteus 是否需要 user-space stack？答案多半是「不」，但理解選項對最終設計重要。**
