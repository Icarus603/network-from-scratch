...
> 跳過 Linux kernel TCP 是極大決策，但 Proteus 應該理解何時值得
...
**Proteus 為什麼大概率不用 user-space stack**：
...
4. **Proteus 的瓶頸不在 TCP stack**：而在 user-space 加密 + 抗指紋邏輯
5. **協議自由度高**：Proteus 設計時可基於 UDP / QUIC（自己控 transport），不必依賴 TCP——若用 TCP 就放 kernel 跑

**但為什麼仍要學**：

1. **理解 mTCP 的 multi-core 設計**：讓你看清 Linux TCP 的 lock contention 病灶——對 Proteus server side multi-worker 設計直接 inform
2. **理解 specialization 哲學**：Marinos 2014 提的「**為 application 量身打造 stack**」思想—— Proteus 自己其實就是「為 proxy 量身打造的 transport」
3. **smoltcp / netstack3 的 Rust 模型**：若 Proteus 將來嵌入 router OS / embedded 場景，smoltcp 是現實選項
...
⭐ **對 Proteus 的啟示**：即使我們不抄 mTCP 的 stack，**share-nothing thread-per-core + 每 worker 獨立 connection table** 是 lesson 一致的。
...
對 Proteus：**F-Stack 是「相對 lower risk 的 user-space stack 選項」**——如果決定走這路，F-Stack 不要重新發明。
...
**對 Proteus implication**：客戶端若移植到 WASM / browser / embedded device，smoltcp 是現實 transport stack。對 server 不適用。
...
⭐ **Proteus 的決策**：**用 Linux kernel TCP**。理由：
...
**但若 Proteus 走 UDP-based / QUIC-based transport**（Part 8 決定），這節討論就不相關——QUIC 本身就是 user-space transport。
...
**對 Proteus 的啟發**：Proteus 自己其實就是「**為 proxy 量身打造的 transport**」——我們不需要 TCP 所有 feature（流量控制 / 重組 / window scale 是必要的，但很多 corner case 可以簡化）。如果我們走 UDP-based transport（QUIC 或自定），這就是 specialization 哲學的直接應用。
...
**QUIC 是 specialization 哲學的勝出實例**：不重寫 TCP 而是設計 better transport。Proteus 大概率走這條路。Part 8 全部議題。

### 10. 對 Proteus 的最終判斷

```
Proteus transport selection decision tree:
...
1. **不採用 mTCP / F-Stack / Seastar**：Proteus server 不重寫 TCP
2. **吸收 share-nothing thread-per-core 哲學**：用於 Proteus server worker 設計（io_uring runtime 層面）
3. **吸收 specialization 哲學**：Proteus 是「為 proxy 量身打造的 transport」——若走 UDP/QUIC，這直接相關
4. **smoltcp 留給 future embedded**：Proteus client 移植到 OpenWrt / 路由器嵌入式裝置時的選項
5. **QUIC 是真正的「2026 user-space stack」**：Part 8 主題
6. **netstack3 (Rust) 是 industry trend**：Google Fuchsia 把整個 stack 用 Rust 重寫，是 Proteus 將來若考慮「memory-safe Rust transport」的 reference
...
2. share-nothing thread-per-core 對 Proteus server 是否該照抄？答 yes 跟 no 的具體理由
3. F-Stack 跟 mTCP 在「重用既有 stack」上的取捨差什麼？工業界更傾向哪個？
4. specialization 哲學（Marinos 2014）對 Proteus 來說最具體的應用是什麼？舉一個 Proteus transport 可以「砍掉的 TCP feature」
5. smoltcp 跟 mTCP 都是 user-space TCP 但「目標市場完全不同」——具體差什麼？
6. QUIC 為什麼可以視為「specialization 哲學的勝出實例」？跟 mTCP 路線的本質區別在哪？
7. Proteus 大概率走 Linux kernel TCP 而非 user-space stack——本堂提供的 5 個理由是什麼？
8. 若 Proteus 將來想嵌入 OpenWrt 路由器（資源受限），會考慮哪些 user-space stack？
...
⇒ 對 Proteus 不採用 user-space stack 是合理的安全保守選擇。
...
Proteus server target：$N=8$ core 達 $T \ge 6 \cdot T(1)$，即 $\alpha \le 0.04$。Linux 6.x + io_uring 達標。
...
4. **Stack specialization for proxy**：給 proxy 量身打造的 transport，是 Proteus 的核心 framing。Part 11 design 直接相關
5. **memory-safe Rust transport in production**：netstack3 production 表現如何？是否能取代 Linux kernel TCP？

> ⭐ 第 4 條 **stack specialization for proxy** 直接對應 Proteus 設計，是論文核心 framing。

---

## 對下一堂的鋪墊

到 2.9，Linux 高效能 I/O 全景（epoll / io_uring / zero-copy / kTLS / eBPF / XDP / DPDK / user-space stack）已講完。但 Proteus 客戶端**會在 macOS 上跑**——macOS 完全沒有上述任何 Linux-specific 機制。下一堂 [2.10 macOS](./2.10-macos.md) 講 macOS 對等技術：kqueue、Network Extension、PacketTunnelProvider、為何 macOS **沒有** eBPF / XDP。讀完 2.10 你會清楚知道「**Proteus 客戶端在 macOS 能做到什麼、做不到什麼**」。
