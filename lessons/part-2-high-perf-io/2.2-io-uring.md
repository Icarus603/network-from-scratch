...
- io_uring 仍然新且 **CVE 多**（2020-2024 已有 ~20 個 CVE，多家雲商 disable io_uring 是 default policy）。我們要評估 Proteus 是否「值得」用 io_uring，這個 trade-off 是真實的
...
⭐ **對 Proteus**：listen socket 的 accept loop 一條 multishot SQE 解決，server hot path 上 SQE 提交頻率大幅下降。
...
對 Proteus：**主要送大 buffer 才開 SEND_ZC**。一般 control message 用普通 SEND。
...
對效能也好：少 context switch、credential cache 命中、CPU affinity 自然。**Proteus server hot ring 應該都開這兩個 flag**。
...
**Proteus 設計選擇**：在 Rust 生態用 [`tokio-uring`](https://github.com/tokio-rs/tokio-uring) 或 [`compio`](https://github.com/compio-rs/compio) crate，把 io_uring 抽象在 runtime 層。client 退 mio (epoll/kqueue)，**transport 層走 trait `AsyncRead/AsyncWrite`** 統一 API。
...
**Rust 生態**：[`io-uring`](https://crates.io/crates/io-uring) crate (低階)、[`tokio-uring`](https://crates.io/crates/tokio-uring)、[`compio`](https://crates.io/crates/compio)、[`monoio`](https://github.com/bytedance/monoio)（字節跳動的 thread-per-core runtime，**設計專為 io_uring**）。**Proteus 候選 runtime 評估**會在 Part 12 做。
...
這對 Proteus 是個機會點：如果用 Rust + monoio，比 Go 寫的 V2Ray/Xray 系列在純 IO throughput 上**理論上能 2-3 倍**。

---

## 與我們協議設計的關聯

1. **Server runtime 選型**：基於 io_uring 的 Rust runtime（monoio / compio / tokio-uring）。**這是把 Proteus push 到 1M qps 的關鍵單一決策**
...
8. 寫一個 Proteus server 主迴圈的 pseudo code：multishot accept、per-conn recv→encrypt→send chain、shared buffer ring、SQPOLL + DEFER_TASKRUN + SINGLE_ISSUER
...
⭐ **Proteus 設計**：先用 io_uring 默認 mode 量 baseline，再逐項打開 REGISTER_FILES / SQPOLL / DEFER_TASKRUN，看哪個對 Proteus 流量 pattern 賺最多——**不要一次全開**，會 debug 不能。
...
⇒ **Proteus 加密層的 nonce 序列**設計時，**不能假設兩個 SEND 提交順序 = 線上順序**，必須 sequence number 自己維護。
...
1. **io_uring + kTLS 整合**：目前 kTLS 跟 io_uring 沒乾淨的整合路徑。`SEND_ZC + kTLS-offload` 是 active 開發中的議題。能否設計一個「ring 內 TLS record encrypt」對 Proteus 是巨大效能 win
...
5. **eBPF + io_uring 結合**：能否註冊一個 BPF program 在 CQE 產生前 transform？這對 Proteus packet-level filtering 大有用

> ⭐ Proteus 若 push 系統頂會，**第 1 條 io_uring + kTLS 整合** 是直接可組合進 protocol design 的子貢獻。
