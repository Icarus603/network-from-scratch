...
> **我們協議的座標**：CID-aware reuseport BPF program 是 G6（我們協議）server 高並發必備。Part 11.X 設計握手時 CID 結構要對齊這個分發需求，**不能把 CID 設計成連線中變動**——這會在 worker 切換時引爆。
...
我們做 evaluation（Part 12.15–17）時要用 AF_PACKET 寫 DPI mock，看 G6 流量在 GFW-like classifier 面前的「真實裸樣」（沒被應用層 buffer 模糊化的時序）。
...
在 G6 設計空間（Part 11 將完整繪製）中，本堂收窄了以下選擇：
