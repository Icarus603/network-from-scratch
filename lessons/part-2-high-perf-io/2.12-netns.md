...
對 Proteus 設計，netns 解決三個現實問題：

1. **協議測試**：要驗證「**Proteus client → Proteus server**」流程，傳統做法是租兩台 VPS。netns 讓你在筆電上就能跑 client + server + 模擬中間網路（含 GFW-like middlebox）
2. **對抗測試**：在 netns 內加 `tc netem` 模擬「中美 100ms RTT + 5% packet loss」，做 reproducible benchmark（[2.13](./2.13-tc-netem.md) 詳講）
3. **CI/CD pipeline**：GitHub Actions / GitLab CI 內跑 Proteus 整合測試，全部用 netns 拉拓樸，秒級啟動，零雲端成本
...
對 Proteus 開發來說，netns 是「**reproducible network experiment 的基礎工具**」。Part 9 / Part 10 / Part 12 的對抗實驗會大量用 netns。
...
要驗證「**client → 中間網路 → Proteus server**」：

```mermaid
flowchart LR
    subgraph nsc["ns_client"]
        c["Proteus client<br/>10.0.0.1"]
    end
...
- 在 ns_c 跑 Proteus client
- 在 ns_s 跑 Proteus server
...
之後 Proteus baseline benchmark 在這拓樸下跑——比租 VPS 量 1 次穩定 100 倍。
...
對 Proteus 自動化 testing 環境**比手寫 ip netns 命令更穩定**。
...
**Proteus 開發**：在 macOS 寫 code，但**所有 netns-based 整合測試在 OrbStack 內跑 Linux VM**。
...
底層全是 netns + veth。**Proteus 整合測試直接用這套**——比手寫 ip netns 命令穩定。
...
2. **CI 跑 Proteus 整合測試**：GitHub Actions runner 內建 netns + tc，秒級拉拓樸
3. **對抗測試**：在 middle ns 用 BPF 抽 Proteus 出口流量特徵，驗證抗指紋
...
7. **整合 containerlab**：Proteus 整合 test 直接寫 yaml，標準化
...
跑你的 Proteus prototype，看 BPF map output。**驗證 Proteus 出口流量是否真的隨機**。
...
中間 ns_m 內裝 mitmproxy 或 sniproxy，把 Proteus 流量導進去，看能不能在 TLS handshake 階段識別 Proteus。**直接面對 GFW 級對手的 baseline**。
...
4. user-ns + net-ns 讓 unprivileged user 能建 netns，這對 Proteus testing 是 win 還是 risk？
5. 為什麼 Cilium 用 XDP 替代 veth 在 host-pod 路徑上能巨大提升性能？
6. macOS 上完全沒有 netns。Proteus 在 macOS 上開發時，整合測試應該在哪裡跑？
7. containerlab vs 手寫 ip netns 命令的 trade-off？Proteus v1 應該用哪個？
8. 寫一個 Proteus 「對抗測試」netns 拓樸 yaml，含 client / middle (GFW) / server 三節點 + tc netem + BPF flow inspector
...
⇒ **Proteus baseline benchmark 必須在 netns 跑，VPS 只用於 final validation**。
...
5. **Proteus 整合測試 framework**：能否標準化「**對抗測試 baseline scenarios**」？對 academic Proteus paper 重要

---

## 對下一堂的鋪墊

本堂建好了「**拓樸**」。下一堂 [2.13 tc / netem](./2.13-tc-netem.md) 講「**怎麼模擬具體鏈路條件**」：HTB、fq、fq_codel、cake、netem 完整參數。重點是「**怎麼用 netem 模擬中美鏈路 5% 丟包 200ms RTT**」——這是 Proteus 對抗測試的核心工具。
