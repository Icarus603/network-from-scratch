...
```
[user app] → connect()/send() → kernel stack → routing → ★ TUN device ★ → ★ G6 client process read() ★
                                                                              ↓ encrypt
                                                                              ↓
[user app] ← recv() ← stack ← ★ TUN write() ★ ← ★ G6 ★ ← decrypt ← network ← G6 server
```
...
- 我們 G6 client 的 transparent-proxy 模式
...
flowchart LR
    ...
    APP["user app"]
    PROC["G6 client process"]

    APP -- "send(8.8.8.8)" --> STACK
    STACK --> RT
    RT -- "route 0.0.0.0/0 dev tun0" --> TUN
    TUN -. "fd read" .-> PROC
    PROC -- "encrypt + tunnel" --> REMOTE["remote G6 server"]
...
每個 fd 由獨立 worker thread 處理。packet 進 tun 用 hash(5-tuple) 分配到 queue。**對 G6 server 也適用**（但 server 用更高層的 io_uring 多 worker，不必依賴 tun multi-queue）。
...
對 G6 用：write 大 segment 一次（64KB），讓 kernel 自己 segment 成 MTU-size packet。**throughput 顯著提升**（少 syscall + 多 batch）。
...
4. **`xdp_prog`**：⭐ TUN device 支援 XDP！可以對「進 tun 的 packet」做 XDP filter——對 G6 來說，能在 tun 入口直接 drop 不感興趣的 packet
...
**G6 client 也會踩同樣問題**：若用 TUN mode，必然有 user-space 介入。每 packet ~200ns 的 syscall + copy overhead。
...
對 G6：開全 4 個 flag + io_uring 是 sweet spot。

#### 4.3 splice 從 tun 到 socket（不適用 G6 但要知道）
...
tun fd 支援 `splice_read`。對 plaintext 路徑可以**完全不過 user-space**。但 G6 要加密，splice 不適用。
...
**G6 若做 Windows 版**：wintun 是 sane default。安裝 driver 即可。
...
**G6 client TUN abstraction 應該抄這個 interface 設計**。
...
sequenceDiagram
    ...
    participant G6 as G6 Client
    participant Srv as G6 Server

    App->>Stack: connect(1.2.3.4:443)
    Stack->>Stack: routing → default route → tun0
    Stack->>Tun: TCP SYN packet
    Tun->>G6: read(fd) → SYN
    G6->>G6: parse 4-tuple (src/dst)
    G6->>G6: open virtual connection
    G6->>Srv: encrypted G6 packet carrying SYN
    Srv-->>G6: encrypted reply (SYN-ACK)
    G6->>Tun: write(fd) → SYN-ACK
    Tun->>Stack: deliver
    Stack->>App: connection established
    Note over App,Srv: 之後雙向資料流經 TUN
```

關鍵：**G6 client 必須在 user-space 跑一個簡化的 TCP/UDP stack**——因為它從 TUN 拿到的是 raw IP packet。可用：
...
**G6 v1 推薦用既有 stack**（gVisor 或 smoltcp），不自寫——TCP corner case 多到爆。
...
```
[user app] → kernel stack → iptables OUTPUT REDIRECT → 127.0.0.1:8080 (G6 client listening) → user-space proxy
```
...
server 端通常 listen socket 直接收 client packet，不用 TUN。但若 G6 server 想 **「把 client 流量 NAT 出去」**（client → G6 server → internet），可能配置：

```
client packet → G6 server socket recv → G6 server decrypt → ★ write to server-side TUN ★ → kernel stack → routing → out NIC
```

server-side TUN 配合 `iptables -t nat -A POSTROUTING -o eth0 -j MASQUERADE`，把 G6 來的流量 SNAT。**這是大多數 VPN server 的標準模式**。

G6 設計：v1 簡化，**只做 proxy（4-tuple level）而非 VPN（packet level）**，server 不裝 TUN。如未來做 site-to-site VPN，server 端才裝。

---

## 與我們協議設計的關聯

1. **client 必有 TUN mode**：transparent proxy 是 G6 client 的核心 UX
...
對 G6 設計 reference 直接可用。

---

## 自我檢查

1. TUN 跟 TAP 的差別具體是什麼？G6 應該用哪個？理由
...
5. macOS utun 比 Linux tun 缺哪些 feature？對 G6 client 跨平台一致性有什麼影響？
6. TUN-based transparent proxy 跟 iptables REDIRECT 各自優缺點？G6 應該都支援嗎？
7. user-space TCP/IP stack（gVisor netstack / smoltcp）在 G6 client TUN mode 是必要的嗎？為什麼？
8. 為什麼 G6 v1 server 不裝 TUN？什麼情況下會考慮加？
...
TUN 是 system-level capability，被誤用會繞過防火牆。對 G6 client：

- 必須 root / Network Extension entitlement 才能 open TUN
- TUN 跑起來後，**所有流量都暴露給 G6 process**——隱私敏感
- 對企業環境：IT 可能不允許用戶建 TUN
- macOS sandbox 嚴格控管 TUN（NE entitlement only）

設計 implication：G6 client 部署文件須清楚說明「**我們有 root / NE 權限**」，並 sign / notarize 讓 user 信任。

### 3. 形式化定義：TUN 路徑延遲

定義 TUN-mode packet 在 G6 client 內的延遲：
...
⇒ **G6 client TUN mode 性能上限 ~1Gbps**。多數消費場景夠。
...
> ⭐ 第 4 條對 G6 抗對抗 deployment 有意義——若對手能 detect「**user 啟用了 TUN**」，這就是 OS-level fingerprint，必須評估

---

## 對下一堂的鋪墊

本堂講 TUN 是「**把流量從 kernel stack 抽出來給 user-space**」。下一堂 [2.12 網路命名空間 (netns)](./2.12-netns.md) 講「**怎麼建立隔離的網路環境**」——這對 G6 protocol testing 是必備工具：你能在同一台機器跑 client+server+模擬中間 GFW，全部在 netns 內。讀完 2.12 你能建任意網路拓樸做對抗測試，不必租兩台 VPS。
