...
上圖把 L4 demux 和 socket queue 畫成單一節點，這對 TCP 路徑夠用。**但 G6 是 UDP-only 協議**（[2.15](./2.15-udp-fastpath.md) 鎖定的設計決策），UDP 一側額外有 GSO/GRO/批次 syscall 的子路徑，必須單獨展開：
...
- 上圖每個粉色節點都對應 G6 server 必須配置或顯式啟用的功能
