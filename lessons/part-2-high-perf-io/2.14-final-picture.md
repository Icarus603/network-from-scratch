...
> 把 13 堂的點連成 Proteus 系統的「**完整 stack layer 全圖**」

過去 13 堂我們一個 mechanism 講一堂，獨立看每個都是「**性能 trick**」。本堂要回答：

1. **一個 packet 從 NIC 到 Proteus application 的完整路徑經過多少 layer？每 layer 做什麼？**
2. **Proteus server / Proteus client 各自的 stack 設計選擇是什麼？跨平台怎麼一致？**
3. **整個 Part 2 為 Proteus 設計鎖死了哪些事？還留了哪些開放問題給 Part 11/12？**
4. **2026 trends + 2030 predictions：DPU、SmartNIC、kernel-bypass / kernel-extension 的下一波**

讀完本堂你會擁有一張「**Part 2 → Part 11 → Part 12 的串接圖**」，是 Proteus 設計階段的 reference。

---

## 核心內容

### 1. 一個 packet 在 Proteus server 完整路徑
...
flowchart TD
    ...
    USR_PROTOCOL["★ Proteus protocol parser<br/>(Part 11 / Part 12)"]
...
**13 個 step**。粉色節點 = Proteus 直接控制 / 配置 / 寫 code 的部分。
...
| XDP filter (drop path) | ~50ns | Proteus code |
| skb alloc + netfilter + tc ingress | ~1-2μs | kernel + Proteus BPF |
| L4 demux | ~500ns | kernel |
| socket recv queue → io_uring | ~500ns | io_uring runtime |
| user-space decrypt | ~1-2μs (1500B ChaCha20) | Proteus code |
| Proteus protocol parser | ~500ns | Proteus code |
| user-space encrypt | ~1-2μs | Proteus code |
| io_uring SEND_ZC | ~500ns | io_uring |
| qdisc + tc egress | ~1-2μs | kernel + Proteus BPF |
...
#### 1.b UDP fastpath：上圖在「UDP 一邊」的真實樣貌

上圖把 L4 demux 和 socket queue 畫成單一節點，這對 TCP 路徑夠用。**但 Proteus 是 UDP-only 協議**（[2.15](./2.15-udp-fastpath.md) 鎖定的設計決策），UDP 一側額外有 GSO/GRO/批次 syscall 的子路徑，必須單獨展開：

```mermaid
flowchart LR
    subgraph rx["UDP RX 子路徑（kernel）"]
        NIC_RX[NIC RX queue<br/>RSS hash] --> NAPI[NAPI poll]
        NAPI --> UGR{{"UDP GRO<br/>(udp_gro_receive)"}}
        UGR -->|"merge 同 5-tuple<br/>連續 datagrams"| SKBQ[socket queue<br/>大 skb + cmsg]
        SKBQ --> RMM["recvmsg / recvmmsg / io_uring multishot"]
    end
    subgraph tx["UDP TX 子路徑（kernel）"]
        SMM["sendmmsg<br/>+ UDP_SEGMENT cmsg<br/>+ SCM_TXTIME"]
        SMM --> USEG{{"udp4_ufo_fragment<br/>切 gso_size"}}
        USEG --> FQ["sch_fq<br/>(EDT pacing by TXTIME)"]
        FQ --> BQL[(BQL throttle<br/>per TX queue)]
        BQL --> NIC_TX[NIC TX ring]
    end
    classDef ours fill:#fde,stroke:#c39
    class UGR,USEG,FQ,BQL,SMM,RMM ours
```

- 上圖每個粉色節點都對應 Proteus server 必須配置或顯式啟用的功能
- 缺任一節點，10Gbps line rate 都做不到（典型瓶頸：缺 GSO 卡在 100 MB/s；缺 GRO 接收側卡在 200 MB/s；缺 BQL 則 BBR 估算錯亂）
- 詳細 setsockopt / cmsg API、commit hash、量化測試數字見 [2.15](./2.15-udp-fastpath.md)

### 2. 一個 packet 在 Proteus client (Linux) 完整路徑
...
flowchart TD
    ...
    TUN["TUN device (2.11)"]
    USR_TUN_READ["★ Proteus client read(tun_fd)"]
    USR_STACK["★ smoltcp user-space stack<br/>(2.9)"]
    USR_PROTO["★ Proteus framing & encrypt (2.3)"]
    USR_SOCK["★ Proteus socket send to remote Proteus server<br/>(io_uring or kqueue)"]
    K_SOCK["kernel TCP/UDP layer"]
    NIC_OUT["NIC TX"]
    WIRE["→ to remote Proteus server"]
...
### 3. Proteus 系統 stack 全景 (整個 Proteus deploy)
...
flowchart LR
    ...
    subgraph Client["Proteus Client (macOS/Linux/Windows)"]
        UI["Proteus UI<br/>(SwiftUI / Tauri)"]
        CORE["Proteus core<br/>(Rust)"]
    ...
    subgraph Server["Proteus Server (Linux VPS)"]
        XDP["XDP filter"]
        IOURING["io_uring runtime"]
        SERVER_CORE["Proteus server core (Rust)"]
        OUT["upstream socket"]
    end
...
### 4. Proteus Stack 設計選擇 final lock-in（Part 2 收尾）
...
| **Server kTLS** | NO（Proteus framing 非 TLS record） | [2.4](./2.4-ktls.md) |
...
4. **QUIC 完全取代 TCP for new protocols**：包含我們的 Proteus
...
對 Proteus：所有「**busy poll + IRQ moderation**」設計都從這條來。理解此 paper 才能 reason about「**為什麼 io_uring + XDP + DPDK 都用 poll-mode**」。
...
對 Proteus implication：**100Gbps+ deployment 必須意識到 PCIe 是瓶頸**。但 Proteus v1 不接觸這層級。
...
對 Proteus：**Proteus server 的 packet processing 內部用 Click-style graph 抽象**——容易測試、可視化、單元測。VPP / Cilium 都受 Click 影響。
...
Part 2 **不是孤立的工具篇**——它是 Proteus 設計的「**系統層基礎**」，跟密碼學 / 協議 / 對手分析 / 形式化驗證並列構成 Proteus 設計的 5 條 leg。
...
- [ ] 能 explain 「**為何 Proteus 不用 DPDK / 不寫 user-space TCP / 不走 kTLS**」
...
**8/10 以上**：你具備寫 production-grade Proteus server / client 的系統 layer 能力。可以進 Part 3 學密碼學。  
...
整個 Part 2 都是為了 Proteus 設計。本堂 §4 已給完整 lock-in 表。Part 11 在做 Proteus design specification 時，**直接引這張表**。
...
完成後你會有一張「**Proteus server 程式骨架 component map**」。

### 實驗 B：寫一個整合腳本

```bash
#!/bin/bash
# proteus-server-stack-up.sh
set -e
...
echo "Proteus server stack ready"
```

**這就是 Proteus server 部署的 day-0 setup**。

### 實驗 C：建一個 end-to-end test scenario

用 [2.12 §11](./2.12-netns.md) 的腳本拉拓樸，加 [2.13 §11](./2.13-tc-netem.md) 的 netem，跑你自己的 Proteus prototype（即使只是 echo）。量：

- baseline TCP throughput
- Proteus prototype throughput
- 切 BBR / CUBIC 比較
- 開 / 關 SO_REUSEPORT 比較

**這是 Proteus 自我量測 framework 的第一個實驗**。
...
2. 為什麼 Proteus server 不走 DPDK？不走 user-space TCP stack？不走 kTLS？三個獨立但相關的決策
3. Proteus client 在 macOS 上的 path 是什麼？跟 Linux client 差幾個 stage？
4. user-space crypto 必然 1 byte touch；唯一能降到 0 的路徑是什麼？對 Proteus 是 future direction 還是當下要做？
5. Part 2 已鎖死什麼？仍 open 給 Part 11 的設計問題是什麼？列 5 項
6. Mogul livelock / Neugebauer PCIe / Kohler Click 三篇 paper 對 Proteus 各帶來什麼啟示？
7. 2030 prediction 哪幾條對 Proteus 設計直接 relevant？我們留了什麼 future-proof 介面？
8. 如果有人問你「Proteus server 跟 Hysteria2 server / TUIC server 比起來在 I/O stack 上有什麼**本質**差別」，你能說出什麼？(提示：Go runtime vs Rust + io_uring 的根本差)
...
對 Proteus，**對手（GFW、ISP）也可以做 stack-level 升級**。Threat model 必須包含：
...
⭐ Proteus 設計 assumption：**對手有跟我們同級的 stack 工具**。**抗指紋必須對抗 line-rate adversary**。

### 3. 形式化定義：Proteus stack design as a constraint satisfaction problem
...
⭐ 這四篇 + Part 2 之前抓的 (Banga 1999、Lemon 2001、Axboe 2019、McCanne 1993、Høiland-Jørgensen 2018、Jeong 2014、Cardwell 2017、CAKE 2018、RFC 8290) 共 **13 篇 paper / spec**，就是 Proteus 系統層設計的 **canonical reading list**。
...
### 7. 開放問題（彙總，給 Proteus push 系統頂會）

1. **eBPF in-kernel custom AEAD**：Proteus 自定 cipher 在 BPF 內跑 — [2.4](./2.4-ktls.md) §7、[2.5](./2.5-ebpf-intro.md) §7、[2.7](./2.7-xdp.md) §7
...
6. **Stack specialization for proxy**：Proteus framing 是 specialization 哲學的應用 — [2.9](./2.9-userspace-tcp.md) §7
...
> ⭐ Proteus PhD thesis 至少可以涵蓋 **3-4 個** 這些 open problem 作為 contribution。Part 11 design specification 直接 reference 這份清單。

---

## 對下一個 Part 的鋪墊

Part 2 整個系統 layer 講完了。但 Proteus 真正的「**對抗論述**」要從 **密碼學** 開始——這是 Proteus 安全保證的核心。
...
讀完 Part 3 你會擁有「**Proteus 密碼學 layer**」的完整設計權。Part 2（系統）+ Part 3（密碼）合起來是 Proteus design 的 **70% 技術基礎**。剩下 30%（TLS/QUIC、形式化、GFW 對抗、設計收尾）在 Part 4-10 完成。
