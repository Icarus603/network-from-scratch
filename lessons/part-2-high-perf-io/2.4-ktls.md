...
> Proteus 要不要為了 kTLS 改自己的 framing？這是本堂要回答的設計 fork
...
但 kTLS 對 Proteus 有個**根本問題**：它只懂**標準 TLS 1.2 / 1.3 record format**。我們設計新協議時：
...
3. **TLS handshake 加 ApplicationData record 切換 + key schedule**：要兼容 kTLS 必須走完整 TLS 1.3 handshake，這跟 Proteus 設計目標的 0-RTT / 自訂 key exchange 衝突

本堂三個目標：

1. 講清楚 kTLS 怎麼運作（state machine、socket option、限制）
2. 評估 Proteus 是否走 path B
3. 不走 path B 也要懂 kTLS——因為 Proteus server 可能要混部 nginx (kTLS) 做 fronting
...
**Proteus 不直接適用**（我們不 serve static file），但機制要懂——因為**理解 kernel 怎麼串 sendfile/splice/kTLS 是設計協議的 reference 心智模型**。
...
- 自訂 record format 完全不行——這就是 Proteus 的硬限制
...
非 enterprise 級 NIC（消費卡、virtio-net）**完全不支援**。Proteus 部署在 budget VPS（KVM virtio）幾乎沒機會用到 hw offload。
...
### 8. kTLS vs Proteus 的關鍵 trade-off 分析

#### Option 1: Proteus 走自訂 framing（不用 kTLS）
...
#### Option 2: Proteus framing 兼容 TLS 1.3 record
...
- ❌ **跟 REALITY 的「藏在真 TLS 後面」設計衝突**——REALITY 是「TLS handshake 跟真 nginx 走、然後切換成自訂 traffic」，Proteus 走純 TLS 1.3 record 反而**沒有 REALITY 的 TLS-in-TLS 隱藏優勢**

#### 結論：**Proteus 預期走 Option 1**

理由：
1. **抗指紋**比 throughput 更重要（per [Part 0.5 SOTA tooling](../part-0-orientation/0.5-tooling.md) 與整個 Part 9 GFW research 的方向）
2. **Proteus 是 proxy，不是 file server**：sendfile 場景沒有，kTLS-HW 的 file-cache pipeline 不 relevant
3. **byte touch 1 次跟 0 次差距**在我們的目標 throughput (1-10Gbps) 不是 deal breaker

但保留**未來門**：Proteus 若有「企業內網模式」（受信任環境、無對抗、追求極致 throughput），可加 kTLS sub-mode。

### 9. 替代路徑：CCM、SoftAES 與 Proteus user-space 加密的最佳化
...
**Proteus 達標 1 cycle/byte ChaCha20 + Poly1305**：1Gbps = 125 MB/s = 125M byte/s ≈ 0.4 GHz CPU = **單 core ~15% 用量**。完全可接受。
...
但 Linux 6.x 起有 **QUIC kernel offload** 在規劃中（[net-next discussions 2024](https://lore.kernel.org/netdev/)）——把 QUIC 整個 packet protection 進 kernel。這對未來 Proteus 若走 QUIC 底層是 relevant。
...
1. **Proteus 主協議走 Option 1**：user-space 加密 + io_uring SEND_ZC + register_buf_ring。byte touch 1 次
2. **保留「TLS-compatible mode」作為未來門**：未來若做企業版 / 受管路徑，可加開 kTLS。但這條路要 framing 改動，預估 Part 11.10+ 才會討論
3. **kTLS 仍是必懂技術**：因為 Proteus 可能 deploy 在 nginx 後面（混部 fronting），nginx 自己用 kTLS 不影響 Proteus
4. **加密性能不再是設計限制**：本堂分析顯示 user-space ChaCha20 在現代 CPU 上 1 cycle/byte，1Gbps 用 15% core。Proteus 不需為了 kTLS 犧牲抗指紋
5. **NIC offload 不在路線圖**：消費級 / VPS 級硬體不支援，不投資
6. **QUIC kernel offload 持續追蹤**：若 Part 8 決定 Proteus 走 QUIC，這條 future work 直接 relevant
...
2. kTLS-SW 跟 kTLS-HW 的根本差異是什麼？哪些 NIC 支援 HW offload？我們 Proteus 預期 deploy 場景能用 HW 嗎？
3. sendfile + kTLS 為何能達到「user 0 byte touch」？對 Proteus 是不是直接 applicable？為什麼不（或為什麼是）？
4. Proteus 走 Option 1（自訂 framing、user-space 加密）後，每 byte touch 幾次？為什麼這個數字是 user-space encrypt 路徑的下界？
...
**對 Proteus 的 implication**：選擇「touch 發生在哪」是設計變數。User-space 給你最大靈活度但 thermal/cache 不利；kernel 中間；NIC 最低但供應商鎖定。
...
| 設計問題 | 本堂收窄了什麼 | 仍 open |
|---|---|---|
| 是否走 kTLS | **否**，Proteus 走自訂 framing + user-space 加密 | 未來「企業/受信版」可能反向 |
...
5. **Proteus 自訂 framing 也能 in-kernel 嗎？**：能否寫一個 eBPF program 在 kernel 內做我們的 AEAD？目前 BPF verifier 不支援 loop heavy operation，但 BPF 演化中可能解開——這對 Proteus 是 future win

> ⭐ 第 5 條（**eBPF in-kernel custom AEAD**）若可行，是 Proteus push 系統頂會的乾淨 contribution——「**自訂 cipher 也能享受 kTLS 級 zero-touch**」。
...
下一堂 [2.5 eBPF 入門](./2.5-ebpf-intro.md) 講 eBPF 的全貌：verifier、JIT、helper、map、CO-RE。讀完你會明白為何 eBPF 是 2026 Linux 性能 / 安全 / observability 的核心 framework，以及它對 Proteus 有什麼具體用法。
