...
1. **ClientHello SessionID 32 byte 是 wire-format trick 典範**：找一個「**外觀正常但內容可控**」的 field 藏 protocol payload。G6 wire-format 設計 Part 11.5 必抄此思路。
2. **AAD = 整個 ClientHello.Raw**：handshake binding 防 attacker 篡改任何 byte。G6 必有此 binding。
3. **「**透明 fork 到真站**」是 fallback 終極形式**：比 Trojan 的 fallback to nginx 強——**不需要 server 自己跑 nginx**。G6 應有同樣機制。
4. **Server 端 trial decryption O(1)**：ShortId-based hash lookup。G6 多 user 設計直接抄。
5. **HMAC binding fake cert** 防 MITM redirect：G6 必有 cert validation hook。
6. **SpiderX 是 well-intentioned hack**：副作用（fingerprint）需要關注。G6 應思考更乾淨的「MITM 後行為」設計——可能方向：**直接返回真站 response 但拒絕後續 user data**（不主動 SpiderX）。
7. **PQ migration 已開始**：ML-DSA-65 部署是 2024+ 開始的工程實踐。G6 設計 from day one 考慮 PQ-ready。
...
flowchart TD
    ...
    REAL_PROTO -- 必抄 --> G_FIELD[G6: 找 wire 中合適 covert field<br/>藏 auth payload]
    REAL_PROTO -- 必抄 --> G_AAD[G6: AEAD AAD = entire handshake]
    REAL_PROTO -- 必抄 --> G_FORK[G6: server 透明 fork 到真站<br/>比 Trojan 強]
    REAL_PROTO -- 必抄 --> G_HMAC[G6: HMAC-binding fake cert<br/>防 MITM redirect]
    REAL_PROTO -. 改進 .-> G_SPIDER[G6: 設計更乾淨的「MITM 後行為」<br/>避 SpiderX 副作用]
    REAL_PROTO -. 內生 .-> G_PQ[G6: PQ-ready from day one]
...
4. **PQ migration 路徑**：ML-DSA-65 cert size 大顯著影響 fingerprint。是否能用 hybrid（X25519 + ML-DSA-65 partial）逐步遷移？這是 G6 的開放挑戰。
