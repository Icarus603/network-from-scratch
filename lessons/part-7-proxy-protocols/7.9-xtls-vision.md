...
1. **「冗餘加密」識別**：Vision 教我們**辨識並消除**「outer 已做的事 inner 不必再做」。G6 設計時必須**逐層分析每層的 security goal**，避免重疊。
2. **Splice 思路**：對 high-throughput 場景，**讓 kernel 做 zero-copy** 而不是 user-space crypto。G6 必支援 splice mode（特別對 QUIC connection 之間的 forwarding）。Part 11.6 / 11.12。
3. **Padding-then-splice 兩階段設計**：早期偽裝 + 後期 raw forward 是平衡 latency / detection 的好策略。G6 可借此設計：handshake + 握手後 N packet 走 obfuscation，之後切 raw QUIC payload。
4. **TLS record sniffing 是合理的中間層 awareness**：Vision 證明「**proxy 知道 inner protocol type 並適應**」是合理設計。G6 可考慮對 inner protocol 作 lightweight sniffing 啟用最佳模式（QUIC vs TCP+TLS vs HTTP）。
5. **CSCSC 是長期挑戰**：Vision 明確標註此為 unfixed。G6 需在 Part 10 traffic analysis 章節定量測 CSCSC，並在 Part 11 提出 mitigation（例如：fake background traffic、constant-rate padding）。
6. **「Spec 在程式碼裡」是雙刃劍**：Vision 沒有 RFC-style spec——靈活演化但**重新實作極困難**。G6 應寫**形式化 spec**（Part 11.10 ProVerif），即使實作迭代仍有 stable reference。
...
flowchart TD
    Vision[XTLS-Vision]
    Vision -- 必抄 --> G_SPLICE[G6: 對 QUIC inner application data<br/>用 zero-copy splice]
    Vision -- 必抄 --> G_PADDING[G6: handshake-phase padding<br/>抹平 inner 握手 length]
    Vision -- 必抄 --> G_SNIFF[G6: inner protocol type sniffing<br/>啟用最佳模式]
    Vision -. 改進 .-> G_SCSC[G6: Part 10 量測 CSCSC<br/>Part 11 設計 fake bg traffic]
    Vision -. 改進 .-> G_FORMAL[G6: 寫形式化 spec<br/>不只 in code]
