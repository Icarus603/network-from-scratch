...
**注意**：`2022-blake3-chacha20-poly1305` 對 UDP 改用 **XChaCha20-Poly1305**（24 B nonce）以避開 separate header trick。**為什麼 ChaCha 是 optional？** 因為現代 x86_64 與 ARMv8 都有 AES-NI / ARMv8 Crypto，AES-GCM 比 ChaCha 快 2-3×。**Proteus 設計參考**：在硬體加速無保證的場景（IoT），ChaCha 仍是首選。
...
兩個 context 推出來的 key 即使 PSK 相同也**永遠不同**。這就是「**deriving multiple keys from one secret**」的標準做法——Proteus 必抄。
...
**對 Proteus 的啟示**：obfuscation 不能是 plugin / 外掛——必須**內生**。SIP022 + shadow-tls 是兩個獨立 spec 拼起來，**部署複雜、版本對齊難**。Proteus wire format 第一版必須**從 byte 1 起就長得像 TLS 1.3 ClientHello / QUIC initial packet**——Part 11.5。
...
2. **BLAKE3 derive_key + context 領域分隔**：SS-2022 教科書級示範。Proteus 至少應該為 `c2s-data, s2c-data, c2s-auth, s2c-auth, mac-key, replay-key` 各 derive 獨立 sub-key。
3. **UDP session ID + packet ID**：QUIC + SIP022 + Hysteria2 收斂在這個設計。Proteus 必抄。
4. **WireGuard replay window**：anti-replay 的工程實作。Proteus 必抄。
5. **EIH 層疊 identity header**：多用戶單 port 的優雅解。Proteus 多用戶設計可參考——但要避開 ECB-XOR 那條坑（建議直接用 AES-GCM-SIV deterministic AEAD）。
6. **Padding-or-payload 規則**：低成本 anti-fingerprint trick。Proteus 內生支援。
7. **Response header 嵌入 request salt** = channel binding：簡潔有效，Proteus 整體 wire format 加 transcript hash 時可參考此最簡形式。
8. **Outer obfuscation 必須內生**：SIP022 + shadow-tls 兩 spec 拼貼是反面教材。Proteus wire format 第一個 byte 起就是偽裝。
...
```mermaid
flowchart TD
    SIP022[SIP022] -- 必抄 --> G_SESS[Proteus: session ID + packet ID UDP]
    SIP022 -- 必抄 --> G_REPLAY[Proteus: WireGuard sliding window]
    SIP022 -- 必抄 --> G_PSK[Proteus: 32B raw PSK, no password derive]
    SIP022 -- 必抄 --> G_KDF[Proteus: BLAKE3 derive_key + context separation]
    SIP022 -- 必抄 --> G_PAD[Proteus: padding-or-payload 強制]
    SIP022 -- 必抄 --> G_CB[Proteus: response header 嵌 request salt = channel binding]
    SIP022 -- 教訓 --> G_OBF[Proteus: outer obfuscation 必須內生<br/>不能 plugin]
    SIP022 -. 改進 .-> G_EIH[Proteus: 多用戶用 deterministic AEAD<br/>避開 ECB-XOR 坑]

    classDef ours fill:#fde,stroke:#c39
    class G_SESS,G_REPLAY,G_PSK,G_KDF,G_PAD,G_CB,G_OBF,G_EIH ours
```

**今日收窄項**：Proteus wire format 約 60% 已被 SIP022 教科書化——剩下 40% 是 outer obfuscation（Part 7.7/7.10/7.13 + Part 11.5）。
...
2. **SIP022 + shadow-tls 的「兩 spec 對齊」運營痛點**——版本升級需同步。學界研究方向：能否用單一 spec 同時提供 SIP022 等級的密碼學工程 **與** TLS 1.3 等級的偽裝？這正是 Proteus 設計目標。
