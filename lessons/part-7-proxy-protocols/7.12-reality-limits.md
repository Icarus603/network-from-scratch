...
研究級的協議理解必須涵蓋 **failure mode**——不是因為 REALITY 不好，而是**所有 censorship resistance 設計都有上限**。理解這些上限，才能在 Proteus 設計時不重蹈覆轍。
...
**對 Proteus**：必須 **explicitly const-time**——auth 驗證的時間 = 無 auth 與失敗 auth 的時間。Part 11.7 安全證明回頭。
...
### 10. 對 Proteus 設計的整合啟示

**REALITY + Vision 教給 Proteus 的「**最後一英里問題**」**：

| 問題 | REALITY+Vision 的應對 | Proteus 可改進方向 |
|---|---|---|
...
**核心 takeaway**：**REALITY 把 「handshake-level indistinguishability」 推到了極限**。**「Traffic-level indistinguishability」 是 next frontier**——**Proteus 必須在這個維度上做出新貢獻**才能 claim SOTA。**Part 11.6 主要 design challenge**。

---

## 與我們協議設計的關聯

1. **明確標註「**不防什麼**」**：REALITY 的 README 列了部分 limits，但社群討論才完整。**Proteus spec 必須詳列所有 known limits**——Part 11.1 spec template。
2. **Cover domain 的 operational guidance**：不只是「選一個白名單 domain」，要 **avoid 高風險 domain**（會被 SNI rewrite 的）。**Proteus 部署文件必有此 list**，並 quarterly 更新。
3. **單 protocol per VPS**：強制部署工具強制此——避免連坐。
4. **Const-time 驗證**：trial decryption 的 timing side-channel 要正面處理。
5. **流量整形 as first-class feature**：traffic volume / CSCSC / port-forwarding pattern 是 REALITY 未解問題——Proteus 必須 attack these head-on。Part 11.4 / 11.6 主要設計題。
6. **Multi-IP / multi-cover rotation**：避免 single-point detection。內建 client rotation logic。
7. **長期觀察社群討論**：REALITY 的 weakness 大多在 GitHub Issues / blog 中浮現，沒進 spec。**Proteus 必須有持續的 「known issues」 跟蹤機制**——可能用 GitHub Wiki 或 separate repo。
...
5. 同機共置弱協議的 IP 連坐：這是技術問題還是 operational 問題？Proteus 應該如何在 spec / 工具層面強制避免？
...
**Proteus 目標**：設計 protocol + 流量整形機制使 TVD < 0.01——**目前無公開生產級方案**。
...
flowchart TD
    ...
    REAL_LIMITS -- 必處理 --> G_TRAFFIC[Proteus: Traffic-level obfuscation<br/>內建流量整形]
    REAL_LIMITS -- 必處理 --> G_MULTI[Proteus: Multi-IP + multi-cover rotation]
    REAL_LIMITS -- 必處理 --> G_HYGIENE[Proteus: 部署工具強制<br/>單 protocol per IP]
    REAL_LIMITS -- 必處理 --> G_DOC[Proteus: Cover domain 風險 list<br/>quarterly update]
    REAL_LIMITS -- 必處理 --> G_CONST[Proteus: const-time auth verification]
    REAL_LIMITS -. 開放 .-> G_NEXT[Proteus: 第 5 代範式<br/>traffic-level indistinguishability]
