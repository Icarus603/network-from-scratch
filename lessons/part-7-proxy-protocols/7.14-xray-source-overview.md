...
2. **Production proxy 的 architecture pattern**：inbound/outbound/router 三段架構是業界共識——理解這個 pattern 才能設計 Proteus 的 reference implementation。
...
**這個三段是業界共識**——sing-box、mihomo、v2ray 同樣結構。**Proteus reference impl 必抄此架構**。
...
**對 Proteus**：routing 設計時應考慮 **trie-based 全 rule lookup**（O(1) per rule type）—— sing-box 部分採此設計。
...
1. **三段架構（inbound/router/outbound）必抄**：Proteus reference implementation 直接此模式。**不要重新發明**。
2. **Transport 與 protocol 解耦**：xray 的 `streamSettings` 把 outer transport 與 inner protocol 完全分離——Proteus 同樣設計。但 Proteus 的「**內生 obfuscation**」意味著 wire format 與 transport 緊耦合的部分需要明確界定。
3. **Buffer pool + multi-buffer**：production throughput 必選工程模式。Go 實作直接借 xray buf package。
4. **Routing engine** 是 **proxy 的另一個複雜度中心**：Proteus 不能跳過——必須有 production-grade routing。可選擇直接整合 sing-box/xray 的 routing 而非自寫。
5. **Fork 模式對 protocol 演化的影響**：xray vs v2ray fork 證明「**激進創新需要獨立 channel**」——Proteus 應預期 fork，提前設計 spec extension mechanism（addons-style）讓社群創新與主線並存。
6. **配置 → code 映射要透明**：xray 的字段名與 package name 一致是好設計——Proteus 的 spec 字段名與 reference impl 的 package/module name 應對齊。
...
6. 為什麼說「**Proteus 不能跳過 routing engine**」？只做 protocol 不做 routing 會在哪些 production 場景碰壁？
...
flowchart TD
    XR[Xray-core arch]
    XR -- 必抄 --> G_ARCH[Proteus: 三段架構]
    XR -- 必抄 --> G_BUF[Proteus: buffer pool + multi-buffer]
    XR -- 必抄 --> G_PLUG[Proteus: protocol/transport plugin model]
    XR -. 改進 .-> G_ROUTE[Proteus: trie-based routing<br/>O(1) per rule type]
    XR -. 改進 .-> G_FORK[Proteus: spec extension mechanism<br/>讓社群 fork 不破壞主線]
    XR -- 學 --> G_MAP[Proteus: 配置字段 ↔ module 名稱對齊]
