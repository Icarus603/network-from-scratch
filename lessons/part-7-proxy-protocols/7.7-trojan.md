...
1. **「Fail-indistinguishable」勝過「fail-uniform」**：Trojan 的 fallback redirect 是 active probing 的根本解。Proteus 必有此機制——server 在 auth 失敗時 **真的轉發到一個 cover service**（不是模仿 cover service）。
2. **Cover site = 一級設計考量**：Proteus 不能只給 spec，必給「**部署運維 best practice**」（cover site 選擇、cert 來源、訪問日誌生成）。Part 11.13 deployment doc。
3. **TLS-in-TLS 必修**：靠外層 TLS 已不夠（2024 後）。Proteus 必對 inner TLS 作特殊處理——**XTLS-Vision splice 思路必抄**。Part 7.9 + Part 11.5。
4. **REALITY-like 設計**：直接借用真網站證書是長期方向。Part 7.10–7.12 + Part 11.6 設計重點。
5. **應用層 anti-replay 仍應加**：Trojan 完全依賴 TLS replay protection 是冒險。Proteus 至少要 timestamp + window，即使「冗餘」。
6. **Spec 簡單性**：Trojan 半頁 spec 是工程典範。**Proteus wire format core spec 不應超過 5 頁**——複雜性必須有強理由。
...
flowchart TD
    ...
    Trojan -- 必繼承 --> G_FALL[Proteus: server 必有真 cover service<br/>不是 fail-fast]
    Trojan -- 必避開 --> G_TLSINTLS[Proteus: 解決 TLS-in-TLS pattern<br/>抄 XTLS-Vision splice]
    Trojan -- 必加 --> G_REPLAY[Proteus: 應用層 timestamp + replay window]
    Trojan -- 必繼承 --> G_RUNBOOK[Proteus: 部署 runbook 是 spec 的一級公民<br/>cover site / cert / log]
    Trojan -. 升級 .-> G_REALITY[Proteus: 借真網站 cert 路線<br/>抄 REALITY]
