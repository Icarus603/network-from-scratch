...
本堂專注 **(1)**：建立 REALITY 之所以存在的 **威脅模型**——理解這個模型才能在 Part 11 設計 Proteus 時做出正確 trade-off。
...
對 Proteus：理解這個演化序列，**每一代都是對前代的根本反應**——Proteus 必須思考「下一代範式是什麼」，而不是「在第 4 代基礎上做小改進」。**Part 11.6 設計目標**。

---

## 與我們協議設計的關聯

1. **Threat model 必須先寫 spec 第一頁**：REALITY 之所以成功，是因為它**精確定義了威脅模型 + 精確定義了不防什麼**。Proteus spec 必先寫此。
2. **「Borrow real」是 SOTA 路線**：Proteus 第一版必繼承此思路——可選借用真網站 cert 或新方法。Part 11.6 設計重點。
3. **Handshake-level vs traffic-level 分離設計**：REALITY 教我們**承認 handshake 與 traffic 是兩個不同問題**——分別解。Proteus 同樣分離設計。
4. **Trust assumption 明確化**：REALITY 的 X25519 公鑰帶外分發假設是合理的。Proteus 也應明確說「我們假設 user 通過 X 渠道拿 secret」。
5. **「不防什麼」與「防什麼」同樣重要**：spec 應該明確列出 known limits，避免 user 誤用。
6. **下一代範式思考**：Proteus 不該停在 REALITY 範式內小改進——應該思考「**第 5 代範式是什麼**」。可能方向：multi-cover（同時偽裝多個 protocol）、QUIC-native handshake hijack（Part 11.6 開放問題）。
...
flowchart TD
    ...
    REAL_THM -- 必抄 --> G_THM[Proteus: spec 第一頁<br/>明確威脅模型]
    REAL_THM -- 必抄 --> G_BORROW[Proteus: borrow-real cert/handshake<br/>路線繼承]
    REAL_THM -- 必抄 --> G_TRUST[Proteus: 明確 X25519 公鑰帶外分發 trust]
    REAL_THM -- 必抄 --> G_LIMITS[Proteus: 明確「不防什麼」<br/>避免 user 誤用]
    REAL_THM -. 升級 .-> G_5GEN[Proteus: 第 5 代範式<br/>traffic-level + handshake-level 同時解]
...
1. **Traffic-level indistinguishability**：能否設計一個 protocol，**handshake + traffic 都不可區分** from real connection？這是 Proteus 的核心目標，但學界尚無公認答案。
...
4. **第 5 代範式**：可能方向 (a) multi-cover (同時偽裝多個 protocol)；(b) QUIC-native handshake hijack；(c) E2EE proxy (server 完全不知 user identity)；(d) mixnet integration。**Proteus 設計需明確選擇方向**，Part 11.6 主場。
