...
**設計優雅**：addons 是 forward-extensible 的——XTLS-Vision、Vision UDP、未來新 flow（如 Proteus 自訂）都通過 addons 表達，**不需要動 wire format core**。
...
1. **「inner protocol = 純 framing + auth」是正確分工**：當 outer 強保證 TLS-grade 安全，inner 不需要重複加密。**Proteus wire-format core 採此設計**——inner 只 framing + routing meta + auth，outer 做 confidentiality/obfuscation。
2. **0-RTT 是必選**：VLESS、SS-2022、Trojan 都做到 0-RTT。Proteus 必跟。Part 11.5。
3. **Stateless server 的部署優勢**：無時鐘要求大幅降低 ops 痛點。Proteus 採 stateless（仍可選用 timestamp anti-replay 但不依賴它）。
4. **UUID 在 outer 內可明文**——但 Proteus 應走 PSK 而非 UUID（PSK 32 byte random，更明確「這是 secret」）。
5. **Addons 機制設計典範**：ProtoBuf 可變長 field 帶 forward extensibility——Proteus 也應有此。
6. **不要過早 spec 加 feature**：VLESS reserve `encryption` 但不啟用是健康設計。**Proteus spec v1 應該極簡**，未來通過 addons 擴展。
7. **不靠 Mux**：multiplex 帶來的 long-lived connection 是 traffic analysis 弱點。Proteus 用 QUIC 內生 multiplex（per-stream isolated），不額外加 application Mux。
...
flowchart TD
    ...
    VLESS -- 必抄 --> G_F[Proteus: inner = pure framing<br/>無 inner crypto]
    VLESS -- 必抄 --> G_S[Proteus: stateless server<br/>無 clock 依賴]
    VLESS -- 必抄 --> G_AD[Proteus: addons-style extensibility<br/>不動 core spec]
    VLESS -- 必抄 --> G_0[Proteus: 0-RTT payload]
    VLESS -. 升級 .-> G_PSK[Proteus: 32 B PSK 取代 UUID<br/>明確「這是 secret」]
    VLESS -. 加 .-> G_CB[Proteus: outer-binding<br/>避免 outer TLS 被偷時 inner 完全裸]
