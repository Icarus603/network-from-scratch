...
**對 Proteus 啟示**：「**真實 base rate 大的 protocol**」是 censorship resistance 的天然優勢——但 Proteus 不是 OpenVPN（且 OpenVPN 已有指紋問題）。
...
1. **Naïve 教我們：「**用真 stack**」是 fingerprint 終極解**——但**部署成本太高**。Proteus 的 utls 模仿是 trade-off。
2. **Snell 教我們：**閉源協議的長期不可持續**——Proteus 必開源 + open spec。
3. **Brook 教我們：**「**多 wire format**」是 anti-pattern**——Proteus 應該**單一 wire format + 多種 cover**，而非多種 wire format。
4. **ShadowTLS 教我們：**outer obfuscation 是真 spec 工程**——v1→v2→v3 演化展示「**partial mimicry → full mimicry**」的迭代。Proteus outer obfuscation 直接從 v3 思路起步。
5. **OpenVPN 教我們：**「**真實 base rate**」是天然優勢**——Proteus 設計時應思考「能否與某個現有大流量 protocol 共享 base rate」。
6. **Outline 教我們：**「**Google 級工程也救不了 wire-format 過時的協議**」**——工程品質不能彌補設計缺陷。Proteus 必須**設計**正確。
...
6. 為什麼說「Brook 的多 wire format 是 anti-pattern」？Proteus 該怎麼避免同樣錯誤？
...
flowchart TD
    ...
    LongTail -- 學 --> G_NAIVE[Proteus 借鑒：可選的「**Naïve mode**」<br/>用真 Chromium stack 做 fingerprint 強防禦]
    LongTail -- 學 --> G_OPEN[Proteus 必須：開源 + open spec]
    LongTail -- 學 --> G_SINGLE[Proteus 必須：單一 wire format + 多 cover<br/>（避 Brook anti-pattern）]
    LongTail -- 學 --> G_BR[Proteus 設計：思考「base rate」<br/>傾向用主流 carrier]
    LongTail -- 學 --> G_REGION[Proteus 必須：避免 region-specific 設計<br/>scale globally]
...
3. **多協議堆疊（Brook）vs 單協議多 cover（Proteus）**——哪個更難被 wholesale 封？需 measurement。
4. **AmneziaWG 對 Russia 有效但對 China 失效** —— 兩國 censorship 的 detector 差異具體在哪？
5. **OpenVPN-over-443 的「灰色地帶」效應**——能否設計 Proteus 進入相似灰色地帶（被部分 detect 但不被 wholesale block）？trade-off 是 user 不確定性。
