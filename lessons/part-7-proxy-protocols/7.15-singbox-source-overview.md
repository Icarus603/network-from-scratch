...
- 為什麼 sing-box 是「**Proteus reference impl 的更好起點**」？
...
**對 Proteus**：sing-box 模式更好——**protocol 邏輯獨立 module，可測試、可重用、可獨立 ship**。
...
**對 Proteus**：TUN mode 不是 protocol layer 的事，但 **Proteus client SDK 必須支援**—— 否則 mobile/desktop 無法整合。直接用 sing-tun 是務實選擇。
...
**對 Proteus**：routing 直接整合 sing-box（或抄其架構）。**自寫 routing engine 是浪費精力**。
...
**對 Proteus**：直接抄 sing-box config 結構——**user 從 xray/sing-box 遷移成本低**。
...
**對 Proteus**：reference client 應同樣 cross-platform。但**自寫 mobile UI 工程量極大**——務實做法：**Proteus 兼容 sing-box JSON config**，user 直接用 SFA/SFI/SFM 即可。
...
**Proteus spec governance 應 from day one 設計**：spec 維護不依賴單個 maintainer，**至少 3-5 個非中國 jurisdiction maintainer**。

---

## 與我們協議設計的關聯

1. **sing-* library 體系是 Proteus reference impl 的更好模式**：每個 protocol 獨立 module，core 只做 wiring。
2. **TUN mode 整合**：Proteus client SDK 必須支援，但**直接用 sing-tun 而非自寫**。
3. **Routing 直接抄 sing-box rule-set + trie**：自寫 routing 是浪費精力。
4. **Cross-platform client 策略**：**不自寫**，依賴 sing-box JSON config 兼容性。
5. **Modular Go 最佳實踐**：generic、context、interface composition——Proteus reference impl 直接抄此風格。
6. **Project governance multi-jurisdictional**：學 sing-box 流亡教訓，from day one 設計。
7. **Spec versioning**：sing-box 有清楚的 version policy，Proteus 必抄。
...
5. 為什麼說 sing-box 是「**Proteus reference impl 的更好起點**」？至少給 3 個技術理由。
6. nekohasekai 流亡事件對 OSS project governance 的教訓——Proteus 該如何 from day one 設計避免單點風險？
...
flowchart TD
    SBOX[sing-box arch]
    SBOX -- 必抄 --> G_MOD[Proteus: modular library 體系<br/>proteus/core, proteus/wire, proteus/transport, proteus/route]
    SBOX -- 必抄 --> G_THIN[Proteus: 薄 wiring inbound/outbound]
    SBOX -- 必抄 --> G_TUN[Proteus: 借 sing-tun 整合<br/>不自寫]
    SBOX -- 必抄 --> G_RS[Proteus: rule-set 預編譯 + trie]
    SBOX -- 必抄 --> G_GOV[Proteus: multi-jurisdictional governance]
    SBOX -- 學 --> G_SCHEMA[Proteus: config schema 驗證]
