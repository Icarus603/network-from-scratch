...
**這就是「狀態化 PRG anti-replay」反面教材**——SS-2022 與 Proteus 都改成 stateless replay window + timestamp。
...
2. **stateless single-AuthID > stateful multi-ID**：alterID 是死路。Proteus 採 stateless（timestamp + rand + CRC）的 SIP022 / VMessAEAD 路線。
3. **避開 multi-user trial decryption**：VMess 與 outline-ss-server 都是 O(N)；Proteus 採 SIP022 EIH 的 hash-table O(1) routing。
4. **不允許「none」cipher**：信任 outer 的設計是反面——outer fail（TLS proxy 解密、CDN tap）就裸奔。Proteus 永遠 inner AEAD。但**性能優化可借**：Part 7.9 XTLS-Vision 的 splice 是聰明解。
5. **Domain separation 用 derive_key/HKDF**：VMess 的 SHA256 truncate 是符號意義差的設計。Proteus 必用 BLAKE3 derive_key context。
6. **UUID 不是 PSK**：身份與 secret 必分離。User identifier 公開、PSK 32 byte random 私有。
7. **內生 obfuscation**：VMess 完全依賴 outer transport——「**stream settings 拼貼**」是 V2Ray 的長處也是脆弱性。Proteus wire format 第一個 byte 起就是偽裝。
...
```mermaid
flowchart TD
    VMess_legacy[VMess legacy] -- 教訓 --> G_AEAD[Proteus: header 必 AEAD]
    VMess_legacy -- 教訓 --> G_FAIL[Proteus: fail-uniform 設計]
    VMess_AEAD[VMessAEAD] -- 教訓 --> G_OBF[Proteus: 內生 obfuscation, 不靠 outer]
    alterID[alterID 反例] -- 教訓 --> G_STATELESS[Proteus: stateless replay protection]
    NoneZero[None/Zero cipher 反例] -- 教訓 --> G_AEAD2[Proteus: inner AEAD 永不關]
    UUID_PSK[UUID = ID + PSK 混淆] -- 教訓 --> G_SEP[Proteus: identity vs secret 分離]
...
1. **能否設計一個 protocol 不依賴 outer transport，靠自己 wire format 通過 FEP detector？**——這是 Proteus 的核心目標。VMess 證明「乾淨的 wire format + outer」是現有路線，但「**單層協議同時提供密碼學 + 偽裝**」尚無生產級方案。Part 11.5 設計目標。
