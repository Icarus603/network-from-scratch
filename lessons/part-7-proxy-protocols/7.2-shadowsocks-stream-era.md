...
1. **絕不用 password-based KDF**：SS 第一代的 `EVP_BytesToKey` 是反面教材。G6 必用 32B/64B 隨機 PSK 或公鑰學派直接給 key，password 只用於 user 端 wallet 解鎖——絕不直接 derive crypto key。
...
4. **必須有 anti-replay**：SS 第一代裸奔的教訓。G6 至少要：(a) per-PSK monotonic counter / nonce；(b) bloom filter 或 cuckoo filter for 過期防禦；(c) 主動探測時 server 行為與「真實 web service」一致——例如 fallback 到真網頁（Trojan / REALITY 的策略，Part 7.7 / 7.10）。
5. **威脅模型先寫**：G6 設計第一頁文件就要列威脅模型，不能等實作完才補。Part 11.1 第一份文件規範。
...
```mermaid
flowchart TD
    A1[反例 1<br/>password-derived key] --> G1[G6: 強制 32B PSK<br/>or KEM-derived]
    A2[反例 2<br/>無 MAC stream cipher] --> G2[G6: AEAD only<br/>ChaCha20-Poly1305 / AES-256-GCM]
    A3[反例 3<br/>第一 byte 高 entropy] --> G3[G6: 偽裝層<br/>TLS-like / QUIC-like / HTTP-like]
    A4[反例 4<br/>無 anti-replay] --> G4[G6: BLOOM filter + nonce + monotonic]
    A5[反例 5<br/>無 threat model] --> G5[G6: spec 第一頁<br/>列 Dolev-Yao + active prober + replay]
