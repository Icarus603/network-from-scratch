...
1. **AEAD 是必要不充分**：SIP004 證明「密碼學正確 ≠ 抗審查」。G6 必用 AEAD（沒得商量），但**不能止於此**。我們的 wire format 必須讓「前 N byte 看起來像合法 protocol」——TLS、HTTP/3、QUIC 三選一。
2. **chunked AEAD record 的「先 AEAD-encrypt length」是必抄設計**：TLS 1.3、SS-AEAD、SS-2022、Hysteria 都這樣做——G6 直接抄。Part 11.4。
3. **不要再依賴 password derive 主 key**：master_key 鏈條全砍掉。User 端要用 password 加密 PSK 的話，那是 client-side wallet 問題，不是協議問題。
4. **Anti-replay 不要靠 bloom filter**：SS-AEAD 補丁式設計告訴我們 bloom 是 hack。G6 採 monotonic counter（per-PSK 或 per-session）+ 短時間 cuckoo filter（24h）作 sliding-window 防護。
5. **HKDF info 字串要明確**：SS 的 `b"ss-subkey"` 是常數——好。G6 應該把 protocol version + role（client/server）寫進 info：`b"g6/v1/c2s-data"`，避免不同 context 的 key 互相 collision。
6. **不依賴外部 plugin 做偽裝**：G6 必須**內生**偽裝層。Part 11.5 wire format 把偽裝作為第一公民 design。
...
```mermaid
flowchart TD
    SS_AEAD[SIP004 SS-AEAD] -- 教訓 1 --> G_AEAD[G6: 強制 AEAD]
    SS_AEAD -- 教訓 2 --> G_KDF[G6: salt + HKDF + sub_key 明確 info]
    SS_AEAD -- 教訓 3 --> G_FRAME[G6: chunked record + AEAD length 必抄]
    SS_AEAD -. 反面 .-> G_FAKE[G6: 必須有偽裝層<br/>不能裸 AEAD]
    SS_AEAD -. 反面 .-> G_REPLAY[G6: anti-replay<br/>monotonic counter + cuckoo<br/>不靠 bloom hack]
    SS_AEAD -. 反面 .-> G_PSK[G6: 不從 password derive<br/>強制 32B PSK or KEM]

    classDef ours fill:#fde,stroke:#c39
    class G_AEAD,G_KDF,G_FRAME,G_FAKE,G_REPLAY,G_PSK ours
```

收窄項：G6 wire format 第一輪設計**直接從 SS-AEAD chunked AEAD record + 偽裝外殼**起步，Part 11.4 詳講。
