...
更重要的研究價值：**WireGuard 的 Go 實作是極好的「乾淨 protocol code 範本」**。10000 行 Go，無依賴黑魔法、無 panic-prone 設計、所有 critical path 都用 sync primitive 嚴格保護。這對我們 Proteus 的 reference implementation 是無價對照。
...
### 11. 對 Proteus 的 implementation 啟示

| WireGuard 原始碼設計 | Proteus 該不該抄 |
|---|---|
...
| 固定 message size constants | ⚠️ 改：Proteus 用 length-prefixed + padding，仍須 deterministic 但變化 |
| Static identity 在 device 級單一 | ⚠️ 改：Proteus 可能 multi-identity / per-session rotation |
...
2. **預測自己寫 Proteus reference impl 時的 module layout**——它應該長得很像 `device/noise-protocol.go + noise-helpers.go + cookie.go`。
...
[Part 11.7 Proteus reference impl](../part-11-design/) 設計階段，這份 source walk 是核心參考。
...
4. `RoutineHandshake` 為什麼用 worker pool 而非 single goroutine？這對 Proteus 的影響？
...
Proteus 的 reference impl module layout 應該是：
