# 課堂 6.10 — WireGuard 給我們的啟示：Part 6 綜合與 Proteus 設計清單
...
本堂把這些拆成**Proteus（我們的新 SOTA 翻牆協議）設計清單**——這是進入 Part 7~9 之前的座標系。Part 7（翻牆協議演化史）會把這份清單與 SS/VMess/Trojan/VLESS/REALITY/Hysteria 的設計選擇對照；Part 11（spec 設計）會直接用這份清單作為起點。
...
| 維度 | IPsec | OpenVPN | WireGuard | **Proteus 目標** |
|---|---|---|---|---|
| 設計年代 | 1995-2014 | 2001-持續 | 2017-持續 | 2026-持續 |
| 規格複雜度 (LoC equiv) | ~150 RFC, 數百選項 | 自定 + TLS | 1 spec + Tamarin proof | 1 spec + ProVerif + Tamarin |
| Reference impl LoC | strongSwan ~400k | OpenVPN ~120k | WG-go ~10k / kernel ~4k | **Proteus-go ≤ 15k, Proteus-rs ≤ 12k** |
...
1. **Hard-coded ciphersuite**：`Noise-Proteus_X25519+MLKEM768_ChaCha20Poly1305_BLAKE2s`（或同等）；無 negotiation。
...
**創新點**：outer identity 可以是任意 fronting domain（REALITY 思想擴展），inner identity 真正驗證 peer。**這是 Proteus 對「server identity 必然洩漏」假設的核心解法**。

#### Abstraction 2: 「Three Sizes」

封包 size 在 Proteus 規格層有三層：
```
...
### 5. Proteus spec structure（建議大綱，後續 Part 11 演進）

```
Proteus Specification v0.1 (draft)
...
§6 Handshake Protocol
   ...
   §6.2 Inner Noise-Proteus (5 DH + 1 KEM)
...
flowchart TD
    ...
    p10["Part 10: 對抗式流量分析"]
    p11["Part 11: Proteus spec 設計"]
    p12["Part 12: Proteus 實作 + 評測 + 論文"]
...
到 Part 12 結束，你應該能：
- 寫出 Proteus spec v1.0（30+ pages, RFC-style）
- 寫出 proteus-go reference impl（10000~15000 LoC Go）
...
直接行動項目（給後續 Part 用）：
- 把本堂的 30 條 Proteus 設計清單 commit 到 `notes/proteus-design-checklist.md`，後續 Part 11 寫 spec 時逐項確認。
- 把「絕對不做」清單 commit 到 `notes/proteus-antipatterns.md`。
...
這是 Proteus paper introduction 的雛形。Part 12.20 寫 paper draft 時直接拿來改。

---

## 自我檢查

1. 不看本堂，列出 Proteus spec 的 Priority 1（10 條 hard constraints）。能列出 ≥ 7 條算過關。
2. IPsec / OpenVPN / WireGuard 三條路線各為 Proteus 提供 1 條最重要的設計決策。哪三條？
3. 「Identity Triple」、「Three Sizes」、「Threat Layered」三個 abstraction 各自要解決什麼問題？哪個是 Part 6 完全沒提供解法的（要等後續 Part）？
4. 為什麼 Proteus 必須 day-1 寫 CryptoVerif spec？對應 [3.15] 與 [5.7] 的哪些觀點？
5. Part 6 給 Proteus 的「不夠」是 4 條。對應 Part 7-10 各補哪一條？
6. 你目前對 Proteus spec 已能寫出多少 §（從 §1-§15）？哪些 § 需要等後續 Part 才能寫？
...
- **Identity Triple / Three Sizes / Threat Layered**：本堂提出的 3 個 Proteus 設計抽象（**研究階段命名，可能在 Part 11 重新命名**）。
...
Proteus 要對抗的對手等級總結（Part 6 + 預讀 Part 9）：
...
**Proteus 必須對 L1-L7 全部 robust**。WireGuard 對 L1-L2 robust，L3-L7 各有缺陷。

### 3. 形式化定義

Proteus 的 ideal security goal（informal）：

> For all adversary A in classes {L1, ..., L7}, A's advantage in distinguishing a Proteus session from {ideal encryption + ideal MAC + ideal cover protocol} is computationally negligible.
...
```
Proteus = (WireGuard's spec discipline)
...
**沒有任何單一現有 protocol 滿足全部**。這是 Proteus 的研究價值——**整合既有最佳實踐 + 補足 anti-censorship/PQ 的 spec gap**。
...
**Part 6 終結語**：你已具備閱讀任何 WireGuard 衍生協議 source code 的能力，理解了「真 VPN」三條路線的全部歷史教訓，並有了 Proteus 設計清單的雛形。**Part 7 開始**，我們進入翻牆協議的家譜——從 SOCKS5 到 REALITY，看人類如何在 GFW 壓力下迭代出 12 個世代的協議。
