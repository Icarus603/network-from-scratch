...
對研究 G6 的目標來說，OpenVPN 是極好的教材：它示範了「**用 TLS 當 KE + 自製 record layer**」這條路為什麼**理論上聰明、實踐上致命**。WireGuard 用 Noise IK 而不是 TLS，VLESS 用 TLS+REALITY 而不是 OpenVPN 那種「裸 TLS-on-UDP」，都跟 OpenVPN 的教訓直接相關。
...
**研究級觀察**：tls-crypt 系列**沒有加密 opcode**，這是 design 層面的疏忽（為了讓接收端能判斷封包類型再嘗試解密）。但這正是 Xue et al. 找到的 byte-pattern fingerprint 的根源。如果重新設計，opcode 必須在 AEAD 內。**這是 G6 day-1 規則**：所有「協議識別資訊」都在 AEAD 之內。
...
> **跟我們協議設計的關聯**：Xue 2022 的方法論對 G6 有兩個 implication：
> 1. **首封 packet 不能有 protocol-identifying byte**。WireGuard 第一 byte = 0x01（type=handshake_init），其實**也犯了同樣的錯**——只是 GFW 暫時不認真打。[6.7](6.7-wireguard-blocked-china.md) 詳講。我們 G6 必須讓第一 byte 看起來 entropy-uniform（這帶到 [Part 9.5 fully encrypted detection](../part-9-gfw-research/)）。
> 2. **必須對抗 active probing**。被 probe 時 server 不能回應「我認識你」。VMess 1.0 的「服務端強迫繼續回應」就是 IMC 2020 *How China Detects and Blocks Shadowsocks* 的攻擊入口。G6 必須像 REALITY 一樣 "probe-resistant by design"。
...
| 配置選項百餘個 | **G6 用 single ciphersuite，no negotiation** |
...
3. Xue 2022 的 two-phase framework（passive + active）對我們設計 G6 的具體意義是什麼？
...
6. 為什麼 OpenVPN 不能「直接把 opcode 放進 AEAD」？這個 backwards compatibility 問題對 G6 設計給了什麼警示（**day-1 寫對，沒有 fix 機會**）？
...
G6 在 OpenVPN 教訓下收窄的設計：

| 維度 | OpenVPN | WireGuard | **G6 目標** |
|---|---|---|---|
