...
但 WireGuard 也有它的盲區，最大的一塊就是**抗審查**：spec 設計時完全沒考慮 GFW 場景，這留給我們 G6 來補。所以本堂的目的是：**精讀 whitepaper，理解每個設計決策的理由，然後在 [6.7] 開始指出哪些決策對抗審查不利**。
...
| 目標 | 對應設計 | 對 G6 的啟示 |
|---|---|---|
| **Simple to use, manage, deploy** | 配置只剩 (peer pk, AllowedIPs, endpoint) | G6 也要追求 minimal config |
| **Strong, modern cryptography** | hard-coded Curve25519/ChaCha20-Poly1305/BLAKE2s/HKDF | G6 同樣 hard-coded，但要 PQ-hybrid |
| **Minimal attack surface** | 4000 LoC vs IPsec ~400k | G6 要 < 10k LoC for core |
| **High performance** | kernel impl + zero copy + AEAD inline | G6 要 user-space 但 io_uring/XDP 加速 |
| **Well-defined and thoroughly considered** | Tamarin/ProVerif 驗證 | G6 day-1 寫 ProVerif 模型 |
| **Stealthy** ❓ | 對 passive observer "looks like noise" — **但對 active prober 完全暴露** | G6 不能止步於此 |
| **No bloat** | 拒絕 negotiation / extensibility / multi-cipher | G6 同樣，但要為 PQ migration 留空間 |

> **研究級觀察**：第 6 條 "Stealthy" 是 WireGuard 後來最大的 weakness。Donenfeld 的定義是「passive observer 看不出來這是 WireGuard」，但他**完全沒考慮 active prober**。Xue 2022 之後的 GFW 研究告訴我們：對 nation-state，"passive stealth" 完全不夠。G6 必須重新定義 stealth = passive + active probe resistance。
...
**未來怎麼換**：v2 protocol，完全不向後相容，全網要協調升級。對小規模 self-hosted 是可接受；對 mass deployment 是難題。**這正是 G6 必須解決的問題**——我們要設計能 PQ-migration 又不開放 negotiation 攻擊面的機制。
...
WireGuard whitepaper 給 G6 的設計清單：

**「直接抄」清單**（這些 day-1 寫對，G6 沒理由重發明）：
...
**必須升級 / 替換**（[Part 11 G6 設計] 詳述）：
1. ⚠️ Noise IK 的 first byte 0x01 = 固定 → **G6 要 entropy-uniform 首封**。
2. ⚠️ Handshake message size 固定 148/92 bytes → **G6 要 randomized padding**。
3. ⚠️ Server static pk 是某種 secret-but-leaks → **G6 要 ephemeral server identity 或 REALITY-style 借殼**。
4. ⚠️ X25519 only → **G6 要 hybrid X25519 + ML-KEM-768**（[3.11 PQC](../part-3-cryptography/3.11-post-quantum.md)）。
5. ⚠️ No probe resistance design → **G6 day-1 加入**。
6. ⚠️ No traffic analysis resistance（packet size leak）→ **G6 要 padding policy + cover traffic**。

**WireGuard 永遠不會做的事（G6 也不一定要做）**：
- ❌ Multi-cipher negotiation。
- ❌ 動態 routing（BGP/OSPF on top）。
- ❌ Cert/PKI 整合。
- ❌ 為 corporate deployment 加 RADIUS / LDAP。

這些都是「保持精簡」的代價，**WireGuard 寧可被罵也不開後門**。G6 同樣應該。
...
6. Dowling-Paterson 2018 ACNS 提到的 "modular proof barrier"（第一封 data packet 兼任 key confirmation）對 G6 的 implications？
...
G6 在 WireGuard 教訓下的收窄：

| 維度 | WireGuard 選擇 | G6 採用？ |
|---|---|---|
| Negotiation | 無 | **採用** |
