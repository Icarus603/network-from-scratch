...
2. **理解 kernel 模組的設計約束**——若未來 G6 想 upstream，必須遵守。
...
對 anti-replay 與 rekey 邏輯，這個精確度差**幾乎沒影響**（WG timer 都是秒級）。但對未來 G6 加 **cover traffic timing** 是重要——cover traffic 需要 jitter 控制在 sub-ms，kernel timer 更友善。
...
### 13. 為什麼 G6 reference 不從 kernel module 開始

雖然 kernel impl 快，G6 reference 應走 user-space-first：

1. **快速迭代**：spec 仍在演進，user-space 改一行 = recompile 5 秒；kernel module 改一行 = 重啟 kernel module 30 秒（或 reboot）。
2. **跨平台**：G6 必須跑 macOS / Windows / iOS，這些平台沒 Linux kernel module 可言。
...
**長期路線**：spec 穩定 + Linux PQ ecosystem 成熟後，再 port G6 到 kernel。

---

## 與我們協議設計的關聯

### 從 kernel WG 學到的 G6 設計守則

1. **dst_cache 設計**：endpoint 路由緩存應 per-CPU 或 lock-free——G6 user-space 用 `sync.Map` 或 atomic pointer。
2. **per-CPU worker affinity**：對 throughput-bound 場景必須的優化。Go 可用 `runtime.LockOSThread` + `GOMAXPROCS` 控制。
3. **RCU 思想**：allowedips 是讀為主，G6 也應採類似 lock-free 讀。Go 的 `atomic.Pointer` + COW 是等價。
...
5. Zinc 為什麼被部分廢棄？對 G6 想 upstream kernel 的影響是什麼？
...
G6 的 kernel port 路線（**post-Phase III**）：

```
spec 穩定 → 寫 g6-go reference → 多 impl 累積 → 形式化驗證 + audit
                                    ↓
                              spec 凍結至少 1 年
                                    ↓
...
**WireGuard 從 2017 提案到 2020 mainline 花 3 年。G6 同樣會。**
...
4. **G6 是否可走 eBPF-only 路線**？即「**user-space 寫 eBPF program，attach 到 XDP**」實作 protocol。這是 [2.7 XDP](../part-2-high-perf-io/) 已伏筆的方向，需要對 verifier 限制做深入分析。

---

**下一堂**：[6.10 WireGuard 給我們的啟示](6.10-wireguard-lessons-for-our-protocol.md) — Part 6 的總結與綜合，把 6.1~6.9 所有教訓匯成 G6 設計清單。
