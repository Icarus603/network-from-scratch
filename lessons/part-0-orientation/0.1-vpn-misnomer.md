...
```
            ...
            ┌────────────────────────────┼────────────────────────────┐
            │                            │                            │
       線 A：真 VPN                 線 B：商業 VPN              線 C：翻牆代理
       （L3 通道）                  （= 線 A 加包裝）           （L4/L7 代理）
            │                            │                            │
   ┌────────┼────────┐                   │              ┌─────────────┼─────────────┐
   │        │        │                   │              │             │             │
WireGuard OpenVPN IPsec/IKEv2     NordVPN  ExpressVPN  SOCKS5/HTTP  SS / SS-2022  VMess
   │                                                                  │
   │                                                              Trojan
   │                                                                  │
   │                                                          VLESS + REALITY
   │                                                                  │
   │                                                          Hysteria / TUIC（QUIC 系）
   │
   └─→ 也可以被改造成翻牆工具，但容易被識別封鎖
       （所以中國用戶用 WireGuard 翻牆會被打）
```
...
線 C 翻牆代理的威脅模型：
    ...
    對手能做：
        ...
        - 主動探測你的伺服器、看回應像不像代理
        - 阻斷可疑連線、把你 IP 拉黑
        - 用流量模式分析（封包大小、時序）猜你是不是代理
    防禦目標：機密性 + 完整性 + 不可識別性 + 抗主動探測
```
...
| **G6** ??? | **我們要做的** | （Part 9~10 列） | （Part 11 設計） |

**這也定位了我們研究目標的位置**：要做 G6，**同時繼承 G4 的抗識別深度與 G5 的速度優勢**——歷史上沒有協議同時拿下這兩件事，這就是我們研究的 contribution claim。
