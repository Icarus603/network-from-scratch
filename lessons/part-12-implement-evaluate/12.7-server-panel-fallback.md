...
| stack          | --eBPF--> | [G6 worker socks]  |
|                |   prog    | [cover sock]       |
+----------------+           +--------------------+
```

實作：
1. G6 server 與 cover server (Caddy) **同時 bind UDP/443 with SO_REUSEPORT**。
...
4. 通過 → return G6 worker sock index；失敗 → return cover sock index。
...
**效益**：cover-bound packet 從未進入 G6 user-space → forward p99 < 200 µs（限速來自 `__inet_lookup` syscall path + kernel TCP stack）。
...
只在 G6 server 是高負載專用機（10+ Gbps）時值得。

1. NIC 啟用 AF_XDP，所有 UDP/443 packet 進 user-space ring buffer。
2. G6 server 在 user-space 自己解 QUIC + auth。
...
**為何不用 long-lived 單一 TCP connection 多 user**：cover server (Caddy) 同 connection 多 request 用 H/2 / H/3 mux，但前面已有 client TCP/TLS handshake bytes → 必須整條 connection forward 給 cover。所以**每個 incoming G6-fail connection 對應一個 pool TCP conn**。
...
```bash
# 1) probe G6 server with random ClientHello (will fail HMAC, forward path)
for i in $(seq 1 10000); do
    /usr/bin/time -f "%e" curl --http3 --resolve probe-${i}.example.com:443:${G6_IP} \
...
python -c "
import numpy as np
g6 = np.loadtxt('probe-times.log')
co = np.loadtxt('cover-times.log')
print(f'G6 forward p50/p95/p99 inflation = {np.percentile(g6-co, [50,95,99])} ms')
"
```
