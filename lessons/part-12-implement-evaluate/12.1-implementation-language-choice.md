...
**結論先講**：對 G6 級別 SOTA proxy，我們的建議結構是 **「Rust core + Go integration shim」**（rationale 在第 6 節）。但結論的價值 ≪ 推導過程，因為當你的 constraint 變了（例如，捨棄 Linux-only，往 Windows datapath 推），結論可能要重來。
...
- **G6 (indistinguishability)**: 整形演算法 worst-case timing 控制 → Rust + 無 GC 勝
