...
對 G6 設計：威脅模型必須升級到 C-regional + C-blackout 兩個新能力。本堂不講方法論——只把 timeline 與 design implication 直接擺在你面前。
...
**為何 #5 重要**：fingerprint 簡單到 client 一抓 PCAP 就能識別「我被 Henan block 還是被 GFW block」。對 G6 telemetry 設計：RST payload pattern 必須記。
...
G6 spec v0.1 §1.3 / §11 威脅模型原本列 C1–C13。本堂後 **必須新增** 兩條：
...
兩條都不在 [[ensafi-gfw-probing]]、[[gfw-report-imc20-shadowsocks]]、[[wu-fep-detection]] 原本的威脅模型內。**這意味著 G6 多 transport profile（γ/β/α）的設計需 normative，不是 nice-to-have**。
...
**取得的研究結論**：GFW 對 paper disclosure 的反應週期 **約 60-90 天**（patch 部分能力），但**完整補洞要 12-18 個月**或更久（如 cross-datagram reassembly 至今未做）。**這給 G6 部署一個窗口**：每次 paper 公開 → GFW 反應前的 60-90 天，是「論文揭露的 bypass」最有效時間。
...
對 G6 含義：**「Russia-like」對手能力**是 G6 必須能對付的——這比中國 GFW 更激進。設計 implication：
...
- TSPU 的 **no-RST timeout** 是 G6 telemetry 死角——需 application-layer keepalive + 反向 idle detection。
...
把本堂結論轉成 G6 spec normative 變更建議：
...
在你的 G6 reference impl 中模擬 2025-08-20 事件：
...
2. Henan finding #4（20-byte TCP header 限制）對 G6 over TCP-cover 有什麼 free defense？
3. 為何 2025-08-20 事件的 incremental TTL fingerprint 引起學界關注？三種可能解釋分別 imply 哪種 GFW 內部架構？
4. GFW 對 paper disclosure 的「反應時程」大約多久？這個窗口期對 G6 部署策略有何含義？
5. Russian TSPU 的 15-20 KB freeze 攻擊：G6 在連線層面如何 detect？哪一個 application signal（用戶可觀察）最早能告訴你被 freeze？
