...
**HMAC-SHA256 / HMAC-SHA512 是 Proteus 的 KDF base**（透過 HKDF）。
...
**對 Proteus**：Proteus 不是 password-based protocol，但 future PSK 模式若 derive from user-typed passphrase 必走 Argon2id（Part 3.9 PAKE 詳論）。
...
- Proteus 默認 SHA-256 + BLAKE2s（雙軌），未來可升級 SHA-3 / BLAKE3。

### 9. Hash 在 Proteus 的具體用法
...
2. 為什麼 SHA-3 (sponge) 不需要 HMAC 也安全？對 Proteus 用 SHA-3 直接 KDF 是否合理？trade-off 是什麼？
...
對 Proteus：必須假設 quantum adversary。Hash output ≥ 384-bit 才能對抗 BHT 給 128-bit collision security。**Proteus 仍用 SHA-256 (256-bit output)**，因為 BHT 的 quantum collision attack 需要 quantum random access memory，現實 100+ 年內不可行；但要在 spec 內說明此假設。
...
flowchart TD
    ...
    Proteus_Hash[Proteus Hash Stack]
    Proteus_Hash --> Pri[Primary: SHA-256<br/>HKDF base, transcript]:::chosen
    Proteus_Hash --> Sec[Secondary: BLAKE2s<br/>noise mix-hash]:::chosen
    Proteus_Hash --> Future[Future: SHA-3 / BLAKE3<br/>hash agility]:::alt

    Proteus_Hash --> KDF[KDF: HKDF-SHA256<br/>RFC 5869]:::chosen
    Proteus_Hash --> MAC[MAC: HMAC-SHA256<br/>or Poly1305 in record]:::chosen
    Proteus_Hash --> Pwd[Password: Argon2id<br/>only if PSK derived from passphrase]:::chosen
    Proteus_Hash --> XOF[XOF: SHAKE128<br/>cover traffic padding<br/>Part 10]:::chosen
```
...
> **下一堂預告**：3.4 公鑰密碼學一：RSA — 數論基礎、RSA-OAEP/PSS、為什麼 textbook RSA 不能用、Bleichenbacher 1998 的 padding oracle、為什麼 Proteus 預設不選 RSA。
