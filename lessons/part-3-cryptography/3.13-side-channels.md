...
> 你的協議數學上完美 secure，實作仍可能被 trivially 破。本堂處理：side-channel 分類學、constant-time programming 不變量、Spectre/Meltdown 的 modern relevance、Proteus implementation 必須遵守的工程紀律。
...
**Proteus 規定**: 禁用 T-table-based AES; only AES-NI 或 bitsliced。
...
**Proteus 教訓**: 同 3.2 / 3.6 教訓，AEAD only。任何 protocol-level「驗證 → 不同 error response」都是 oracle。
...
**對 Proteus impact**:
- Proteus 主要 deploy on server / desktop with physical isolation → power analysis 不是 primary threat。
- 但 IoT Proteus client (smart router, smart TV with VPN built-in) 仍 vulnerable to physical adversary with oscilloscope。
...
**對 Proteus**: spec 要求 server deployment use frequency-scaling-off mode for crypto operations (impractical for high-throughput server, but document as known threat)。

### 10. Proteus implementation 的 constant-time 要求

```mermaid
flowchart TD
    ProteusCT[Proteus Constant-Time Requirements]
    ProteusCT --> Crypto[All crypto primitives<br/>constant-time]
    ...
    ProteusCT --> Compare[Tag/MAC verify constant-time]
    ProteusCT --> Swap[Conditional swap/select constant-time]

    ProteusCT --> NoOracle[No protocol-level error oracles]
    NoOracle --> SilentDrop[Drop invalid packets silently]
    NoOracle --> NoTimeDiff[No time differences in error paths]

    ProteusCT --> Spectre[Spectre-aware]
...
3. Bernstein cache-timing on AES 的 attack vector？Proteus 防禦策略？
...
6. Hertzbleed 對 「constant-time impl is enough」假設的衝擊？Proteus 實際部署如何 mitigate？
7. Proteus Rust impl vs C impl 在 constant-time 保證上的差別？
...
flowchart TD
    ...
    ProteusSC[Proteus Side-Channel Strategy]
    ProteusSC --> CT[All crypto code constant-time<br/>verified by ctgrind/dudect]:::chosen
    ProteusSC --> Lang[Rust impl 強制 constant-time guarantees]:::chosen
    ProteusSC --> Spectre[Spectre v1 mitigation:<br/>compiler hardening + lfence]:::chosen
    ProteusSC --> NoOracle[No protocol-level error oracles<br/>silent drop]:::chosen
    ProteusSC --> Hertz[Hertzbleed: documented threat<br/>recommend disable Turbo Boost]:::ack
    ProteusSC --> Power[Power analysis: out-of-scope for server]:::ack
    ProteusSC --> IoT[IoT Proteus client: documented physical threat]:::known
```
