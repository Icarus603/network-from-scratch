...
> 上一堂處理對稱加密 + hash；本堂進公鑰世界。RSA 是公鑰加密的祖宗（1978），主導 25 年但在 2000 年代被橢圓曲線取代。**G6 預設不選 RSA**——但你要知道為什麼不選；要能解 Bleichenbacher / Boneh / Coppersmith 都打過 RSA 哪些洞；以及 RSA-PSS / RSA-OAEP 在 hybrid PQ 時代仍有的角色。
...
對 G6：**不選 RSA-OAEP** for key exchange（用 ECDH/ECDHE 更好）。如果為了 backward compat 需要 RSA encryption，會用 RSA-OAEP-SHA256。
...
對 G6：**若需要 RSA cert auth（與舊系統互通）必用 RSA-PSS**。Long-term default 是 Ed25519，RSA 是 fallback。
...
**對 G6 教訓**：RNG 是 protocol 安全的單點故障。Part 3.12 詳論。

### 8. RSA vs ECC 的比較（為什麼 G6 選 ECC）
...
**G6 立場**：跳過 RSA，直接 Ed25519 + ML-DSA-65 (Dilithium-3) hybrid。
...
| 設計問題 | 本堂答案 | spec 位置 |
|---|---|---|
| G6 用 RSA 嗎？ | **不用**。簽章用 Ed25519 + ML-DSA hybrid | 11.5 |
...
6. Heninger 2012 的攻擊揭示 IoT RSA key 0.5% share primes。設計 G6 時要在哪一層防禦這風險？
...
- Quantum：assume Shor breaks RSA in 10-20 年，所以 G6 default 不用 RSA。
...
flowchart TD
    ...
    G6Auth[G6 Auth Stack]
    G6Auth --> Sig[Signature: Ed25519]:::chosen
    G6Auth --> PQ_Sig[PQ-hybrid: + ML-DSA-65]:::chosen
    G6Auth --> Cert_RSA[Cert verify support: RSA-PSS only]:::compat
    G6Auth --> Reject_OAEP[Do NOT use RSA-OAEP for KEX]:::avoid
    G6Auth --> Reject_PKCSv15[Reject PKCS#1 v1.5]:::avoid
```
