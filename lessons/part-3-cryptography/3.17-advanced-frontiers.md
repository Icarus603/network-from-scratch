...
> **本堂是「研究級補遺的補遺」**。3.1–3.16 已建出 Proteus v1 cryptographic stack；本堂處理 6 個**最近 5 年才成熟、但被 SOTA 協議完全忽略或處理不當**的議題。這些是 Proteus 真正能宣稱「超越 SOTA」的差異化空間。**新手不需馬上吸收，但 Phase III 11.3 設計空間探索與 11.7 安全考量必回頭引用本堂**。

---

## 動機：為何 16 堂仍不夠

3.16 已給 Proteus v1 完整 cryptographic spec。如果我們的目標只是「比現有 SOTA 不差」，到此可進 Phase II。但目標是**超越**：
...
flowchart LR
    ...
    Proteus[Proteus differentiator]:::adv
    Proteus --> Commit[Committing AEAD<br/>抗 partitioning oracle]:::adv
    Proteus --> KEMTLS[KEMTLS-style PQ AKE<br/>避大簽章 round-trip]:::adv
    Proteus --> Hybrid[Provable hybrid combiner<br/>Bindel 2019 standard]:::adv
    Proteus --> DR[Double Ratchet derived PCS<br/>細粒度 ratchet]:::adv
    Proteus --> DomSep[Strict domain separation<br/>protocol label hygiene]:::adv
    Proteus --> RAE[Robust AEAD<br/>對 reorder/replay 強化]:::adv
```
...
**Proteus PSK / OPAQUE 模式同樣 vulnerable**（若不修）：
- 若 Proteus v1 用 PSK + ChaCha20-Poly1305 record layer，且 server 對 incoming ciphertext 做「pre-handshake decrypt to identify session」(類 WireGuard cookie mechanism)，攻擊者可送 multi-key ciphertext partitioning 多個 candidate PSK。
...
**Proteus spec delta (相對 3.16 §4 Record Layer)**：

```text
Proteus v1.1 Record Layer (升級):
...
Result: 每 record +16 byte, 但 Proteus 完全免疫 partitioning oracle / multi-key collision attack。
        WireGuard / Hysteria2 / TUIC 均未做，Proteus 為 SOTA-first。
```

**Performance check**: 在 64-byte records (典型 IM payload) 約 +20% overhead；1500-byte (MTU) records 約 +1.5%。**default-on**，Proteus 不留開關（avoid downgrade）。
...
**Proteus 設計選擇 (Phase III 11.3 設計空間)**：

```mermaid
flowchart TD
    ProteusAuth[Proteus Mutual Auth Modes]
    ProteusAuth --> Mode_HybSig[Mode A: Hybrid Sig<br/>Ed25519 + ML-DSA-65]
    ProteusAuth --> Mode_KEMonly[Mode B: KEMTLS-style<br/>X25519+ML-KEM-768 only,<br/>no PQ sig]
    ProteusAuth --> Mode_Bal[Mode C: Hybrid Sig + Sig-Free Server<br/>Ed25519 client sig, KEMTLS server]
    
    classDef chosen fill:#cfc,stroke:#080
    Mode_Bal:::chosen
```

**Proteus v1 採 Mode C（hybrid）**：
...
**論文還沒解的問題**（Proteus 可貢獻）：
- KEMTLS + 0-RTT: 當前 KEMTLS spec 未支援 0-RTT；Proteus 可嘗試 PSK-based 0-RTT 與 KEMTLS server-auth 整合。
- KEMTLS + cover-traffic: KEM ciphertext 體積規律可能 fingerprintable（Hysteria2 已知問題）；Proteus 用 Elligator2-style obfuscation。

### 3. Hybrid KEM Combiner 形式化安全

**問題**：

Proteus (3.11) 已選 hybrid `X25519 ‖ ML-KEM-768`。但**怎麼 combine 兩個 shared secret 才 provably secure**？常見錯誤做法：
...
**Proteus spec delta (相對 3.11 §9)**：

```text
Proteus hybrid combine:
...
Justification: 直接套用 Bindel et al. 2019 Theorem 3.4 → "Proteus session key is IND-CCA2
...
Proteus 是 **synchronous proxy traffic**，不是 IM。連線中可能 every 10ms 來回 packet — 不需要 per-packet DH ratchet（成本太高，DH ~50µs per side > packet latency budget）。

```text
Proteus v1 Ratchet 設計（Signal-derived but coarser）：
...
Proteus ratchet 只做 single DH (~50µs) → 100× cheaper, same PCS guarantee。
```

**Out-of-order delivery handling**：

UDP-based Proteus 必須處理亂序 / 丟包。Signal Double Ratchet 用 skipped-message-keys cache:
...
**Proteus buffer 設計**: 沿用 Signal pattern，cap = 2^16 (對 Proteus typical record rate ~10K/sec → ~6sec coverage)。
...
正確 (Krawczyk HKDF 2010, RFC 5869):
    ...
    K_enc = HKDF-Expand(PRK, "proteus v1 encryption", 32)
    K_mac = HKDF-Expand(PRK, "proteus v1 mac", 32)
...
```text
Proteus HKDF-Expand-Label(Secret, Label, Context, Length):
    ProteusHkdfLabel struct:
        uint16 length = Length
        opaque label<8..255> = "g6_v1__ " + Label      // 8-byte fixed prefix
        opaque context<0..255> = Context
    return HKDF-Expand(Secret, ProteusHkdfLabel, Length)

Proteus 內所有 key derivation:
...
Cross-protocol invariant: 任何協議 (TLS 1.3, Noise, WireGuard) 的 label 與 Proteus 不可能碰撞.
                          因 prefix "g6_v1__ " unique。

Cross-version invariant: Proteus v2 用 "g6_v2__ "; v1/v2 不可能 cross-version key reuse。
```
...
**Proteus 為何仍用 SHA-256 而非 SHA-3**:
- HMAC-SHA-256 fix 了 indifferentiability (HMAC 結構 hide 內部 MD weakness)。
- Proteus 對 hash 的所有使用都包在 HMAC / HKDF (基於 HMAC) 之內 → 安全。
...
Proteus 採 explicit RAE-style record layer:
...
**Proteus spec delta** (vs 3.16 §4):

```text
Proteus record layer 採 XChaCha20-Poly1305-CTX (combine XChaCha20 + Committing AEAD):
...
Records-per-key practical limit: ~2^48 (vs WireGuard ~2^32)
                                  → 鬆 60K× 對 Proteus long-lived sessions 有意義
Nonce-misuse on crash: random_8 防 counter reset 後 nonce collision
                       (WireGuard 必 panic on counter rollback; Proteus graceful)
```
...
| Cross-protocol / cross-version key reuse | Proteus HKDF-Expand-Label with "g6_v1__ " prefix |
...
4. Proteus 採 "Mode C hybrid": Ed25519 client sig + KEMTLS server。為何不全用 KEMTLS？
5. Bindel et al. 2019 hybrid KEM combiner 為何要在 KDF input 含 ciphertext？XOR-only combine 為何不安全？
6. Signal Double Ratchet 的 DH ratchet 與 chain ratchet 各自貢獻什麼性質？Proteus 為何粗粒度足夠？
7. 為何 TLS 1.3 HKDF-Expand-Label 一定要 "tls13 " 前綴？跨協議 collision 的具體 scenario 是什麼？
8. SHA-2 Merkle-Damgård 不 indifferentiable，但 HMAC-SHA-256 在 Proteus 中安全。為什麼？
9. XChaCha20-Poly1305 24-byte nonce 為何給 birthday-bound 從 2^32 推到 2^48？
10. Proteus 為何 mandate "constant-time error code abstraction"？哪個側信道 attack scenario 觸發此設計？
...
### 5. Proteus 完整 spec delta vs 3.16
...
flowchart TD
    ...
    Spec316[Proteus v1 spec from 3.16]:::base
...
### 7. 開放問題（Proteus 可貢獻方向）

1. **KEMTLS + 0-RTT**: 當前 KEMTLS spec 無 0-RTT；Proteus PSK 模式整合 KEMTLS server auth 是 open research。
...
> **Phase I 真正完結**。Part 3 加 3.17 後，Proteus 的 cryptographic stack 已具備所有「超越 SOTA」必需的元件。Phase II (Part 6 WireGuard 開始) 將拆解現有 SOTA，並回頭以本堂的尺規評分：每個 SOTA 協議在這 7 個 differentiator 上做或沒做。
