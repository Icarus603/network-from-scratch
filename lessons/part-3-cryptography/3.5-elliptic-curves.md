...
> RSA 主導 1978-2005；ECC 是 1985-2005 開發、2005-2015 普及、2015+ 主導。本堂處理：橢圓曲線群結構、Curve25519 為何設計成那樣、Ed25519 簽章的 deterministic + sUF 性質、Ristretto255 解決 cofactor 問題、SafeCurves 評估準則。**Proteus 的 X25519 + Ed25519 全部從此堂出發**。
...
WireGuard 設計者 Donenfeld 直接寫：「Curve25519 is one of the most beautiful pieces of cryptography I've ever seen. It's fast, simple, and you almost can't misuse it.」這就是 Proteus 也要走 Curve25519/Ed25519 的核心理由。
...
2. **sUF-CMA (strong unforgeability)**: 同 (M, σ) 對唯一 valid signature，不存在 malleability。**Proteus 用此**。
...
**結論**：Curve25519 在 SafeCurves 表現幾乎 perfect。NIST P-curves 有多項問題（雖然不一定可 exploit，但 "robust" 不夠）。**Proteus 選 Curve25519**。
...
**Curve25519 對 Pollard rho 是 ~2^126 operations**，遠超實務。Quantum Shor 是真實威脅；故 Proteus 必須 PQ-hybrid。
...
| 設計問題 | 答案 |
|---|---|
| Proteus key exchange | X25519 (RFC 7748)，PSK 模式可選 |
| Proteus signature | Ed25519 (RFC 8032) + ML-DSA-65 (PQ hybrid) |
| Proteus transcript hash | SHA-256 (TLS 1.3 同款) |
...
let msg = b"Proteus handshake transcript hash bytes";
...
5. 為什麼 Ed25519 是 sUF-CMA 而 ECDSA 不是？signature malleability 在 Bitcoin / Proteus 各自什麼影響？
6. Cofactor 8 在 Edwards25519 為何存在？X25519 怎麼處理？Ristretto255 怎麼處理？
7. SafeCurves 對 P-256 critique 主要是哪些？這些 critique 對 Proteus 是否 deal-breaker？
...
- **Elligator** (Bernstein-Hamburg-Krasnova-Lange 2013)：map random bytes ↔ curve point indistinguishably。Proteus cover-traffic 用此把 ephemeral pk 偽裝成 random bytes。
...
flowchart TD
    ...
    ProteusKeys[Proteus Asymmetric Primitives]
    ProteusKeys --> KEX[Key Exchange: X25519]:::chosen
    ProteusKeys --> KEM[KEM hybrid: + Kyber768]:::pq
    ProteusKeys --> Sig[Sign: Ed25519]:::chosen
    ProteusKeys --> SigPQ[Sign hybrid: + ML-DSA-65]:::pq
    ProteusKeys --> Group[Prime-order group ops: Ristretto255]:::chosen
    ProteusKeys --> Compat[RFC 7748/8032 compat: WireGuard互通]:::compat
    ProteusKeys --> Disguise[Elligator2 for pk-as-bytes cover]:::chosen
```
