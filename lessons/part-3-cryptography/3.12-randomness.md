...
> RNG 是整個 cryptographic stack 的單點故障——RNG 弱化 → 所有 keys/nonces/IVs 全死。歷史災難一籮筐：Debian OpenSSL 2008 (Bug #363516)、Sony PS3 2010、Android Bitcoin wallet 2013、Heninger 2012 IoT shared primes、Dual_EC_DRBG backdoor (Snowden 2013)。本堂處理：entropy source、CSPRNG 設計、實作 pitfalls、Proteus 的 RNG strategy。
...
**Proteus 用**: Linux `getrandom(2)` → kernel ChaCha20-based CSPRNG with multiple entropy sources。
...
**Proteus IoT-target deployment**: spec 要求 device 在 ≥ 256-bit entropy seeded **after** KGen.
...
3. Heninger 2012 shared-prime attack 的 root cause 是什麼？Proteus IoT 部署如何防？
4. Dual_EC_DRBG backdoor mechanism 是什麼？對 Proteus 「avoid magic constants」原則的具體啟示？
5. fork() 後 userspace PRNG state 為何 dangerous？如何 mitigate？
6. RDRAND 對 modern crypto 應該 sole source 還是 mixed source？理由？
7. Proteus cover-traffic timing jitter 用 RNG output；entropy 不足會 leak 什麼？
...
flowchart TD
    ...
    ProteusRNG[Proteus RNG Stack]
    ProteusRNG --> OS[OS getrandom syscall<br/>Linux/macOS/Windows]:::chosen
    ProteusRNG --> NoDevRand[/dev/random no longer use/]:::avoid
    ProteusRNG --> NoRand[rand() / pseudo prohibited]:::avoid
    ProteusRNG --> Reseed[Reseed userspace CSPRNG every N ops]:::chosen
    ProteusRNG --> ForkSafe[fork() handler reseeds]:::chosen
    ProteusRNG --> MagicAudit[No magic constants without rigid origin]:::chosen
```
