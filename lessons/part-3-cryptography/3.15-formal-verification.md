...
> 「Formal verification 是 protocol-level 安全的最高 standard」。Needham-Schroeder protocol 1978 設計後 17 年才被 Lowe 1995 用 FDR (model checker) 找到 bug。TLS 1.3 設計過程與 Tamarin/ProVerif 共同進化。Proteus 必須跟此 standard——Phase III 11.10 要產 ProVerif model 與 Tamarin proof。
...
**Proteus 計畫**: Phase III 11.10 用 ProVerif/Tamarin 驗證 Proteus IK variant 的 secrecy + authentication + KCI/UKS resistance + FS + PCS。
...
**Proteus ProVerif 計畫**:
- Model Proteus IK variant + cover-traffic disguise + PSK extension。
- Verify queries:
...
**Tamarin Proteus 計畫**：
- 對 Proteus IK + ratchet variant 證 PCS。
- 對 Proteus multi-device scenario 證 no Selfie attack。
- 對 Proteus PQ-hybrid 證 hybrid security (each share independent)。
...
**Proteus implementation 是否 follow Project Everest?** —
- 短期: Proteus v1 用 ring (well-audited, not formally verified primitive)。
- 中期: 評估 EverCrypt Rust binding for primitive layer。
- 長期: Proteus implementation 自身 F* / Coq 驗證 (research direction)。
...
**Proteus 用 Noise Explorer**:
- 把 Proteus spec encode 為 Noise pattern variant + custom tokens (PQ KEM, cover-traffic)。
...
**綜合**: formal verification 是 baseline + necessary, 不是 sufficient. Proteus 需要：
...
### 8. Proteus Phase III 形式化驗證 plan

```mermaid
flowchart TD
    ProteusVerif[Proteus Formal Verification Pipeline]
    ProteusVerif --> Step1[Step 1: Tamarin model<br/>full Proteus IK variant + PSK + ratchet]
    ProteusVerif --> Step2[Step 2: ProVerif model<br/>Noise Explorer-derived + manual extensions]
    ProteusVerif --> Step3[Step 3: CryptoVerif handshake proof<br/>computational]
    ProteusVerif --> Step4[Step 4: ctgrind/dudect on Rust impl]
    ProteusVerif --> Step5[Step 5: Independent code review]
...
| Spec → model translation | follow Noise Explorer pipeline + manual for Proteus extensions |
...
5. Noise Explorer 自動化哪些步驟？Proteus 用它能 cover 多少？需要 manual 補哪些？
6. WireGuard 怎麼與 formal verification co-evolve？對 Proteus 的 design lesson 是什麼？
7. 對 Proteus 來說，formal verification 是 sufficient 還是 necessary？另需哪些 verification?
...
flowchart TD
    ...
    ProteusFV[Proteus Formal Verification]
    ProteusFV --> Symbolic[ProVerif + Tamarin<br/>Phase III 11.10]:::plan
    ProteusFV --> Comp[CryptoVerif (handshake)<br/>Phase III 11.11]:::plan
    ProteusFV --> Impl[ctgrind/dudect for Rust impl]:::plan
    ProteusFV --> Audit[3rd party security audit]:::plan
    ProteusFV --> Future_FV[Future: F*/Coq verified Proteus impl]:::research
    ProteusFV --> NoiseExpl[Noise Explorer-derived initial model]:::plan
```
...
> **下一堂預告**：3.16 整合 — Phase I 密碼學的期末考；設計 protocol 時用什麼 primitive 的決策樹；Proteus 完整 cryptographic stack。
