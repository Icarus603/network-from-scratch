...
**對 Proteus 候選**：
- **IK**：1-RTT，client 假設 server 公鑰已知 (out-of-band)。WireGuard 範式。**Proteus 採此**。
...
**Proteus 選 IK**：1-RTT 對 proxy 場景重要；0-RTT data 是 cover-traffic 設計關鍵。Initiator identity protection 透過額外 layer（cover document hash）達成。
...
**Proteus 計畫**:
- 用 Noise Explorer generate ProVerif model for Proteus IK variant。
- Manual extend ProVerif model to cover Proteus's cover-traffic + PSK + PQ-hybrid 變動。
...
**Proteus 偏向 Noise**: 小、modular、formally verified、與 WireGuard 互通 friendly。
...
| Formal verification | Noise Explorer + manual extension for Proteus |
...
3. WireGuard 的 MAC1 與 MAC2 各自防什麼 attack？Proteus 如何借鑑？
4. PSK 在 Noise 中怎麼混入 chaining key？對 PQ hybrid 為何關鍵？
5. fACCE framework 與 Noise 各自 security model 差異？哪些 Proteus 該採用？
6. 用 Noise Explorer 找出 IK pattern 的「unknown key share attack」是否存在？
7. Proteus 如果想做「對 active MitM with responder static 仍 identity-protect initiator」要 switch to 哪個 pattern？代價？
...
flowchart TD
    ...
    ProteusNoise[Proteus Handshake = Noise IK + extensions]
    ProteusNoise --> Base[Base: Noise_IK_25519_ChaChaPoly_BLAKE2s]:::chosen
    ProteusNoise --> PSK[PSK extension for quantum-safe hybrid]:::chosen
    ProteusNoise --> PQ_KEM[+ Kyber768 KEM in custom token]:::extend
    ProteusNoise --> MAC1[MAC1 anti-DoS (WireGuard-style)]:::chosen
    ProteusNoise --> Cookie[Cookie reply for spoofed IP defense]:::chosen
    ProteusNoise --> CoverDisguise[Elligator2-disguised ephemeral pk]:::extend
    ProteusNoise --> Ratchet[Per-N-record DH ratchet for fine PCS]:::extend
```
