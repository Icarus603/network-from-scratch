...
> 3.4 講 RSA signature；3.5 講 Ed25519 細節。本堂統合：簽章設計 design space + ECDSA 與 EdDSA 的精確比較 + Schnorr aggregation + BLS pairing-based + Certificate Transparency。G6 用 Ed25519，但 verify ECDSA / RSA cert 必須兼容。
...
**G6 不直接用 aggregation**（單 client-server，不是 BFT）但 **borrowing for future group mode**。
...
**對 G6**：
- 不直接用 CT（G6 不是 cert-based PKI）。
- 但 G6 可以 borrow Merkle tree 結構做 **transparency log for protocol version updates**——讓 attacker 無法給特定 client 推 backdoor version。

### 7. 簽章在 G6 的角色

```mermaid
flowchart TD
    G6Sig[G6 Signature Use Cases]
    G6Sig --> HSAuth[Handshake authentication<br/>Ed25519 sign transcript hash]:::main
    G6Sig --> CertVerify[PKI cert verify<br/>RSA-PSS / ECDSA-P256 / Ed25519]:::main
    G6Sig --> PQHybrid[PQ-hybrid sig<br/>Ed25519 ‖ ML-DSA-65]:::main
    G6Sig --> SoftwareUpdate[Software update signing<br/>Ed25519 with reproducible build]:::aux
    G6Sig --> Transparency[Update transparency log<br/>Merkle tree per CT pattern]:::aux
...
7. G6 用 Ed25519，對 TLS PKI 上的 ECDSA cert 該如何 verify? 必加什麼防護？
...
flowchart TD
    ...
    G6S[G6 Signature]
    G6S --> Main[Primary: Ed25519]:::chosen
    G6S --> PQ[PQ hybrid: ML-DSA-65]:::chosen
    G6S --> Pki[PKI compat: RSA-PSS, ECDSA-P256]:::compat
    G6S --> ECDSACheck[ECDSA verify with low-s enforcement]:::compat
    G6S --> LogCT[Update log: Merkle tree, CT-inspired]:::chosen
```
