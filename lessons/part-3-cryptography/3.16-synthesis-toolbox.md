...
- **必讀資源**：本堂不額外加 paper；綜合前 15 堂的 design choice 為**Proteus v1 cryptographic specification**。

> **本堂是 Phase I 密碼學的期末考**。把前 15 堂工具整理成「設計新協議時你會選什麼」的決策樹 + Proteus v1 完整 spec snapshot。Phase II 開始拆解現有 SOTA 協議；Phase III 設計 Proteus——所有 design 決定都源自本堂的 toolkit。
...
flowchart TD
    ...
    Start --> SpecDone[Proteus v1 Spec]
    A1 -.feeds.-> SpecDone
    A2 -.feeds.-> SpecDone
    A3 -.feeds.-> SpecDone
    A4 -.feeds.-> SpecDone
...
## 核心：Proteus v1 Cryptographic Specification（Phase I 期末成果）

以下是 Phase I 結束時 Proteus spec 的 cryptographic 決定 snapshot。Phase II 會繼續精煉 (handshake message 細節、cover traffic、anti-DoS)；Phase III 完整 spec。
...
- Decoy-Indistinguishability (Proteus unique; cover traffic shaping)。
...
Effect:
    ...
    Difficulty distinguishing Proteus handshake from random UDP payload.
...
## 動手：寫一份 Proteus v1 spec abbreviated table

```markdown
# Proteus v1 Cryptographic Specification (snapshot)
...
1. 為什麼 Proteus 用 X25519 而非 NIST P-256？(→ 3.5 SafeCurves)
...
5. WireGuard MAC1 / MAC2 各防什麼 attack？Proteus 怎麼借用？(→ 3.8)
6. OPAQUE 與 Argon2-PSK 對 server compromise 場景的差別？(→ 3.9)
7. 為什麼 Proteus 的 ratchet 給粗粒度 PCS, 不像 Signal 給 fine-grained？trade-off?(→ 3.6 + 3.8)
8. RSA / NIST P-curves / Dual_EC_DRBG 的歷史教訓對 Proteus 「avoid magic」原則的影響？(→ 3.4 + 3.5 + 3.12)
9. Hertzbleed 為何顛覆「constant-time enough」的假設？Proteus 如何 mitigate？(→ 3.13)
10. ProVerif + Tamarin + CryptoVerif 的 verification 各覆蓋哪些 property？Proteus 為何 triple-check?(→ 3.15)
...
- **Proteus spec terminology**: protocol_name, transcript_hash, chaining_key, MixHash, MixKey, Split, MAC1, MAC2 — 與 Noise / WireGuard 對齊。
- **Hybrid combination**: HKDF(ck, dh1 ‖ dh2) — concatenation 是 IETF CFRG hybrid spec 推薦。
- **Decoy-indistinguishability**: Proteus-specific term，formalization 待 Phase III research。
- **Verifier graph**: ProVerif / Tamarin internal proof structure。
- **Game hops**: CryptoVerif game transformation chain steps。
- **eCK+ model**: Proteus spec target for AKE security proof。

### 2. 對手分類學總覽

```mermaid
flowchart TD
    ProteusAdv[Proteus Adversary Spectrum]
    ProteusAdv --> Net[Network adversary<br/>Dolev-Yao]
    Net --> Active[Active MitM]
    Net --> Passive[Passive observer]

    ProteusAdv --> Compromise[Endpoint compromise]
...
Proteus spec 應包含以下 formal definitions explicit：
...
- **Decoy-indistinguishability**: PPT distinguisher between Proteus traffic and cover protocol traffic ≤ ε (formalization pending Phase III)。
...
flowchart TD
    ...
    Proteus [Proteus Cryptographic Spec — Phase I exit]:::done
    Proteus --> Adversary[Threat model defined]:::done
    Proteus --> Primitives[All primitives chosen]:::done
    Proteus --> Protocol[Handshake structure outlined]:::done
    Proteus --> Verification[Verification plan defined]:::done

    Proteus --> P2[Phase II: SOTA dissection]:::phase2
    P2 --> WireGuard_dissect[WireGuard line-by-line]
    P2 --> Xray[Xray + REALITY internals]
    P2 --> QUIC[Hysteria2 + TUIC]
    P2 --> GFW[GFW.report papers]

    Proteus --> P3[Phase III: Design + Build]:::phase3
...
從 Part 3 累積，這些 open problem 可能成為 Proteus research contribution：

1. **Decoy-indistinguishability formal definition + proof for Proteus design**（Part 11.10 / 12.17）。
...
5. **Verified end-to-end implementation of Proteus**：HACL\* 證 primitive + ProVerif 證 protocol + Vale 證 assembly — full chain still research。
...
>
> **補遺**：[3.17 進階補遺：超越 SOTA 必備的密碼學深水區](./3.17-advanced-frontiers.md) — 本堂 §2 Proteus v1 spec 在 3.17 升級為 v1.1（committing AEAD、KEMTLS Mode C、Bindel hybrid combiner、Double Ratchet-derived 粗粒度 PCS、嚴格 domain separation、XChaCha20 BBB、RAE error path）。Phase III 11.3 與 11.7 回頭引用。
