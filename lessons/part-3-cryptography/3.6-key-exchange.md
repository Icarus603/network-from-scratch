...
> 3.1 已交代 SIGMA / KCI / UKS / FS / PCS 概念。本堂處理具體 protocol design：DH 怎麼從 1976 純 KEX 演化到 2026 modern AKE；為什麼 SIGMA 結構成為 TLS 1.3 / IPsec / Noise / WireGuard / Signal 共同骨架；Proteus 握手要選哪個。
...
flowchart TD
    ...
    Noise --> Ours[Proteus<br/>(我們的協議)]:::ours
...
Proteus 站在兩條傳統交匯：SIGMA-I 結構（TLS 1.3 / Noise IK）+ X3DH/Double Ratchet 思想（PCS via per-message ratchet）。
...
**Proteus 要在哪個 model 下證明？** Spec 寫 eCK + KCI-resistance + PCS。對應 SIGMA + ratchet 結構。
...
### 3. SIGMA-I：TLS 1.3 / Noise IK / Proteus 共同骨架
...
**Proteus 不選 HMQV**：選 SIGMA-I (Ed25519 signature) 更符合 modern industry default。
...
**Proteus 不直接用 X3DH**（Proteus 是 synchronous proxy 不是 messaging）但**借鑑兩件事**：
1. Multiple-DH combine → robust against single-key compromise。
2. One-time prekey 概念可用在 Proteus 的 "cover-traffic seed" 設計。
...
**Proteus 採用 Noise IK 變體**：加上 PQ hybrid (Kyber768 KEM) 與 cover-traffic disguise。
...
**Proteus 教訓**：
- **Hard-code curve**: Proteus 只允許 X25519 + (PQ-hybrid Kyber768)。No negotiable group。
...
**對 Proteus**：spec 必須處理 multi-device case + identifier binding。
...
| 設計問題 | 答案 |
|---|---|
| Proteus 握手結構 | Noise IK pattern (1-RTT) + SIGMA-I-style transcript binding |
...
5. Logjam 的 root cause 是 cipher negotiation 不夠 secure。Proteus 如何 architectually 避免？
6. eCK model 與 CK model 差別？Proteus 證明選哪個？為什麼？
7. Signal X3DH 的 Selfie attack 怎麼發生？Proteus multi-device 場景怎麼預防？
...
flowchart TD
    ...
    ProteusKE[Proteus AKE Stack]
    ProteusKE --> Pattern[Noise IK pattern]:::chosen
    ProteusKE --> PrimKE[Primitives: X25519 + Kyber768 hybrid]:::chosen
    ProteusKE --> Sig[Authentication: Ed25519 + ML-DSA-65]:::chosen
    ProteusKE --> Transcript[Transcript bind ciphersuite + identities]:::chosen
    ProteusKE --> Ratchet[Per-N-record DH ratchet for PCS]:::inspire
    ProteusKE --> ZeroRTT[Optional PSK 0-RTT for idempotent only]:::inspire
    ProteusKE --> Disguise[Elligator2 on ephemeral pk for cover]:::chosen
    ProteusKE --> Proof[eCK+ model proof + ProVerif symbolic]:::chosen
```
