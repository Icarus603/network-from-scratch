...
> 前 13 堂建工具；本堂教你「**正確地用工具**」。Latacora "Cryptographic Right Answers" 是 modern crypto engineering 的 manifesto；libsodium 是「正確 API」的範本；HACL\* 是「正確實作」的範本。Proteus 的 crypto layer 必須選 high-level safe API，不能自製 primitive。
...
**Proteus 選 ring (Rust)**: type-safe + memory-safe + BoringSSL-derived + audited。
...
**Proteus 對 verified crypto 立場**:
- 短期: ring (well-audited 但非 formally verified)。
- 中期: 評估 HACL\* / EverCrypt Rust binding (e.g., `evercrypt` crate)。
- 長期: 對 Proteus implementation 做 verified compilation。

### 5. API design 對 Proteus implementation 的 impact

**Proteus API hierarchy**:
```mermaid
flowchart TD
...
| Library | Lang | License | Verified | API Style | Proteus 適用 |
|---|---|---|---|---|---|
...
**Proteus primary**: ring + rustls (Rust ecosystem)。Future evaluate HACL\* bindings。
...
## 動手：用 ring 寫一個 minimal Proteus prototype
...
fn handshake() -> Result<(), ring::error::Unspecified> {
    ...
    let shared = agreement::agree_ephemeral(
        ...
        |key_material| {
            ...
            prk.expand(&[b"proteus session key"], ring::hkdf::HKDF_SHA256).unwrap().fill(&mut session_key).unwrap();
            Ok(session_key)
        }
    )?;
    ...
    let mut data = b"hello Proteus".to_vec();
...
1. Latacora 2018 right answers 表中，Proteus 對每項選擇與 default 一致嗎？哪裡偏離？為什麼？
2. NaCl 「Operations not algorithms」設計哲學在 Proteus 怎麼體現？user/plugin/crypto 三層 API 邊界？
3. OpenSSL Heartbleed 的 root cause？Proteus 用 ring + Rust 為何 architecturally 免疫此類 bug？
4. HACL\* / EverCrypt 「verified crypto」對 Proteus v1 是否 mandatory？trade-off 是什麼？
5. Crypto agility 在 Proteus 怎麼處理？version-based vs per-handshake negotiation 的選擇？
6. 為什麼 Proteus 用 ring 而非 OpenSSL (即使 OpenSSL 3.0 在 ML-KEM 支援更完整)？
...
flowchart TD
    ...
    ProteusEng[Proteus Crypto Engineering]
    ProteusEng --> Lib[ring + rustls (Rust)]:::now
    ProteusEng --> Verify[ctgrind + dudect constant-time audit]:::now
    ProteusEng --> RustSafety[Rust memory safety + no unsafe in crypto path]:::now
    ProteusEng --> SpecSimple[Hard-coded ciphers per version]:::now
    ProteusEng --> APILayer[3-layer API: user / protocol / crypto]:::now
    ProteusEng --> Hacl[Future: HACL* verified primitives evaluation]:::future
    ProteusEng --> EverCrypt[Future: EverCrypt evaluation]:::future
    ProteusEng --> FullVerify[Future: full Proteus protocol verified]:::future
```
