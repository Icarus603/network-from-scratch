...
> 上一堂說「G6 record layer 必須用 AEAD」。這一堂深入到「**為什麼是這兩個 (ChaCha20-Poly1305 / AES-GCM)**、別的不行；逐 quarter-round 看 ChaCha20 怎麼工作；為什麼 GCM 一旦 nonce 重用整個 key 都崩；為什麼 AES-NI 改變了硬體與軟體加密的權力均衡」。
...
對 G6 而言，這堂要產出**結論**：record layer 預設 ChaCha20-Poly1305，AES-256-GCM 為硬體加速 fallback；nonce 用 epoch+counter 結構保證唯一；達 ~5 Gbps 線速處理（Part 11 Section 11.5 直接寫進 spec）。
...
**對 G6 意義**：AES-256 給 256-bit security against classical, 128-bit against quantum (Grover)，是 G6 在硬體加速場景的選項。
...
### 6. ChaCha20-Poly1305 (RFC 8439) — G6 預設 AEAD
...
| AEAD | Nonce | Tag | 速度 (cycles/byte, Skylake AES-NI) | Misuse-Resistant | RFC | G6 用途 |
|---|---|---|---|---|---|---|
...
**為什麼這樣分**：
- **Direction byte 不可省**。Bernstein-Salsa 著名 pitfall：兩端用同 key 同 nonce 加密 → forbidden attack。WireGuard 用 separate send/recv key 避開；G6 用 direction byte（更節省 key material）。
...
**G6 implementation rule**：禁止使用任何 T-table-based AES 實作；只用 AES-NI 或 bitsliced。ChaCha20 因 ARX 設計天然 constant-time，無此問題。
...
**對 G6 spec 影響**：必須在 Security Considerations 寫明 multi-user analysis；reference Bellare-Tackmann 2016 + 給 concrete numbers (e.g., "with 10M users × 2^30 records each, advantage ≤ 2^-30")。
...
6. 為什麼 AEAD 輸入有 associated data (A)？舉一個 G6 場景，A 該包含什麼。
...
對 G6：standard model + nonce-misuse-aware（0-RTT）+ side-channel-resistant（constant-time impl）。Multi-user 必證。RUP 不在 spec 範圍但 implementation 要 ensure 不 release plaintext until verify pass。
...
6. **Rogaway, Bellare, Black, *OCB* (TISSEC 2003)** — 最快 AEAD 之一；G6 不選但讀 design 思想。
...
flowchart TD
    ...
    G6_AEAD[G6 Record AEAD]
    G6_AEAD --> Default[Default: ChaCha20-Poly1305<br/>RFC 8439]:::chosen
    G6_AEAD --> HW[Hardware path: AES-256-GCM<br/>when AES-NI available]:::chosen
    G6_AEAD --> ZeroRTT[0-RTT data: AES-GCM-SIV<br/>RFC 8452]:::open
    G6_AEAD --> Reject_OCB[Reject: OCB3<br/>IPR conservatism]:::rejected
    G6_AEAD --> Reject_AEGIS[Defer: AEGIS-128L<br/>still standardizing]:::rejected
    G6_AEAD --> Future_Ascon[Future: Ascon<br/>NIST LWC standard]:::open

    G6_AEAD --> Nonce[Nonce structure<br/>6B epoch + 1B dir + 5B counter]:::chosen
    G6_AEAD --> Tag[Tag: 128-bit fixed]:::chosen
    G6_AEAD --> CT[Constant-time impl required]:::chosen
```
...
- **AEAD with traffic-shaping integration**：對 G6 的 cover traffic 場景，AEAD 是否能 inherently 提供 length-hiding？目前需另外加 padding scheme。
