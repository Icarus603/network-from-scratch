# 課堂 8.12 — Proteus 在 MASQUE 上的 wire-format mapping：實作藍圖
...
- 前置課：
  ...
  - [11.5 Proteus wire format spec](../part-11-design/11.5-spec-wire-format.md)
  - [11.6 Proteus handshake state](../part-11-design/11.6-spec-handshake-state.md)
- 預計閱讀時間：**45 分鐘**
- 必讀規格：
  ...
  - Proteus spec `assets/spec/proteus-v0.1.md` §3, §4, §5, §7

## 動機

Proteus spec §3 列出三個 transport profile：
...
但**spec §4 給出的 wire format（`ProteusAuthExtension`、`ProteusInnerHeader`）沒有寫明：當走 γ profile 時，這些 byte 怎麼放進 MASQUE capsule / HTTP Datagram / 0xfe0d TLS 擴展**。

換句話說，**spec 給了「位元結構」，但缺「位元結構 ↔ MASQUE 各層位置」的綁定**。沒有這個 binding，Part 12 implementation 寫不出 byte-exact code。

本堂解決：

1. Proteus client 啟動到 application data 全程的位元順序圖
2. `ProteusAuthExtension` 怎麼塞進 QUIC ClientHello 內的 TLS extension
3. `ProteusInnerHeader` 怎麼包進 CONNECT-UDP HTTP Datagram / Capsule
...
sequenceDiagram
    ...
    participant S as Proteus Server (also H/3 + cover)
    ...
    alt HMAC ok
        ...
        loop application data
            C->>Net: UDP[QUIC DATAGRAM(quarter_stream_id || ProteusInnerHeader || payload)]
            S->>Net: UDP[QUIC DATAGRAM(quarter_stream_id || ProteusInnerHeader || payload)]
        end
    else HMAC fail
...
九個 step 是 Proteus γ profile 的全部 wire interaction。本堂逐 step 拆 byte 解。
...
`Payload` 內的 CRYPTO frame 解開後是 TLS ClientHello。Proteus 在這裡掛上 `extension_type = 0xfe0d`：
...
TLS Extension (RFC 8446 §4.2 generic frame):
  ...
  opaque  extension_data[1186]:
    ...
    struct ProteusAuthExtension {                  // spec §4.1
...
- [[wu-henan-sp25]] / [[zohaib-quic-sni-usenix25]] 揭示 censor 對 multi-datagram CRYPTO reassembly 仍不穩 (GFW 部分修, Henan 完全沒做)。**依賴 reassembly = 依賴 censor 之 bug**。Proteus 不依賴。
...
對應 REALITY 差異：REALITY 透明轉發給 cover 來生 cert；Proteus server **直接持有 cover cert**，自己回。這是 Proteus vs REALITY 一個關鍵簡化（也是限制——server admin 必須能控 cover domain DNS 或借真實 cover）。
...
SETTINGS frame (RFC 9114 §7.2.4):
  ...
  Settings (varint-pair list):
    ...
    [Proteus reserved] 0xfeff = 1   // 表示 server speaks Proteus inner protocol
```
...
HTTP/3 HEADERS frame on a new client-initiated bidi stream:
  ...
  user-agent = <Proteus-imitating-Chrome>
```

對 Proteus：

- `target_host` / `target_port` **不是真正的 target**——是 Proteus 用來標識 session 的 dummy。spec §4.2 inner packet 才帶真正的 target。
- 為何 dummy？因為 Proteus 是一個 multi-target proxy；CONNECT-UDP 一次 request 對一個 (host:port)，Proteus 在 inner-frame 自行 demux。
...
### 5. Step 6: HTTP Datagram 載 Proteus inner packet
...
#### 5.3 Proteus 在 `UDP Proxying Payload` 內塞 ProteusInnerHeader

**Proteus 的關鍵設計**：把 `UDP Proxying Payload` 解釋為 Proteus framing：
...
flowchart TD
    ...
    D["Proteus Inner Datagram"]
    E["ProteusInnerHeader (12 byte) — spec §4.2<br/>uint8 type (0x01 DATA / 0x02 ACK / 0x03 NEW_STREAM / 0x07 PADDING)<br/>uint8 flags<br/>uint16 reserved (MUST be 0, else reject)<br/>uint64 seqnum"]
    F["Proteus Inner Payload<br/>(bytes, type-dependent)"]
    A --> B --> C --> D
    D --> E
    D --> F
...
`Context ID = 0` 因 Proteus 對每一條 associated stream **僅用一個 context**（沒有 per-target context demux，demux 由 inner `ProteusInnerHeader` 完成）。
...
UDP datagram (1280 byte total):
  ...
  ProteusInnerHeader #1                   (12 byte) + payload data (X byte)
  ProteusInnerHeader #2 (type=0x07 PADDING) + padding bytes to fill remaining
  ...
```

`X` 由 effective MTU 反算：`1280 - 5 (QUIC hdr) - 4 (frame hdr) - 2 (HTTP DG) - 1 (Context) - 12 (Proteus hdr) = ~1256 byte data per cell`。

**多 inner packet per UDP datagram 的設計選擇**：spec §4.3 預設**一個 UDP datagram 內可以有多個 Proteus inner packet**（用 12-byte header 分隔）。理由：
...
5. If HMAC verify OK:
   a. Continue Proteus path (Step 2-3 above).
```
...
- 否則「冷啟 TCP/QUIC 到 cover_url」要 ~50-100 ms → 立刻被 fingerprint 為 Proteus server。
...
- 0xfe0d ext (1186 byte) = Proteus KEM auth
...
```
QUIC DATAGRAM frame:
  ProteusInnerHeader (12 byte) + payload
```

（無 H/3 / Quarter Stream ID / Context ID 層；直接 Proteus frame）
...
| §4.3 | 1280-byte alignment 公式: `1280 - QUIC_hdr - frame_hdr - HTTP_DG - context_id - Proteus_hdr = 1256 byte payload` |
...
### 任務 2：寫 Proteus in/out wire codec test

對 quic-go/masque-go 的 `connect-udp` 樣板代碼，加入：

```go
// pseudocode
type ProteusInnerHeader struct {
    Type     uint8
    Flags    uint8
    Reserved uint16
    Seqnum   uint64
...
func (h *ProteusInnerHeader) Marshal() []byte { ... }
func ParseProteusInnerHeader(b []byte) (*ProteusInnerHeader, []byte, error) { ... }

// round-trip test
for typ := 0x01..0x07 {
    h := ProteusInnerHeader{Type: typ, Seqnum: rand.Uint64()}
    out := h.Marshal()
    h2, rest, err := ParseProteusInnerHeader(out)
...
2. Quarter Stream ID 為何要 `>> 2`？對 Proteus 一個 session 是否能用 multiple H/3 streams 並列？
...
6. 為何 Proteus 不採 ECH 作為 outer SNI 隱藏層？兩個技術原因 + 一個部署原因。
...
**Wire indistinguishability claim (Proteus γ profile)**: For any PPT distinguisher D with access to wire traffic on UDP/443 between a real Chrome+H/3 client and a Proteus client (both connecting to a Cloudflare-cover host), `Pr[D outputs Proteus] ≤ 1/2 + ε(λ)` where ε is negligible in security parameter λ.
...
2. **OP-2**: CONNECT-IP（RFC 9484）路徑 vs CONNECT-UDP：未來 Proteus v0.2 是否該支援 L3 tunneling？對 latency / framing overhead 影響待測。
3. **OP-3**: 與 EHIP / connect-quic-proxy（IETF draft）的對接：當這些 RFC 化後，Proteus 是否該升級到 quic-aware proxy 層？目前 spec 防守式 stay-at-CONNECT-UDP。
4. **OP-4**: Cover forward pool 對 cover server 視角是「一個 IP 大量並發 TCP/443 連線」——CDN 反爬蟲是否會封 Proteus server IP？需 measurement。
